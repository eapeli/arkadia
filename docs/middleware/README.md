...
Damon middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Damon'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Damon calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Damon retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Damon guardrails,
...
- `telemetry_schema_version`: currently `damon.observer.v1`
- `middleware_schema_version`: currently `damon.middleware.v1`
...
Damon stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Damon runs
them as a nested chain in registration order. Middleware failures are fail-open:
Damon logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Damon applies middleware in this order:
...
For each tool call, Damon applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Damon pre-execution path against the effective arguments:
...
```bash
damon plugins enable <plugin-name>
```

For isolated local testing, use one `DAMON_HOME` for plugin enablement and the
agent run:

```bash
export DAMON_HOME=/tmp/damon-middleware-test
mkdir -p "$DAMON_HOME"
damon plugins enable <plugin-name>
damon chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run damon plugins enable <plugin-name>
uv run damon chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["damon_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/damon-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Damon expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Damon treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Damon preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Damon does not convert downstream
