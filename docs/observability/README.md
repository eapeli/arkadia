# Damon Observer Hooks

Damon observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Damon' planner, model providers, memory, tool registry,
...
```text
telemetry_schema_version = "damon.observer.v1"
```

Hook callbacks are fail-open. Damon catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Damon only builds
