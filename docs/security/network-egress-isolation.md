...
When running Damon inside Docker, the default `network_mode: host` gives the
...
The Damon [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   DAMON_UID=$(id -u) DAMON_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=damon,damon-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://damon-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Damon trust model and vulnerability reporting
