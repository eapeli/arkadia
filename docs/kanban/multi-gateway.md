...
Damon supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.damon/config.yaml`:
...
Or set the env var: `DAMON_KANBAN_DISPATCH_IN_GATEWAY=false`
