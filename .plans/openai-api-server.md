...
SSE streaming. By exposing this endpoint, damon-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  damon-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.damon/.env`
...
4. Chat with damon-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns damon-agent as a model) |
...
Authorization: Bearer damon-api-key-here
...
But damon-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "damon-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured damon-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of damon-agent's core prompt
...
Once implemented, damon-agent works as a drop-in backend for:
