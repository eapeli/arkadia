...
load_damon_dotenv(
    damon_home=_damon_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.damon/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_damon_home, "logs", "tui_gateway_crash.log")
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_damon_home).resolve():
        return None
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            session_db = None
            if profile_home:
                home_token = set_damon_home_override(profile_home)
                try:
                    from damon_state import SessionDB
            ...
            if home_token is not None:
                reset_damon_home_override(home_token)
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _damon_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_damon_home_override()
        home = override if isinstance(override, str) and override else _damon_home
...
def resolve_skin() -> dict:
    try:
        from damon_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from damon_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from damon_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(
    ...
    *,
    ...
    from damon_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from damon_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from damon_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from damon_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from damon_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from damon_cli.config import load_config as _load_full_cfg
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses damon_cli.mcp_startup; TUI stdio keeps
    ...
    if isinstance(model_override, dict) and model_override.get("model"):
        ...
        if (
            ...
            from damon_cli.runtime_provider import find_custom_provider_identity
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_damon_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_damon_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_damon_home_override(init_home_token)
            if sid in _sessions:
...
def _(rid, params: dict) -> dict:
    ...
    # (~/.damon/sessions/saved/) rather than the project/workspace CWD, and
...
def _spawn_trees_root():
    ...
    root = get_damon_home() / "spawn-trees"
    ...
    def run():
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_damon_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from damon_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from damon_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: damon_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from damon_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _damon_home / "images"
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.damon/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.damon/desktop-attachments/``.
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from damon_cli.model_switch import parse_model_flags
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from damon_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_damon_home), "display": display_damon_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from damon_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from damon_cli.runtime_provider import resolve_runtime_provider
        from damon_cli.auth import has_usable_secret
        from damon_cli.main import _has_any_provider_configured
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.damon/.env`` into the gateway process via
    ``damon_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m damon_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "damon_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from damon_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from damon_cli.config import is_managed, save_env_value
        from damon_cli.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from damon_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from damon_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from damon_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from damon_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from damon_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from damon_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from damon_cli.browser_connect import try_launch_chrome_debug
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_damon_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from damon_cli.skills_hub import do_install
        ...
        if action == "browse":
            from damon_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from damon_cli.skills_hub import inspect_skill
