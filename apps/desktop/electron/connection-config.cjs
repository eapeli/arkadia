 ...
 *     `X-Damon-Session-Token` header; WS uses `?token=`.
...
// damon_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - damon_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - damon_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / damon #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-damon_session_at', '__Secure-damon_session_at', 'damon_session_at']
const RT_COOKIE_VARIANTS = ['__Host-damon_session_rt', '__Secure-damon_session_rt', 'damon_session_rt']
...
/**
 * True if any cookie in `cookies` is a damon session ACCESS-token cookie
