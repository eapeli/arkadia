...
const { buildDesktopBackendEnv, normalizeDamonHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.DAMON_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.DAMON_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with DAMON_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[damon] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[damon] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[damon] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[damon] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// DAMON_HOME — the user-facing root for everything Damon-related. Mirrors
// scripts/install.ps1's $DamonHome and scripts/install.sh's $DAMON_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\damon (matches install.ps1)
//   macOS / Linux: ~/.damon (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.damon directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\damon yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// DAMON_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// DAMON_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.damon / %LOCALAPPDATA%\damon.
function resolveDamonHome() {
  if (process.env.DAMON_HOME) return normalizeDamonHomeRoot(process.env.DAMON_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'damon-home')
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'damon')
    const legacy = path.join(app.getPath('home'), '.damon')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.damon setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.damon')
}

const DAMON_HOME = resolveDamonHome()
// ACTIVE_DAMON_ROOT — the canonical mutable Damon install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_DAMON_ROOT = path.join(DAMON_HOME, 'damon-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_DAMON_ROOT, 'venv')
...
// boots, going straight to `resolveDamonBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_DAMON_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_DAMON_ROOT, '.damon-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Damon profile the desktop launches its
// local backend as. When set, startDamon() passes `damon --profile <name>
// dashboard …`, which deterministically pins DAMON_HOME (see
// _apply_profile_override in damon_cli/main.py) and bypasses the sticky
// ~/.damon/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors damon_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// damonDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under DAMON_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by damon_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(DAMON_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (damon_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.DAMON_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.DAMON_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'Damon'
...
// 'damon:native-theme' ('dark' | 'light' | 'system'); we pin
...
// Seed the native About panel with the live Damon version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `damon update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveDamonVersion(),
...
const MEDIA_PROTOCOL = 'damon-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(damon-media://…)`.
...
let mainWindow = null
let damonProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by damonProcess +
// connectionPromise + startDamon(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.DAMON_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.DAMON_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startDamon() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Damon backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  damonLog.push(...lines)
  if (damonLog.length > 300) {
    damonLog.splice(0, damonLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-damon-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('damon:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the damon:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('damon:bootstrap:event', ev)
}
...
function isDamonSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'damon_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.DAMON_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Damon deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Damon' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\damon\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'damon', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'damon', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\damon\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'damon', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'damon', 'git', 'bin', 'git.exe'))
  }
...
function recentDamonLog() {
  return damonLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's damon root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_DAMON_ROOT.
// DAMON_DESKTOP_DAMON_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.DAMON_DESKTOP_DAMON_ROOT && path.resolve(process.env.DAMON_DESKTOP_DAMON_ROOT),
    !IS_PACKAGED && isDamonSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isDamonSourceRoot(ACTIVE_DAMON_ROOT) ? ACTIVE_DAMON_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_DAMON_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('damon:updates:progress', merged)
  }
}
...
// DAMON_HOME/damon-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_damon_home). That binary owns
// ALL repo mutation — running `damon update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'damon-setup.exe' : 'damon-setup'
  const candidate = path.join(DAMON_HOME, name)
...
// Path to the venv shim whose lock decides whether `damon update` can write
// fresh entry points. On Windows this is the file the running backend
// `damon.exe` holds open; on POSIX it's never mandatory-locked.
function venvDamonShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'damon.exe')
    : path.join(updateRoot, 'venv', 'bin', 'damon')
}
...
// only signals the direct child, so on Windows a backend `damon.exe` that
// spawned its own grandchildren (a `damon` REPL, a pty terminal session, the
...
// `damonProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `damon.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a damon
// REPL, a pty terminal, the gateway) keep `damon.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (damonProcess && Number.isInteger(damonProcess.pid)) pids.push(damonProcess.pid)
  ...
  if (damonProcess && !damonProcess.killed) {
    try {
      damonProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvDamonShimPath(updateRoot)
...
// `damon update`). Instead we spawn the staged Damon-Setup binary with
// --update and quit, so it can run `damon update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri damon-setup. Unlike Windows
      ...
      // whole update itself: `damon update` (backend) + `damon desktop
    ...
    if (!updater) {
      ...
      // `damon desktop`, never the Tauri installer that self-copies
      // damon-setup.exe into DAMON_HOME). They DO have a working `damon`
      ...
      // checkout they're on — bare `damon update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'damon update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `damon update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `damon update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, damonRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Damon updater…', percent: 100 })
    ...
    // damon.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `damon update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: DAMON_HOME,
      env: {
        ...process.env,
        DAMON_HOME,
        PATH: [path.join(DAMON_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvDamon = path.join(venvBin, IS_WINDOWS ? 'damon.exe' : 'damon')
  const updaterArgs = fileExists(venvDamon) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
// Resolve the damon CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `damon` on PATH.
function resolveDamonCliBinary(updateRoot) {
  const venvDamon = path.join(updateRoot, 'venv', 'bin', 'damon')
  if (fileExists(venvDamon)) return venvDamon
  return findOnPath('damon') || null
}
...
// macOS/Linux in-app update: backend (`damon update`) + OS-aware GUI rebuild
// (`damon desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const damon = resolveDamonCliBinary(updateRoot)
  if (!damon) {
    emitUpdateProgress({ stage: 'manual', message: 'damon update', percent: null })
    return { ok: true, manual: true, command: 'damon update', damonRoot: updateRoot }
  }

  // Put the Damon-managed Node and the venv on PATH so `damon desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(DAMON_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    DAMON_HOME,
  ...
  // `damon update` reaps stale `damon dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads DAMON_DESKTOP_CHILD_PID and excludes
  ...
  if (damonProcess && Number.isInteger(damonProcess.pid)) {
    desktopChildPids.push(damonProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.DAMON_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Damon (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(damon, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'damon update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'damon update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(damon, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Damon to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Damon.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Damon.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Damon to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.damon-update-new"; then
    rm -rf "$DST.damon-update-old" 2>/dev/null || true
    mv "$DST" "$DST.damon-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.damon-update-new" "$DST"
    rm -rf "$DST.damon-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `damon-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Damon to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `damon
  ...
  return isDamonSourceRoot(ACTIVE_DAMON_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.DAMON_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set DAMON_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: damon desktop --force-build`
  )
  return candidates[0]
}
...
function resolveDamonCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Damon.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveDamonCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'damon_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      damonHome: DAMON_HOME,
...
// createActiveBackend — build a backend pointing at ACTIVE_DAMON_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Damon at ${ACTIVE_DAMON_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'damon_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      damonHome: DAMON_HOME,
      pythonPathEntries: [ACTIVE_DAMON_ROOT],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_DAMON_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveDamonBackend(dashboardArgs) {
  // 1. Explicit override -- DAMON_DESKTOP_DAMON_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.DAMON_DESKTOP_DAMON_ROOT && path.resolve(process.env.DAMON_DESKTOP_DAMON_ROOT)
  if (overrideRoot && isDamonSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Damon source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `damon` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isDamonSourceRoot.)
  if (!IS_PACKAGED && isDamonSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Damon source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_DAMON_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\damon\damon-agent (Windows) or ~/.damon/damon-agent.
  ...
  //    to spawning damon. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `damon update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `damon` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    DAMON_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.DAMON_DESKTOP_IGNORE_EXISTING !== '1') {
    let damonCommand = null
    const damonOverride = process.env.DAMON_DESKTOP_DAMON

    if (damonOverride) {
      const resolvedOverride = findOnPath(damonOverride)
      if (resolvedOverride) {
        damonCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(damonOverride, { isWsl: IS_WSL })) {
        damonCommand = damonOverride
      } else {
        rememberLog(`Ignoring Windows Damon override under WSL: ${damonOverride}`)
      }
    } else {
      damonCommand = findOnPath('damon')
    }

    if (damonCommand) {
      if (looksLikeDesktopAppBinary(damonCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Damon CLI: ${damonCommand}`)
        damonCommand = null
      }
    }

    if (damonCommand) {
      // Smoke-test the candidate before trusting it. A `damon` shim
      ...
      const shellForProbe = isCommandScript(damonCommand)
      if (verifyDamonCli(damonCommand, { shell: shellForProbe })) {
        return {
          label: `existing Damon CLI at ${damonCommand}`,
      ...
      rememberLog(
        `Ignoring existing Damon CLI at ${damonCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed damon_cli module via system Python.
  ...
  if (python) {
    ...
    // having damon_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\damon\damon-agent\venv.
    if (canImportDamonCli(python)) {
      return {
        kind: 'python',
        label: `installed damon_cli module via ${python}`,
        command: python,
        args: ['-m', 'damon_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: damon_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveDamonBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Damon Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveDamonBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Damon install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('Damon recovery was handed off to Damon Setup. The desktop will restart when recovery completes.')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      damonHome: DAMON_HOME,
      logRoot: path.join(DAMON_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Damon install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Damon bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(DAMON_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startDamon() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // damon:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveDamonBackend(backend.args))
  }
  ...
  if (!isDamonSourceRoot(ACTIVE_DAMON_ROOT)) {
    throw new Error(
      `Damon install at ${ACTIVE_DAMON_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Damon' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\damon\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `damon` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Damon on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isDamonSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Damon venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Damon at ${ACTIVE_DAMON_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Damon runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Damon backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Damon backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Damon backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Damon-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('damon:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveDamonCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('damon:preview-file-changed', payload)
}
...
async function waitForDamon(baseUrl, token) {
  ...
  throw new Error(`Damon backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('damon:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('damon:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Damon...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('damon:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('damon:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('damon:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'damon:desktop:zoomLevel'
...
//
// Hosted Damon gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``damon_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / damon #37247).
//     Both are set as HttpOnly cookies (``damon_session_at`` ~15 min,
//     ``damon_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:damon-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Damon gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Damon backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root DAMON_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.DAMON_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.DAMON_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by DAMON_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (damon #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Damon gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Damon gateway is selected, but no session token is saved. ' +
...
//   2. env override (DAMON_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.DAMON_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.DAMON_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'DAMON_DESKTOP_REMOTE_URL is set but DAMON_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Damon backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.DAMON_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Damon
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startDamon())
  ...
  // connect to Damon gateway". Mirror the renderer's connect here so the test
...
// startDamon() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetDamonConnection() nulls damonProcess.
  const dying = damonProcess && !damonProcess.killed ? damonProcess : null
  resetDamonConnection()
...
// profile to startDamon() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startDamon() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForDamon(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: damonLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited DAMON_HOME env (see _apply_profile_override
  // step 3 in damon_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveDamonBackend(dashboardArgs))
  const damonCwd = resolveDamonCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting Damon backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: damonCwd,
        DAMON_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        DAMON_DESKTOP: '1',
        DAMON_WEB_DIST: webDist
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Damon backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Damon backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Damon backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForDamon(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Damon backend for profile "${profile}"`,
    rememberLog
  })
...
async function startDamon() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startDamon() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Damon backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Damon backend at ${remote.baseUrl}`, 24)
      await waitForDamon(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Damon backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.damon/active_profile file) and
    // resolves DAMON_HOME the same way `damon -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Damon runtime', 28)
    const backend = await ensureRuntime(resolveDamonBackend(dashboardArgs))
    const damonCwd = resolveDamonCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Damon backend via ${backend.label}`, 84)
    rememberLog(`Starting Damon backend via ${backend.label}`)
    ...
    damonProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin DAMON_HOME for the child so Python's get_damon_home()
          // resolves to the SAME location our resolveDamonHome() picked. Without
          // this pin, Python falls back to ~/.damon on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\damon, which differs from C:\Users\<u>\.damon.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets DAMON_HOME via setx; the desktop
    ...
    damonProcess.stdout.on('data', rememberLog)
    damonProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    damonProcess.once('error', error => {
      rememberLog(`Damon backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Damon backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    damonProcess.once('exit', (code, signal) => {
      rememberLog(`Damon backend exited (${signal || code})`)
      damonProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Damon backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Damon backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentDamonLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Damon backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(damonProcess), backendStartFailed])

    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Damon backend to become ready', 90)
    await Promise.race([waitForDamon(baseUrl, token), backendStartFailed])
    backendReady = true
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null damonProcess when the child dies.
      childAlive: () => damonProcess !== null && damonProcess.exitCode === null && !damonProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Damon backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startDamon().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('damon:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Damon…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('damon:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetDamonConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Damon backend failed liveness probe; dropping stale connection.')
    resetDamonConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('damon:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('damon:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('damon:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('damon:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startDamon() call restarts the
...
ipcMain.handle('damon:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startDamon() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('damon:bootstrap:cancel', async () => {
...
ipcMain.handle('damon:boot-progress:get', async () => bootProgressState)
ipcMain.handle('damon:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('damon:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('damon:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('damon:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('damon:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('damon:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('damon:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('damon:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('damon:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('damon:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new DAMON_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('damon:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('damon:requestMicrophoneAccess', async () => {
...
ipcMain.handle('damon:api', async (_event, request) => {
...
ipcMain.handle('damon:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('damon:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('damon:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('damon:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('damon:readFileText', async (_event, filePath) => {
...
ipcMain.handle('damon:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('damon:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('damon:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('damon:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('damon:saveClipboardImage', async () => {
...
ipcMain.handle('damon:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('damon:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('damon:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('damon:titlebar-theme', (_event, payload) => {
...
ipcMain.on('damon:native-theme', (_event, mode) => {
...
ipcMain.on('damon:translucency', (_event, payload) => {
...
ipcMain.handle('damon:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveDamonCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('damon:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveDamonCwd()
}))

ipcMain.handle('damon:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('damon:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('damon:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('damon:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('damon:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('damon:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: damonLog.slice(-200) }))
...
function terminalShellCommand() {
  // DAMON_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.DAMON_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Damon' TUI into
  ...
  env.TERM_PROGRAM = 'Damon'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a damon/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from DAMON_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.DAMON_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `damon:terminal:${id}:${suffix}`
}
...
ipcMain.handle('damon:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('damon:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

ipcMain.handle('damon:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))

ipcMain.handle('damon:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Damon.')
  }
...
ipcMain.handle('damon:terminal:write', (_event, id, data) => {
...
ipcMain.handle('damon:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('damon:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('damon:updates:check', async () =>
...
ipcMain.handle('damon:updates:apply', async (_event, payload) =>
...
ipcMain.handle('damon:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('damon:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Damon version (the one `release.py` bumps in
// damon_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Damon version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveDamonVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'damon_cli', '__init__.py')
...
// Re-resolve the live Damon version and push it into the native About panel
// just before showing it, so an in-place `damon update` is reflected without
...
ipcMain.handle('damon:version', async () => ({
  appVersion: resolveDamonVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  damonRoot: resolveUpdateRoot()
}))
...
// the actual removal via `damon uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (damon_cli/uninstall.py + damon_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_DAMON_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isDamonSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'damon_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, DAMON_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Damon agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import damon_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_DAMON_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `damon-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `damon-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('damon:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('damon:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('damon:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('damon:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// damon:// deep links (e.g. damon://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const DAMON_PROTOCOL = 'damon'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${DAMON_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // damon://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('damon:deep-link', payload)
...
ipcMain.handle('damon:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${DAMON_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(DAMON_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(DAMON_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `damon://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching damon:// URL is in our own argv.
