...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'damon-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/damon-runner-test',
    sourceRepoRoot: null,
    damonHome: '/tmp/damon-runner-test',
    logRoot: '/tmp/damon-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'damon-agent', 'scripts')
