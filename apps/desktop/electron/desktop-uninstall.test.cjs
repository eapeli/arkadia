...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'damon_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'damon_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'damon_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Damon.app/Contents/MacOS/Damon', 'darwin'),
    '/Applications/Damon.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Damon.app/Contents/MacOS/Damon', 'darwin'),
    '/Users/x/Applications/Damon.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Damon\\Damon.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Damon'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\damon-desktop\\Damon.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\damon-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Damon.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_DamonXXXX/damon', 'linux', { APPIMAGE: '/home/x/Apps/Damon.AppImage' }),
    '/home/x/Apps/Damon.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(
    resolveRemovableAppPath('/opt/damon/linux-unpacked/damon', 'linux', {}),
    '/opt/damon/linux-unpacked'
  )
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/damon', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Damon.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Damon.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.damon/damon-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.damon/damon-agent',
    uninstallArgs: ['-m', 'damon_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/damon/linux-unpacked',
    damonHome: '/home/x/.damon'
  })
  ...
  assert.match(script, /'-m' 'damon_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/damon\/linux-unpacked'/)
  assert.match(script, /export DAMON_HOME='\/home\/x\/\.damon'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.damon/damon-agent',
    agentRoot: '/home/x/.damon/damon-agent',
    uninstallArgs: ['-m', 'damon_cli.uninstall', '--mode', 'full'],
    appPath: null,
    damonHome: '/home/x/.damon'
  })
  // System python + source on PYTHONPATH so import damon_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.damon\/damon-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'damon_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'damon_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    damonHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'damon_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\damon',
    uninstallArgs: ['-m', 'damon_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Damon',
    damonHome: 'C:\\Users\\x\\AppData\\Local\\damon'
  })
  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import damon_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\damon;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "damon_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Damon" >nul 2>&1/)
