        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.damonDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Damon to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.damonDesktop?.watchPreviewFile ||
      !window.damonDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.damonDesktop.onPreviewFileChanged(payload => {
    ...
    void window.damonDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.damonDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.damonDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:damon-preview')
