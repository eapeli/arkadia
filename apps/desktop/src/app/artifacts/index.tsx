...
import { getSessionMessages, listAllProfileSessions } from '@/damon'
...
import type { SessionInfo, SessionMessage } from '@/types/damon'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.damonDesktop?.openExternal) {
        await window.damonDesktop.openExternal(href)
      } else {
