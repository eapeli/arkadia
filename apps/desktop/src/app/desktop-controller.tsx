...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../damon'
...
import { useDamonConfig } from './session/hooks/use-damon-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.damonDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.damonDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.damonDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.damonDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // damon:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.damonDesktop?.onDeepLink?.(payload => {
    ...
    void window.damonDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.damonDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshDamonConfig, sttEnabled, voiceMaxRecordingSeconds } = useDamonConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshDamonConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshDamonConfig])
