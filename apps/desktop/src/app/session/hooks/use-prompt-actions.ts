...
import { getProfiles, transcribeAudio } from '@/damon'
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.damonDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `damon gateway` process performs the actual transfer, so we
