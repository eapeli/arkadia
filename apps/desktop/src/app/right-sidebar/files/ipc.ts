...
import type { DamonReadDirEntry, DamonReadDirResult } from '@/global'

export type ProjectTreeEntry = DamonReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: DamonReadDirEntry) {
...
async function filterIgnored(entries: DamonReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<DamonReadDirResult> {
  if (!window.damonDesktop) {
