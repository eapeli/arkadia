...
import type { DamonReadDirResult } from '@/global'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<DamonReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { damonDesktop: { readDir: typeof readDir } }).damonDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { damonDesktop?: unknown }).damonDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): DamonReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { damonDesktop: unknown }).damonDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: DamonReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<DamonReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { damonDesktop: unknown }).damonDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.damonDesktop is missing', async () => {
    delete (window as unknown as { damonDesktop?: unknown }).damonDesktop
