...
import {
  getElevenLabsVoices,
  getDamonConfigDefaults,
  getDamonConfigRecord,
  getDamonConfigSchema,
  saveDamonConfig
} from '@/damon'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, DamonConfigRecord } from '@/types/damon'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<DamonConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<DamonConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getDamonConfigRecord(), getDamonConfigDefaults(), getDamonConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: DamonConfigRecord) => {
