...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: DamonConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: DamonConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // DAMON_QWEN_ and DAMON_GEMINI_ both share the DAMON_ stem.
      expect(providerGroup('DAMON_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('DAMON_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
