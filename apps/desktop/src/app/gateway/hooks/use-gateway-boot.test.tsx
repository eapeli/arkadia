...
// bug that drives the REAL useGatewayBoot hook + REAL DamonGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshDamonConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { damonDesktop?: unknown }).damonDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { damonDesktop?: unknown }).damonDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForDamon) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startDamon()'s remote branch awaits waitForDamon() for 45s before it
    ...
    ;(window as { damonDesktop?: unknown }).damonDesktop = desktop
    ...
    // After ~45s waitForDamon gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Damon backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
