...
import type { DamonConnection } from '@/global'
import { DamonGateway } from '@/damon'
...
import type { RpcEvent } from '@/types/damon'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.damonDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: DamonGateway | null) => void
  refreshDamonConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: DamonConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Damon...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Damon…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Damon gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshDamonConfig().catch(() => undefined)
    ...
    const gateway = new DamonGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshDamonConfig()
