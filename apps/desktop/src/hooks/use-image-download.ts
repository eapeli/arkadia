...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'damon:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.damonDesktop?.saveImageFromUrl) {
        if (await window.damonDesktop.saveImageFromUrl(src)) {
