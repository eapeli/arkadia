import type {
  DamonConnection,
  DamonReadDirResult,
  DamonReadFileTextResult,
  DamonSelectPathsOptions,
  DamonWorktreeInfo
} from '@/global'
...
function connectionCacheKey(connection: DamonConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
function bridge() {
  const desktop = window.damonDesktop
  if (!desktop) {
    throw new Error('Damon Desktop bridge is unavailable')
  }
  return desktop
}

export async function readDesktopDir(path: string): Promise<DamonReadDirResult> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
    return desktop.readDir(path)
  }
  return desktop.api<DamonReadDirResult>({ path: fsPath('list', path) })
}

export async function readDesktopFileText(path: string): Promise<DamonReadFileTextResult> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
    return desktop.readFileText(path)
  }
  return desktop.api<DamonReadFileTextResult>({ path: fsPath('read-text', path) })
}
...
export async function desktopWorktrees(cwds: string[]): Promise<Record<string, DamonWorktreeInfo | null>> {
  if (isDesktopFsRemoteMode()) {
    return {}
  }
...
export async function selectDesktopPaths(options?: DamonSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
  if (!isDesktopFsRemoteMode()) {
