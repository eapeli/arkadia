...
/** Why a known Damon command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Damon profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Damon Agent version', surface: exec() },
...
function isKnownDamonSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Damon' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownDamonSlashCommand(normalized)
}
