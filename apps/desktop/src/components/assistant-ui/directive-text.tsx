...
const DAMON_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type DamonRefType = (typeof DAMON_REF_TYPES)[number]
...
const ICON_PATHS: Record<DamonRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as DamonRefType] ?? ICON_FALLBACK
...
const DAMON_DIRECTIVE_RE = new RegExp(
...
export const damonDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(DAMON_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as DamonRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: DamonRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Damon directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => damonDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.damonDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.damonDesktop?.readFileDataUrl(id)
