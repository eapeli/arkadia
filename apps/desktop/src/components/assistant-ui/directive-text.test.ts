...
import { formatRefValue, damonDirectiveFormatter } from './directive-text'
...
describe('damonDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = damonDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = damonDirectiveFormatter.parse('@file:src/main.tsx the entry point')
