...
import { getDamonConfigRecord, type DamonConfigRecord, saveDamonConfig } from '@/damon'
...
export interface I18nConfigClient {
  getConfig: () => Promise<DamonConfigRecord>
  saveConfig: (config: DamonConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.damonDesktop?.api) {
      return Promise.resolve({})
    }

    return getDamonConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.damonDesktop?.api) {
...
export function getConfigDisplayLanguage(config: DamonConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: DamonConfigRecord, locale: Locale): DamonConfigRecord {
