...
const SKIN_KEY = 'damon-desktop-theme-v2'
const MODE_KEY = 'damon-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'damon-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'damon-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'damon-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.damonTheme = skinName
  root.dataset.damonMode = rendered
  ...
  window.damonDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('damon-boot-background', c.background)
    window.localStorage.setItem('damon-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.damonThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.damonDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Damon backend theme on connect. */
