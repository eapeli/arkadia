import type { ConnectionState, GatewayEvent } from '@damon/shared'
...
const isOpen = (gateway: DamonGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<DamonGateway | null>(null)
...
let primaryGateway: DamonGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: DamonGateway | null, profile = 'default'): void {
...
export function activeGateway(): DamonGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<DamonGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
