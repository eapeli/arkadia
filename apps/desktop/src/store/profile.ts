...
import { getProfiles, setApiRequestProfile } from '@/damon'
...
import type { ProfileInfo } from '@/types/damon'
...
// /api/profiles/active `current`). "default" is the root ~/.damon. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'damon.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'damon.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.damonDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new DAMON_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.damonDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'damon.desktop.showAllProfiles'
...
// Switch to the default (root ~/.damon) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.damonDesktop?.touchBackend?.(target).catch(() => undefined)
}
