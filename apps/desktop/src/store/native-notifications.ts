...
const STORAGE_KEY = 'damon:native-notifications'
...
// "Backgrounded" = the user isn't on Damon. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.damonDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.damonDesktop
