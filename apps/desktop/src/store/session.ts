...
import type { SessionInfo, UsageStats } from '@/types/damon'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'damon.desktop.workspace-cwd'

let configuredDefaultProjectDir = ''

function workspaceCwdKey(connection: DamonConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.damonDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.damonDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<DamonConnection | null>(null)
...
export const setConnection = (next: Updater<DamonConnection | null>) => updateAtom($connection, next)
