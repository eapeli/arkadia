...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/damon'
...
const CONFIGURED_CACHE_KEY = 'damon-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'damon-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Damon is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Nous Tool Gateway,
// mirroring damon_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
async function fetchProviderDefaultModel(
  ...
  // `damon model` does (for Nous it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Damon still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Damon still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.damonDesktop?.openExternal) {
    try {
      await window.damonDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Damon still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Damon still cannot reach ${url}.` }
    }
