...
declare global {
  interface Window {
    damonDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<DamonConnection>
      ...
      profile: {
        ...
        // backend under the new DAMON_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: DamonApiRequest) => Promise<T>
      notify: (payload: DamonNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<DamonReadFileTextResult>
      selectPaths: (options?: DamonSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<DamonPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<DamonPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: DamonTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<DamonReadDirResult>
      ...
      worktrees?: (cwds: string[]) => Promise<Record<string, DamonWorktreeInfo | null>>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: DamonTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<DamonTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: DamonWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: DamonPreviewFileChanged) => void) => () => void
...
export interface DamonTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface DamonTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `damon update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  damonRoot?: string
}
...
export interface DamonConnection {
  baseUrl: string
...
export interface DamonTitleBarTheme {
  background: string
  foreground: string
}

export interface DamonWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DamonApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface DamonNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface DamonPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface DamonReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface DamonPreviewWatch {
  id: string
  path: string
}

export interface DamonWorktreeInfo {
...
export interface DamonReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface DamonReadDirResult {
  entries: DamonReadDirEntry[]
  error?: string
}

export interface DamonPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface DamonSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
