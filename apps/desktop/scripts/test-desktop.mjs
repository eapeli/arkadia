...
// no longer bundles the Damon Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Damon.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Damon'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Damon.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'damon'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default DAMON_HOME for non-sandboxed runs -- matches main.cjs's
// resolveDamonHome(). On Windows it's %LOCALAPPDATA%\damon; elsewhere
// it's ~/.damon. The fresh-install sandbox launchFresh() sets its own
// DAMON_HOME and never touches this.
const DEFAULT_DAMON_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'damon')
  }
  return path.join(os.homedir(), '.damon')
})()
const VENV_ROOT = path.join(DEFAULT_DAMON_HOME, 'damon-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'damon-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.DAMON_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Damon-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Damon-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Damon-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Damon-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.DAMON_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.DAMON_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const damonHome = path.join(sandbox, 'damon-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(damonHome, { recursive: true })
  ...
  env.DAMON_DESKTOP_CWD = cwd
  env.DAMON_DESKTOP_IGNORE_EXISTING = '1'
  env.DAMON_DESKTOP_TEST_MODE = 'fresh-install'
  env.DAMON_DESKTOP_USER_DATA_DIR = userDataDir
  env.DAMON_HOME = damonHome
  delete env.DAMON_DESKTOP_DAMON
  delete env.DAMON_DESKTOP_DAMON_ROOT
  ...
  console.log(`  DAMON_HOME: ${damonHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(damonHome, 'damon-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Damon Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of damon_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'damon-agent', 'damon_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Damon
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + DAMON_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  DAMON_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
