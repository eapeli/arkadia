 ...
 * product name (`Damon`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Damon'
 *
 * This is a hard failure with no obvious cause for the user — `damon desktop`
 ...
 * (the mac-unpacked Damon.app bundle) and Windows (win-unpacked), so we
