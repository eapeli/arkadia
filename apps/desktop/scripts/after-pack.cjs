 ...
 *
 * Stamps the Damon icon + identity onto the packed Windows Damon.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `damon desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Damon')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Damon'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Damon.exe keeps the stock Electron icon`)
  }
}
