...
export const $damonHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, damonHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_damon_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $damonHome.set(damonHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Damon" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Damon-Setup.exe --update);
...
export async function launchDamonDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_damon_desktop', { installRoot })
}
