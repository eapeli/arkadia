...
//!
//! Driven when the installer is launched as `Damon-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Damon desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `damon update`
//!      or repair bootstrap can race locked files),
//!   2. run `damon update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in damon_cli/main.py),
//!   3. run `damon desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `damon update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_damon) and
...
/// `damon update` exit code meaning "another damon process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// damon_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `damon update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Damon"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let damon_home = crate::paths::damon_home();
    let install_root = damon_home.join("damon-agent");
    ...
    let damon = resolve_damon(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the damon CLI under {}. Is Damon installed? \
    ...
    // async on Windows. If it still holds the venv shim, `damon update`
    ...
    // ---- stage 1: damon update -----------------------------------------
    // Pass --branch so `damon update` targets the branch this installer was
    ...
    // without --branch, `damon update` switches the checkout to `main` (a
    ...
    // --force skips `damon update`'s Windows running-exe guard (which would
    ...
    // time `damon update` runs there is no legitimate damon.exe to protect,
    // and the guard would only produce a false "Damon is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `damon update` lazily imports
    ...
    // second `damon update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Damon" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Damon is still running. Close all Damon windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "damon update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::damon_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: damon desktop --build-only ----------------------------
    // `damon update` deliberately does NOT build apps/desktop (it installs
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `damon desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Damon manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_damon_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Damon to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend damon.exe (or the desktop Damon.exe
            ...
            // force-kill every Damon.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Damon still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_damon();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_damon(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Damon.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Damon.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `damon.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `damon.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\damon.exe update`. At this
/// point no update-driven damon.exe exists yet, so the only damon.exe images
...
/// Spawn `damon <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv damon shim under an install root, regardless of existence.
fn venv_damon(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("damon.exe")
    } else {
        install_root.join("venv").join("bin").join("damon")
    }
}

/// Resolve the damon CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `damon` on PATH.
fn resolve_damon(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_damon(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "damon.exe" } else { "damon" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let damon_home = crate::paths::damon_home();
    let mut envs = vec![(
        "DAMON_HOME".to_string(),
        damon_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        damon_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_damon_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Damon.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.damon-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.damon-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::damon_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_damon_is_under_install_root() {
        let root = Path::new("/x/damon-agent");
        let shim = venv_damon(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_damon(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/damon-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Damon.app"]),
            Some(PathBuf::from("/Applications/Damon.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Damon.app");
        let tmp = base.join("Damon.app.damon-update-new");
        let old = base.join("Damon.app.damon-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Damon.app");
        let tmp = base.join("Damon.app.damon-update-new"); // intentionally absent
        let old = base.join("Damon.app.damon-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Damon.app");
        let tmp = base.join("Damon.app.damon-update-new"); // absent
        let old = base.join("Damon.app.damon-update-old");
        write_marker(&target, "OLD");
