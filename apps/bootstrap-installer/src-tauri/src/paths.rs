...
//!
//! Mirrors `damon_constants.get_damon_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\damon
//!   macOS:   ~/.damon
//!   Linux:   ~/.damon  (override via $DAMON_HOME)
//!
//! NOTE (macOS): Python's get_damon_home(), scripts/install.sh, and the
//! Electron desktop's resolveDamonHome() ALL use ~/.damon on macOS — there
...
/// Returns the canonical Damon home directory, respecting $DAMON_HOME if set.
pub fn damon_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("DAMON_HOME") {
    ...
    {
        // %LOCALAPPDATA%\damon — matches scripts/install.ps1's $DamonHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("damon");
        }
    }

    // macOS + Linux + fallback: ~/.damon (matches Python get_damon_home(),
    // install.sh, and the Electron desktop's resolveDamonHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".damon");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    damon_home().join("bootstrap-cache")
}
...
/// DAMON_HOME so it survives repo checkout deletion (unlike anything under
/// damon-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\damon\damon-setup.exe`; on other
...
pub fn copy_self_to_damon_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to DAMON_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_DAMON_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".damon-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under DAMON_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[damon-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("DAMON_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_damon_home() -> String {
    damon_home().to_string_lossy().into_owned()
}
