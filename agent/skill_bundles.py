...
Storage
-------
Bundles live in ``~/.damon/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``DAMON_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``damon bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under DAMON_HOME.

    Honors ``DAMON_BUNDLES_DIR`` for tests; falls back to
    ``<DAMON_HOME>/skill-bundles``.
    """
    override = os.environ.get("DAMON_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_damon_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `damon bundles` CLI subcommand.
# ---------------------------------------------------------------------------
