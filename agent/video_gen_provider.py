...
as ``kind: backend``) or ``~/.damon/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``damon tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$DAMON_HOME/cache/videos/``, creating parents as needed."""
    from damon_pkg.constants import get_damon_home

    path = get_damon_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$DAMON_HOME/cache/videos/``.
