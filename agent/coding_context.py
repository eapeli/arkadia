"""Coding-context awareness — base Damon, every interactive surface.

When the user runs Damon inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Damon shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("damon.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "damon", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _DAMON_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``DAMON_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Damon should operate in its coding posture right now."""
