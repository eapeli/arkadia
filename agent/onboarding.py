...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Damon starts and finds ``~/.openclaw/``.

    Points users at ``damon claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``damon claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Damon, run "
        "`damon claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `damon claw cleanup` (renames it to ~/.openclaw.pre-migration — "
