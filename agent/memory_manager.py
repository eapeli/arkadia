...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        from toolsets import _DAMON_CORE_TOOLS

        _core_tool_names = set(_DAMON_CORE_TOOLS)
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``damon_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_damon_home()`` themselves.
        """
        if "damon_home" not in kwargs:
            from damon_pkg.constants import get_damon_home
            kwargs["damon_home"] = str(get_damon_home())
        for provider in self._providers:
            try:
