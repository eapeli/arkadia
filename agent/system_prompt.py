...
upstream prefix cache warm.  See ``damon-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Damon never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want DAMON_HOME persona while keeping
    ...
    # Pointer to the damon-agent skill + docs for user questions about Damon itself.
    stable_parts.append(DAMON_AGENT_HELP_GUIDANCE)
    ...
    # DAMON_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Damon, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Damon profile the agent is running
    # under so it doesn't conflate ~/.damon/skills/ (default profile) with
    # ~/.damon/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Damon profile: default. Other profiles (if any) live "
            "under ~/.damon/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Damon profile: {active_profile}. This session reads "
            f"and writes ~/.damon/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.damon/skills/, ~/.damon/plugins/, "
            f"~/.damon/cron/, ~/.damon/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Damon never rebuilds or reinjects parts of it
