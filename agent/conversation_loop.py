...
from damon_pkg.constants import PARTIAL_STREAM_STUB_ID
from damon_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Damon tool use: "
    ...
    return (
        ...
        f"context, but Damon needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Damon config, set `model.ollama_num_ctx: 65536` "
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `damon fallback add`."
)
...
def run_conversation(
    ...
    # all run inside Codex). Default Damon path is bypassed entirely.
    ...
    # ── KV Cache Warmup (MARK XL pattern) ──────────────────────────────
    # Pre-load the static system prompt into the LLM's KV cache so the first
    # real request skips the ~10-20s cold evaluation.  Only runs once per
    # agent instance.  Uses the cached system prompt which is byte-stable
    # for the session.
    if not getattr(agent, "_kv_warmed_up", False):
        agent._kv_warmed_up = True
        try:
            from damon_cli.middleware import apply_llm_request_middleware
            # Build minimal warmup messages: static system prompt only
            warmup_messages = [{"role": "system", "content": active_system_prompt}]
            warmup_kwargs = agent._build_api_kwargs(warmup_messages)
            warmup_kwargs = apply_llm_request_middleware(
                warmup_kwargs,
                task_id=effective_task_id,
                turn_id=turn_id,
                api_request_id=f"{turn_id}:warmup" if turn_id else "warmup",
            ...
            # Fire-and-forget warmup: don't await/block, just trigger KV population
            # The actual call is made async in background to avoid blocking startup
            def _do_warmup():
                try:
                    agent._get_transport().call(**warmup_kwargs)
                except Exception:
                    pass  # Warmup is best-effort
            threading.Thread(target=_do_warmup, name="kv-warmup", daemon=True).start()
        except Exception:
            pass  # Warmup is entirely best-effort
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Damon internals.
        #
        # Damon invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Damon tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if env_var_enabled("DAMON_DUMP_REQUESTS"):
                ...
                from damon_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Damon/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    # Structured metrics: LLM API call latency
                    try:
                        from damon_logging import log_llm_metric
                        log_llm_metric(
                            model=agent.model,
                            ...
                            api_call_id=f"{turn_id}:api:{api_call_count}" if turn_id else None,
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: damon auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `damon doctor` for credential-chain diagnostics, or")
                    ...
                    from damon_pkg.constants import display_damon_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Damon-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: damon config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: damon config set ANTHROPIC_API_KEY \"\"")
                ...
                # providers (DeepSeek, Kimi, MiMo, Damon).  A 429 can
                ...
                # which Damon' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Damon' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`damon",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `damon auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `damon model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: damon portal", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: damon setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        damon fallback add   (interactive picker — same as `damon model`)",
                            force=True,
                        )
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                # ── Fast-path: direct-result tools don't need another LLM round ──
                # If ALL tools in this turn are "direct-result" (return user-ready strings),
                # use the last tool's result as the final response and exit the loop.
                # This mirrors MARK XL's _NEEDS_LLM_ROUND / fast-ack pattern.
                _DIRECT_RESULT_TOOLS = frozenset({
                    "memory", "todo", "skill_manage", "session_search",
                    "clarify", "read_terminal", "agent_task",
                })
                _tc_names = {tc.function.name for tc in assistant_message.tool_calls}
                if _tc_names and _tc_names.issubset(_DIRECT_RESULT_TOOLS):
                    # Get the last tool result (already appended to messages)
                    last_tool_msg = None
                    ...
                    if last_tool_msg and last_tool_msg.get("content"):
                        ...
                        final_response = last_tool_msg["content"]
                        _turn_exit_reason = "fast_path_direct_result"
                        # Append as assistant message for session consistency
