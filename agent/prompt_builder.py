...
from damon_pkg.constants import get_damon_home, get_skills_dir, is_wsl
...
_DAMON_MD_NAMES = (".damon.md", "DAMON.md")


def _find_damon_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.damon.md`` or ``DAMON.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Damon Agent, an intelligent AI assistant created by Nous Research. "
...
DAMON_AGENT_HELP_GUIDANCE = (
    "You run on Damon Agent (by Nous Research). When the user needs help with "
    "Damon itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://damon-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `damon-agent` skill with skill_view(name='damon-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.damon/kanban.db`. Your task id is in "
    "`$DAMON_KANBAN_TASK`; your workspace is `$DAMON_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $DAMON_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `damon kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Damon "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Damon WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Damon itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Damon runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Damon itself is running. The host OS, home, and cwd "
                f"of the Damon process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Damon "
    ...
    # Damon desktop GUI — any agent running under the desktop app should know
    # it. DAMON_DESKTOP marks the backend powering the chat; DAMON_DESKTOP_TERMINAL
    # marks a damon launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("DAMON_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("DAMON_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Damon desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Damon
    ...
    extra = (os.getenv("DAMON_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_damon_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.damon/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("DAMON_PLATFORM")
        or get_session_env("DAMON_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Damon Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `damon-agent` skill "
            "first. It has the actual commands (e.g. `damon config set …`, `damon tools`, "
            "`damon setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: damon setup, damon setup tools, damon setup terminal, damon status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from DAMON_HOME and return its content, or None.
    ...
    """
    try:
        from damon_cli.config import ensure_damon_home
        ensure_damon_home()
    except Exception as e:
        logger.debug("Could not ensure DAMON_HOME before loading SOUL.md: %s", e)

    soul_path = get_damon_home() / "SOUL.md"
...
def _load_damon_md(cwd_path: Path) -> str:
    """.damon.md / DAMON.md — walk to git root."""
    damon_md_path = _find_damon_md(cwd_path)
    if not damon_md_path:
        return ""
    try:
        content = damon_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = damon_md_path.name
        try:
            rel = str(damon_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".damon.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", damon_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .damon.md / DAMON.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from DAMON_HOME is independent and always included when present.
    ...
    # SOUL.md from DAMON_HOME only — skip when already loaded as identity
    if not skip_soul:
