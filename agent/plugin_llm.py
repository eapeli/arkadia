...
Plugins built on Damon Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Damon
...
:class:`~damon_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Damon' main config
...
handles every provider, fallback chain, and per-task override Damon
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`damon_cli.plugins.PluginContext`
