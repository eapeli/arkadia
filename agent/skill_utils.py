...
from damon_pkg.constants import get_config_path, get_skills_dir, is_termux
...
def is_skill_support_path(path) -> bool:
    """Best-effort gate for support surfaces that should expose skill metadata."""
    return not is_excluded_skill_path(path)
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``DAMON_KANBAN_TASK`` / ``DAMON_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("DAMON_KANBAN_TASK") or os.getenv("DAMON_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Damon Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``DAMON_PLATFORM`` or
            ``DAMON_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``damon`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.damon/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``damon`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against DAMON_HOME, not cwd
        if not p.is_absolute():
            p = (damon_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.damon/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    damon = metadata.get("damon") or {}
    if not isinstance(damon, dict):
        damon = {}
    return {
        "fallback_for_toolsets": damon.get("fallback_for_toolsets", []),
        "requires_toolsets": damon.get("requires_toolsets", []),
        "fallback_for_tools": damon.get("fallback_for_tools", []),
        "requires_tools": damon.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    damon = metadata.get("damon")
    if not isinstance(damon, dict):
        return []
    raw = damon.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Damon metadata, VCS, virtualenv/dependency, and cache
