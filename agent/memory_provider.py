...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - damon_home (str): The active DAMON_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.damon``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "damon").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'damon memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], damon_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'damon memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``damon_home`` is the active DAMON_HOME directory path.
