...
Every Damon request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Damon Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=damon-agent",
        "client=damon-client-v<__version__>",
    ]

The version is sourced live from ``damon_cli.__version__`` so it auto-aligns
...
``damon_cli.__version__`` is the canonical source of truth.
"""
...
def _damon_version() -> str:
    """Return the current Damon release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``damon_cli`` cannot be imported (should
...
def damon_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=damon-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=damon-client-v{_damon_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=damon-agent", damon_client_tag()]
