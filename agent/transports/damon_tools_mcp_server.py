...
MCP server (per `~/.codex/config.toml [mcp_servers.damon-tools]`) and
...
Scope (what we expose):
  ...
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    ...
    unblock/link)                          write ~/.damon/kanban.db)
...
Run with: python -m agent.transports.damon_tools_mcp_server
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    # the env var and write to ~/.damon/kanban.db.
    "kanban_complete",
    "kanban_block",
    "kanban_comment",
    "kanban_heartbeat",
...
def _build_server() -> Any:
    ...
    try:
        ...
        raise ImportError(
            f"damon-tools MCP server requires the 'mcp' package: {exc}"
        ) from exc
    ...
    logger.info(
        "damon-tools MCP server registered %d/%d tools",
        exposed_count,
        len(EXPOSED_TOOLS),
    )
    return mcp
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.damon_tools_mcp_server`."""
    ...
    try:
        ...
        sys.stderr.write(f"damon-tools MCP server cannot start: {exc}\n")
        return 2
    ...
    try:
        ...
        logger.exception("damon-tools MCP server crashed")
        sys.stderr.write(f"damon-tools MCP server error: {exc}\n")
        return 1
    return 0
