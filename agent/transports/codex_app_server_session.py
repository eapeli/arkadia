...
Owns one Codex thread per Damon session. Drives `turn/start`, consumes
...
# Damon' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_DAMON_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Damon/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Damon session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _DAMON_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("DAMON_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="damon",
            client_title="Damon Agent",
            client_version=_get_damon_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Damon' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Damon' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own damon-tools callback we
            # auto-accept — the user already approved Damon' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Damon approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Damon returns 'once', 'session', 'always', or 'deny'.
...
def _get_damon_version() -> str:
    """Best-effort Damon version string for codex's userAgent line."""
    try:
