...
``auth.json``.  They intentionally have no dependency on ``damon_cli.auth`` so
...
# Sources Damon owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Damon-owned OAuth/device-code state)
