...
TOOLSETS = {
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in damon_cli/tools_config.py).
        "posture": True,
    },
    ...
    "damon-cron": {
        # Mirrors damon-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `hermes tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as damon-cli; gated by `hermes tools`",
        "tools": _HERMES_CORE_TOOLS,
        "includes": []
    },
    ...
    "damon-gateway": {
        ...
        "tools": [],
        "includes": ["damon-telegram", "damon-discord", "damon-whatsapp", "damon-slack", "damon-signal", "damon-bluebubbles", "damon-homeassistant", "damon-email", "damon-sms", "damon-mattermost", "damon-matrix", "damon-dingtalk", "damon-feishu", "damon-wecom", "damon-wecom-callback", "damon-weixin", "damon-qqbot", "damon-webhook", "damon-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (damon-<name>).
        # Gives them _HERMES_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("damon-"):
            platform_name = name[len("damon-"):]
            try:
