...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      damonNpmLib = self'.packages.default.passthru.damonNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. damon-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          echo "Damon Agent dev shell"
          ${combinedNonNpm}
          ${damonNpmLib.mkNpmDevShellHook npmPackageJsonPaths fixLockfilesExe}
          echo "Ready. Run 'damon' to start."
        '';
      };
    };
}
