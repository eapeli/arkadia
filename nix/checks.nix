...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      damon-agent = self'.packages.default;
      damonVenv = damon-agent.damonVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "damon-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${damonVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "damon-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "damon-build-package" { } ''
          echo "PASS: package built at ${damon-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "damon-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "damon-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${damon-agent}/bin/damon || (echo "FAIL: damon binary missing"; exit 1)
          test -x ${damon-agent}/bin/damon-agent || (echo "FAIL: damon-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${damon-agent}/bin/damon version 2>&1 | grep -qi "damon" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "damon-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in damon damon-agent damon-acp; do
            test -x ${damon-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "damon-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking damon --help ==="
          ${damon-agent}/bin/damon --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${damon-agent}/bin/damon --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "damon-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${damon-agent}/share/damon-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${damon-agent}/share/damon-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "DAMON_BUNDLED_SKILLS" ${damon-agent}/bin/damon || \
            (echo "FAIL: DAMON_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: DAMON_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "damon-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${damon-agent}/share/damon-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${damon-agent}/share/damon-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "DAMON_BUNDLED_PLUGINS" ${damon-agent}/bin/damon || \
            (echo "FAIL: DAMON_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: DAMON_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "damon-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${damon-agent}/share/damon-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${damon-agent}/share/damon-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${damon-agent}/share/damon-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "DAMON_BUNDLED_LOCALES" ${damon-agent}/bin/damon || \
            (echo "FAIL: DAMON_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: DAMON_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (DAMON_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && DAMON_BUNDLED_LOCALES=${damon-agent}/share/damon-agent/locales \
            ${damonVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with DAMON_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${damonVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${damonVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without DAMON_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "damon-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${damon-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${damon-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "DAMON_TUI_DIR" ${damon-agent}/bin/damon || \
            (echo "FAIL: DAMON_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: DAMON_TUI_DIR set in wrapper"
        ...
        # Verify DAMON_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        damon-node = pkgs.runCommand "damon-node-version" { } ''
          set -e
          echo "=== Checking DAMON_NODE in wrapper ==="
          grep -q "DAMON_NODE" ${damon-agent}/bin/damon || \
            (echo "FAIL: DAMON_NODE not set in wrapper"; exit 1)
          echo "PASS: DAMON_NODE present in wrapper"

          DAMON_NODE=$(sed -n "s/^export DAMON_NODE='\(.*\)'/\1/p" ${damon-agent}/bin/damon)
          test -x "$DAMON_NODE" || (echo "FAIL: DAMON_NODE=$DAMON_NODE not executable"; exit 1)
          echo "PASS: DAMON_NODE executable at $DAMON_NODE"

          NODE_MAJOR=$("$DAMON_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All DAMON_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify DAMON_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "damon-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(DAMON_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking DAMON_MANAGED guards ==="
          check_blocked "config set" ${damon-agent}/bin/damon config set model foo
          check_blocked "config edit" ${damon-agent}/bin/damon config edit
        ...
        extra-python-packages = let
          ...
          damonWithExtra = damon-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "damon-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${damonWithExtra}/bin/damon || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${damonWithExtra}/bin/damon || \
          ...
          if grep -q "PYTHONPATH" ${damon-agent}/bin/damon; then
        ...
        extra-dependency-groups = let
          damonWithGroups = damon-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "damon-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${damonWithGroups}"
          echo "venv: ${damonWithGroups.damonVenv}"
        ...
        messaging-variant = pkgs.runCommand "damon-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.damonVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "damon-config-roundtrip" {
          ...
          merge_and_load() {
            local damon_home="$1"
            export DAMON_HOME="$damon_home"
            ${configMergeScript} ${nixSettings} "$damon_home/config.yaml"
            ${damonVenv}/bin/python3 -c '
import json, sys
