# nix/desktop.nix — Damon Desktop (Electron) app build + wrapper
#
# `damonAgent` is the fully-built `.#default` package — it ships the
# `damon` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `DAMON_DESKTOP_DAMON` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Damon CLI").
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/damon-desktop $out/bin
    cp -r ${renderer}/* $out/share/damon-desktop/
    ...
    substituteInPlace $out/share/damon-desktop/electron/main.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/damon-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # DAMON_DESKTOP_DAMON to the absolute path of the nix-built `damon`
    # binary so the desktop's resolver step 4 ("existing Damon CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/damon-desktop \
      --add-flags "$out/share/damon-desktop" \
      --set DAMON_DESKTOP_DAMON "${lib.getExe damonAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Damon Agent";
    homepage = "https://github.com/eapeli/damon";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "damon-desktop";
  };
}
