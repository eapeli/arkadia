# nix/nixosModules.nix — NixOS module for damon-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: damon runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $DAMON_HOME/.env
# and read by damon at startup — no container recreation needed for env changes.
#
# Tool resolution: the damon wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.damon-agent;
    effectivePackage =
    ...
    damon-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-damon-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "damon-config-attrs";
      description = "Damon YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "damon-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # DAMON_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "damon-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'DAMON_DOC_EOF'\n${value}\nDAMON_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "damon-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/damon";
    ...
    # damon user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "damon-container-entrypoint" ''
      set -eu

      DAMON_UID="''${DAMON_UID:?DAMON_UID must be set}"
      DAMON_GID="''${DAMON_GID:?DAMON_GID must be set}"

      # ── Group: ensure a group with GID=$DAMON_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$DAMON_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="damon"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$DAMON_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$DAMON_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$DAMON_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$DAMON_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="damon"
        TARGET_HOME="/home/damon"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$DAMON_UID" -g "$DAMON_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$DAMON_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$DAMON_UID:$DAMON_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure DAMON_HOME is owned by the target user.
      ...
      if [ -n "''${DAMON_HOME:-}" ] && [ -d "$DAMON_HOME" ]; then
        find "$DAMON_HOME" \! -user "$DAMON_UID" -exec chown "$DAMON_UID:$DAMON_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/damon-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/damon-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/damon ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/damon
        chmod 0440 /etc/sudoers.d/damon
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$DAMON_UID" --regid="$DAMON_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $DAMON_HOME/.env.
    ...
    # Default: /var/lib/damon/workspace → /data/workspace.
  ...
  in {
    options.services.damon-agent = with lib; {
      enable = mkEnableOption "Damon Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = damon-agent;
        description = "The damon-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/damon";
        description = "State directory. Contains .damon/ subdir (DAMON_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Damon config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $DAMON_HOME/.env at activation time.
          Damon reads this file on every startup via load_damon_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $DAMON_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $DAMON_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `damon gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the damon user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the damon plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Damon discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          damon_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as damon (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in damon-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the damon CLI to environment.systemPackages and export
          DAMON_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.damon symlink to the service
            stateDir. These users are automatically added to the damon group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.damon-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the damon CLI to system PATH and export DAMON_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.damon/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.DAMON_HOME = "${cfg.stateDir}/.damon";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.damon-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the damon user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.damon-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed damon
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure damon is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.damon        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.damon/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.damon/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.damon/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.damon/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.damon/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."damon-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.damon
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.damon ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.damon ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.damon -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.damon/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.damon/$_subdir"
            chmod 2770 "${cfg.stateDir}/.damon/$_subdir"
            find "${cfg.stateDir}/.damon/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # damon-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.damon/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.damon/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.damon/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.damon/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.damon/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.damon/.managed
          chmod 0644 ${cfg.stateDir}/.damon/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.damon/.container-mode <<'DAMON_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    damon_bin=${containerDataDir}/current-package/bin/damon
    DAMON_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.damon/.container-mode
            chmod 0644 ${cfg.stateDir}/.damon/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.damon/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.damon";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.damon" ]; then
                  rm -f "${symlinkPath}"
                  echo "damon-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.damon -> stateDir/.damon for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.damon";
                target = "${cfg.stateDir}/.damon";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "damon-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.damon/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.damon/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.damon/auth.json
              fi
            ''}
          ''}
          ...
          # Damon reads $DAMON_HOME/.env at startup via load_damon_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.damon/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'DAMON_NIX_ENV_EOF'
    ${envFileContent}
    DAMON_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.damon/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.damon/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.damon/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.damon-agent = {
          description = "Damon Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $DAMON_HOME/.env by the activation script. load_damon_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/damon"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the damon group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.damon-agent = {
          description = "Damon Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              DAMON_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              DAMON_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env DAMON_UID="$DAMON_UID" \
                --env DAMON_GID="$DAMON_GID" \
                --env DAMON_HOME=${containerDataDir}/.damon \
                --env DAMON_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/damon gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
