# nix/damon-agent.nix — Overridable Hermes Agent package
#
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in damon sealed venv', file=sys.stderr)
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/damon-agent $out/bin
    cp -r ${bundledSkills} $out/share/damon-agent/skills
    cp -r ${bundledPlugins} $out/share/damon-agent/plugins
    cp -r ${bundledLocales} $out/share/damon-agent/locales
    cp -r ${hermesWeb} $out/share/damon-agent/web_dist

    mkdir -p $out/ui-tui
    cp -r ${hermesTui}/lib/damon-tui/* $out/ui-tui/

    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${hermesVenv}/bin/${name} $out/bin/${name} \
          --suffix PATH : "${runtimePath}" \
          --set HERMES_BUNDLED_SKILLS $out/share/damon-agent/skills \
          --set HERMES_BUNDLED_PLUGINS $out/share/damon-agent/plugins \
          --set HERMES_BUNDLED_LOCALES $out/share/damon-agent/locales \
          --set HERMES_WEB_DIST $out/share/damon-agent/web_dist \
  ...
  passthru = {
    ...
    # ("existing damon on PATH") then picks up the fully wrapped
    ...
    devShellHook = ''
      STAMP=".nix-stamps/damon-agent"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "damon-agent: installing Python dependencies..."
