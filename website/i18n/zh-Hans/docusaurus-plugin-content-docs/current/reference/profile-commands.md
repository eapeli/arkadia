...
```bash
$ damon profile show work
Profile: work
Path:    ~/.damon/profiles/work
...
# ~/.damon/profiles/mybot → ~/.damon/profiles/assistant
...
`damon_requires` 支持 `>=`、`<=`、`==`、`!=`、`>`、`<`，或裸版本号（视为 `>=`）。若当前 Hermes 版本不满足规格，安装将失败并给出明确错误。
