...
- **平台（Platform）** — 针对特定部署环境的完整工具配置（例如，`damon-cli` 是交互式 CLI 会话的默认配置）
...
| `discord` | `discord` | 核心 Discord 文本/嵌入/私信操作（仅限 gateway）。在 `damon-discord` 工具集上激活。 |
| `discord_admin` | `discord_admin` | Discord 管理操作（封禁、角色变更、频道管理）。在 `damon-discord` 工具集上激活；需要 bot 持有相关 Discord 权限。 |
| `feishu_doc` | `feishu_doc_read` | 读取飞书/Lark 文档内容。由飞书文档评论智能回复处理器使用。 |
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | 飞书/Lark 云盘评论操作。仅限评论 agent 使用；不在 `damon-cli` 或其他消息工具集上暴露。 |
...
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | 元宝私信/群组操作和表情包搜索。仅在 `damon-yuanbao` 上注册。 |

## 平台工具集

平台工具集定义了部署目标的完整工具配置。大多数消息平台使用与 `damon-cli` 相同的配置：

| 工具集 | 与 `damon-cli` 的差异 |
|--------|------------------------|
| `damon-cli` | 完整工具集——交互式 CLI 会话的默认配置。包含 file、terminal、web、browser、memory、skills、vision、image_gen、todo、tts、delegation、code_execution、cronjob、session_search、clarify 和 `safe`（只读）套件，以及标准消息工具。 |
| `damon-acp` | 移除了 `clarify`、`cronjob`、`image_generate`、`send_message`、`text_to_speech` 以及全部四个 Home Assistant 工具。专注于 IDE 环境中的编码任务。 |
| `damon-api-server` | 移除了 `clarify`、`send_message` 和 `text_to_speech`。保留其他所有工具——适用于无法进行用户交互的程序化访问场景。 |
| `damon-cron` | 与 `damon-cli` 相同。 |
| `damon-telegram` | 与 `damon-cli` 相同。 |
| `damon-discord` | 在 `damon-cli` 基础上添加了 `discord` 和 `discord_admin`。 |
| `damon-slack` | 与 `damon-cli` 相同。 |
| `damon-whatsapp` | 与 `damon-cli` 相同。 |
| `damon-signal` | 与 `damon-cli` 相同。 |
| `damon-matrix` | 与 `damon-cli` 相同。 |
| `damon-mattermost` | 与 `damon-cli` 相同。 |
| `damon-email` | 与 `damon-cli` 相同。 |
| `damon-sms` | 与 `damon-cli` 相同。 |
| `damon-bluebubbles` | 与 `damon-cli` 相同。 |
| `damon-dingtalk` | 与 `damon-cli` 相同。 |
| `damon-feishu` | 添加了五个 `feishu_doc_*` / `feishu_drive_*` 工具（仅由文档评论处理器使用，不用于常规聊天适配器）。 |
| `damon-qqbot` | 与 `damon-cli` 相同。 |
| `damon-wecom` | 与 `damon-cli` 相同。 |
| `damon-wecom-callback` | 与 `damon-cli` 相同。 |
| `damon-weixin` | 与 `damon-cli` 相同。 |
| `damon-yuanbao` | 在 `damon-cli` 基础上添加了五个 `yb_*` 工具（私信/群组/表情包）。 |
| `damon-homeassistant` | 与 `damon-cli` 相同（Home Assistant 工具默认已存在，在设置 `HASS_TOKEN` 时激活）。 |
| `damon-webhook` | 与 `damon-cli` 相同。 |
| `damon-gateway` | 内部 gateway 编排器工具集——所有 `damon-<platform>` 工具集的并集；当 gateway 需要接受任意消息来源时使用。 |
