...
| `--ignore-user-config` | 忽略 `~/.damon/config.yaml`，回退到内置默认值。`.env` 中的凭据仍会加载。 |
...
| `hermes checkpoints` | 检查/修剪/清除 `~/.damon/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
...
| `--ignore-user-config` | 忽略 `~/.damon/config.yaml`，使用内置默认值。`.env` 中的凭据仍会加载。适用于隔离的 CI 运行、可复现的 bug 报告和第三方集成。 |
...
:::warning damon model 与 /model——了解区别
...
:::tip WSL 用户
使用 `hermes gateway run` 而非 `hermes gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s damon 'hermes gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-damon-gateway-start-fails)。
:::
...
hermes slack manifest --write      # 写入 ~/.damon/slack-manifest.json
...
多 profile、多项目协作看板。每个安装可托管多个看板（每个项目、仓库或领域一个）；每个看板是独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装从名为 `default` 的单个看板开始，其数据库为 `~/.damon/kanban.db`（向后兼容）；其他看板位于 `~/.damon/kanban/boards/<slug>/kanban.db`。嵌入在 gateway 中的调度器每次 tick 扫描所有看板。
...
| `boards switch <slug>` / `boards use` | 将 `<slug>` 持久化为活跃看板（写入 `~/.damon/kanban/current`）。 |
...
看板解析顺序（优先级从高到低）：`--board <slug>` 标志 → `HERMES_KANBAN_BOARD` 环境变量 → `~/.damon/kanban/current` 文件 → `default`。

所有操作也可作为 gateway 中的斜杠命令使用（`/kanban …`），参数界面相同——包括 `boards` 子命令和 `--board` 标志。

完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的对比、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/damon-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/user-guide/features/kanban)。
...
订阅持久化到 `~/.damon/webhook_subscriptions.json`，webhook 适配器无需重启 gateway 即可热重载。
...
创建 Hermes 配置、skill、会话和数据的 zip 归档。备份不包含 damon-agent 代码库本身。

| 选项 | 说明 |
|--------|-------------|
| `-o`, `--output <path>` | zip 文件的输出路径（默认：`~/damon-backup-<timestamp>.zip`）。 |
...
- `damon-agent` 代码本身（这是用户数据备份，不是仓库快照）。

### 示例

```bash
hermes backup                           # 完整备份到 ~/damon-backup-*.zip
...
检查和管理 `~/.damon/checkpoints/` 处的影子 git 存储——会话内 `/rollback` 命令的存储层。可随时安全运行；不需要 agent 正在运行。
...
### 示例
```bash
hermes import ~/damon-backup-20260423.zip           # 覆盖现有配置前提示确认
hermes import ~/damon-backup-20260423.zip --force   # 不提示直接覆盖
```
...
查看、跟踪和过滤 Hermes 日志文件。所有日志存储在 `~/.damon/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
Skill bundle 将多个 skill 归组到一个 `/<bundle-name>` 斜杠命令下。调用 bundle 会将每个引用的 skill 加载到单个合并的用户消息中。存储位置：`~/.damon/skill-bundles/<slug>.yaml`。YAML schema 和行为请参阅 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。
...
| `reload` | 重新扫描 `~/.damon/skill-bundles/` 并报告新增/删除的 bundle |
...
| `backup` | 手动对 `~/.damon/skills/` 进行 tar.gz 快照（curator 在每次真实运行前也会自动快照） |
| `rollback` | 从快照恢复 `~/.damon/skills/`（默认使用最新快照） |
...
检查 `~/.damon/config.yaml` 中声明的 shell 脚本 hook，针对合成 payload 测试它们，并管理 `~/.damon/shell-hooks-allowlist.json` 处的首次使用同意许可名单。
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[在 Hermes 中使用 MCP](../guides/use-mcp-with-hermes.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#running-damon-as-an-mcp-server)。
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 Hermes Plugin](../guides/build-a-damon-plugin.md)。
...
| `--no-backup` | 跳过迁移前对 `~/.damon/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.damon/backups/pre-migration-*.zip`；可用 `hermes import` 恢复）。 |
...
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `pip install damon-agent[web]`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页始终可用，但额外需要 `pty` extra（`pip install 'damon-agent[web,pty]'`）以及 POSIX PTY 环境（如 Linux、macOS 或 WSL2）。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
拉取最新的 `damon-agent` 代码并在 venv 中重新安装依赖，然后重新运行安装后 hook（MCP 服务器、skill 同步、补全安装）。可在运行中的安装上安全执行。

**pip 安装：** `hermes update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade damon-agent`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。
...
- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 前对 `~/.damon/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `hermes backup restore --state pre-update` 回滚。
- **旧版 `hermes.service` 警告。** 如果 Hermes 检测到预重命名的 `hermes.service` systemd 单元（而非当前的 `damon-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
