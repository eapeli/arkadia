...
- **你只想在自己的机器上备份一个 profile。** 使用 [`hermes profile export` / `import`](../reference/profile-commands.md#damon-profile-export)——那正是这两个命令的用途。
...
创建 `~/.damon/profiles/research-bot/distribution.yaml`：
...
4. 请求确认。传入 `-y` / `--yes` 可跳过。
5. 将分发所有的文件复制到 `~/.damon/profiles/research-bot/`（或 manifest 中 `name` 解析到的位置）。
...
# 在 ~/.damon/profiles/forked-research-bot/ 中本地迭代
...
- [`hermes profile export` / `import`](../reference/profile-commands.md#damon-profile-export) — 本地备份 / 恢复（非分发）
