...
**配置：** `$HERMES_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$HERMES_HOME/honcho.json` > `~/.damon/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/damon-ai/damon-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/hermes)。
...
参见[配置参考](https://github.com/damon-ai/damon-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/hermes)。
...
**本地模式 UI：** `hindsight-embed -p damon ui start`
...
完整配置参考参见[插件 README](https://github.com/NousResearch/damon-agent/blob/main/plugins/memory/hindsight/README.md)。
...
**配置：** `plugins.damon-memory-store` 下的 `config.yaml`
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `damon-{identity}` → `damon-coder`），按 Hermes profile 隔离记忆
