...
sudo damon gateway install --system   # Linux：服务器开机启动的系统服务
...
`~/.damon/cron/.tick.lock` 处的文件锁防止重叠的调度器 tick 重复运行同一批任务。
...
| `"local"` | 仅保存到本地文件（`~/.damon/cron/output/`） | CLI 上的默认值 |
...
如果 agent 的最终响应以 `[SILENT]` 开头，投递将被完全抑制。输出仍会保存到本地以供审计（位于 `~/.damon/cron/output/`），但不会向投递目标发送任何消息。
...
`.sh`/`.bash` 文件在 `/bin/bash` 下运行；其他文件在当前 Python 解释器（`sys.executable`）下运行。脚本必须位于 `~/.damon/scripts/`（与预运行脚本门控相同的沙箱规则）。
...
Hermes 会通过 `write_file` 将检查脚本写入 `~/.damon/scripts/`，然后调用：
...
- 任务 2 触发时，Hermes 从 `~/.damon/cron/output/{job1_id}/*.md` 读取任务 1 的最新输出
...
:::tip
Hermes 自身的 `~/.damon/state.db` 是内部 schema，会在版本间变更。不要从预运行门控中查询它——指向你自己的数据库或 feed。
:::

致谢：此方案集由 @iankar8 在 [#2654](https://github.com/NousResearch/damon-agent/pull/2654) 中的探索所启发，该 PR 提议将 sql/file/command 触发器作为并行机制添加。`script` + `wakeAgent` 门控已以零成本覆盖了所有三种情况，因此该工作以文档形式落地。
...
任务存储在 `~/.damon/cron/jobs.json`。任务运行的输出保存到 `~/.damon/cron/output/{job_id}/{timestamp}.md`。
