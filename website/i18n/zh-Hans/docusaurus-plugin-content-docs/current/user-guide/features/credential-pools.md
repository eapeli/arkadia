...
| Hermes PKCE OAuth | `~/.damon/auth.json` | 是（Anthropic） |
...
2. **`damon_cli/auth_commands.py`** — CLI 命令和交互式向导
3. **`damon_cli/runtime_provider.py`** — 感知池的凭证解析
4. **`run_agent.py`** — 错误恢复：429/402/401 → 池轮换 → 备用

## 存储

池状态存储在 `~/.damon/auth.json` 的 `credential_pool` 键下：
