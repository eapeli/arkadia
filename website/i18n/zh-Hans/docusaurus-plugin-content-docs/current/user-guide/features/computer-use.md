...
4. 启动启用了该工具集的会话：
   ...
   ```
   或在 `~/.damon/config.yaml` 中将 `computer_use` 添加到已启用的工具集列表。
...
如需对每个操作进行确认，可在 `~/.damon/config.yaml` 中配置 `approvals.mode: manual`。
...
- [通用技能：`macos-computer-use`](https://github.com/NousResearch/damon-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
