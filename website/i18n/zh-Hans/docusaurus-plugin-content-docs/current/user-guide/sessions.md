...
1. **SQLite 数据库**（`~/.damon/state.db`）——包含 FTS5 全文搜索的结构化 session 元数据，以及完整消息历史
...
要禁用摘要并保留最简单的单行行为，在 `~/.damon/config.yaml` 中设置：
...
如需更深入的分析——token 用量、费用估算、工具分解和活动模式——请使用 [`hermes insights`](/reference/cli-commands#damon-insights)。
...
| 内容 | 路径 | 描述 |
|------|------|-------------|
| SQLite 数据库 | `~/.damon/state.db` | 所有 session 元数据 + 带 FTS5 的消息 |
| Gateway 消息 | `~/.damon/state.db` | SQLite——所有 session 消息的权威存储 |
| Gateway 路由索引 | `~/.damon/sessions/sessions.json` | 将 session 键映射到活跃 session ID（来源元数据、过期标志） |

SQLite 数据库使用 WAL 模式支持并发读取和单写入，非常适合 gateway 的多平台架构。

:::note 遗留 JSONL 对话记录
在 state.db 成为权威存储之前创建的 session 可能在 `~/.damon/sessions/` 中留有
...
默认为**关闭**——session 历史对 `session_search` 召回很有价值，静默删除可能会让用户感到意外。在 `~/.damon/config.yaml` 中启用：
