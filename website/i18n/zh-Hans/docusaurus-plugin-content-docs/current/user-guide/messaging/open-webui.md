...
flowchart LR
    ...
    B["damon-agent<br/>gateway API 服务器<br/>端口 8642"]
...
- 确保 `~/.damon/.env` 包含 `API_SERVER_ENABLED`、`API_SERVER_HOST`、`API_SERVER_KEY`、`API_SERVER_PORT` 和 `API_SERVER_MODEL_NAME`
...
`hermes config set` 会自动将标志路由到 `config.yaml`，将密钥路由到 `~/.damon/.env`。如果 gateway 已在运行，请重启以使更改生效：
...
访问 **http://localhost:3000** 。创建管理员账户（第一个用户将成为管理员）。你应该能在模型下拉列表中看到你的 agent（以你的 profile 命名，默认 profile 则显示为 **damon-agent**）。开始聊天吧！
...
你的 agent 模型现在应出现在模型下拉列表中（以你的 profile 命名，默认 profile 则显示为 **damon-agent**）。
...
如果你今天需要工具在**本地**工作区运行，请在本地运行 Hermes 并将其指向纯 LLM 提供商或纯 OpenAI 兼容模型代理（例如 vLLM、LiteLLM、Ollama、llama.cpp、OpenAI、OpenRouter 等）。"远程大脑、本地执行"的分离运行时模式正在 [#18715](https://github.com/NousResearch/damon-agent/issues/18715) 中跟踪；这不是当前 API 服务器的行为。
...
- **检查模型列表**：`curl -H "Authorization: Bearer your-secret-key" http://localhost:8642/v1/models` 应返回包含 `damon-agent` 的列表
