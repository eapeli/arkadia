...
sudo damon gateway install --system   # 仅 Linux：安装开机启动的系统服务
...
在 `~/.damon/gateway.json` 中配置各平台的覆盖设置：
...
在 `~/.damon/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.damon/config.yaml` 中的 `display.background_process_notifications` 控制：
...
journalctl --user -u damon-gateway -f  # 查看日志
...
:::info 多个安装
如果你在同一台机器上运行多个 Hermes 安装（使用不同的 `HERMES_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.hermes` 使用 `damon-gateway`；其他安装使用 `damon-gateway-<hash>`。`hermes gateway` 命令会自动针对当前 `HERMES_HOME` 对应的正确服务。
:::
...
hermes gateway stop                  # 停止服务
hermes gateway status                # 检查状态
tail -f ~/.damon/logs/gateway.log   # 查看日志
```
...
|----------|---------|--------------|
| CLI | `damon-cli` | 完全访问 |
| Telegram | `damon-telegram` | 完整工具，包括终端 |
| Discord | `damon-discord` | 完整工具，包括终端 |
| WhatsApp | `damon-whatsapp` | 完整工具，包括终端 |
| Slack | `damon-slack` | 完整工具，包括终端 |
| Google Chat | `damon-google_chat` | 完整工具，包括终端 |
| Signal | `damon-signal` | 完整工具，包括终端 |
| SMS | `damon-sms` | 完整工具，包括终端 |
| Email | `damon-email` | 完整工具，包括终端 |
| Home Assistant | `damon-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `damon-mattermost` | 完整工具，包括终端 |
| Matrix | `damon-matrix` | 完整工具，包括终端 |
| DingTalk | `damon-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `damon-feishu` | 完整工具，包括终端 |
| WeCom | `damon-wecom` | 完整工具，包括终端 |
| WeCom Callback | `damon-wecom-callback` | 完整工具，包括终端 |
| Weixin | `damon-weixin` | 完整工具，包括终端 |
| BlueBubbles | `damon-bluebubbles` | 完整工具，包括终端 |
| QQBot | `damon-qqbot` | 完整工具，包括终端 |
| Yuanbao | `damon-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `damon-teams` | 完整工具，包括终端 |
| API Server | `damon-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `damon-webhook` | 完整工具，包括终端 |
...
1. **网关日志**（`~/.damon/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
