...
Baileys 桥接将会话保存在 `~/.damon/platforms/whatsapp/session` 目录下。这意味着：
...
| **会话未持久化** | 检查 `~/.damon/platforms/whatsapp/session` 是否存在且可写。如在容器中运行，请将其挂载为持久卷。 |
...
| **机器人向陌生人回复配对码** | 如需对未授权私信静默处理，在 `~/.damon/config.yaml` 中设置 `whatsapp.unauthorized_dm_behavior: ignore`。 |
...
- `~/.damon/platforms/whatsapp/session` 目录包含完整会话凭证——请像保护密码一样保护它
- 设置文件权限：`chmod 700 ~/.damon/platforms/whatsapp/session`
