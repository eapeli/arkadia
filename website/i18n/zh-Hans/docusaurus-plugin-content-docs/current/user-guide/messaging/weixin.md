...
3. 等待你用微信手机端扫描二维码
4. 提示你在手机上确认登录
5. 自动将账号凭据保存至 `~/.damon/weixin/accounts/`
...
完成首次扫码登录后，在 `~/.damon/.env` 中至少设置账号 ID：
...
- Token 按账号+对话方保存至 `~/.damon/weixin/accounts/<account_id>.context-tokens.json`
- 启动时恢复之前保存的 token
...
| 终端二维码无法渲染 | 使用 messaging 扩展重新安装：`pip install damon-agent[messaging]`。或者，打开二维码上方打印的 URL |