...
1. 生成 manifest：
   ...
   ```
   此命令会将 `~/.damon/slack-manifest.json` 写入磁盘并打印粘贴说明。
...
sudo damon gateway install --system   # 仅 Linux：开机启动系统服务
```
...
底层实现：Hermes 附带一个生成的 Slack 应用 manifest（见第一步，方式 A），它将 [`COMMAND_REGISTRY`](https://github.com/NousResearch/damon-agent/blob/main/damon_cli/commands.py) 中的每个命令声明为斜杠命令。在 Socket Mode 下，无论 manifest 的 `url` 字段如何，Slack 都会通过 WebSocket 路由命令事件。
...
3. 粘贴 `~/.damon/slack-manifest.json` 的新内容
...
除了第八步中的必需环境变量外，你还可以通过 `~/.damon/config.yaml` 自定义 Slack 机器人行为。
...
- Token 应存储在 `~/.damon/.env` 中（文件权限 `600`）
