...
规则存储在 `~/.damon/feishu_comment_rules.json`（pairing 授权存储在 `~/.damon/feishu_comment_pairing.json`），支持基于 mtime 缓存的热重载——编辑后无需重启 gateway，下一个评论事件即生效。
...
入站消息使用消息 ID 去重，TTL 为 24 小时。去重状态持久化到 `~/.damon/feishu_seen_message_ids.json`，重启后仍有效。
...
飞书 / Lark 使用 `damon-feishu` 平台预设，包含与 Telegram 及其他基于 gateway 的消息平台相同的核心工具。