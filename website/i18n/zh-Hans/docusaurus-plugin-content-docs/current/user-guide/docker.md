...
这将进入设置向导，向导会提示你输入 API 密钥并将其写入 `~/.damon/.env`。你只需执行一次。强烈建议此时为 gateway 配置一个聊天系统。
...
`/opt/data` 卷是所有 Hermes 状态的唯一数据来源。它映射到宿主机的 `~/.damon/` 目录，包含：
...
Hermes 支持[多个 profile](../reference/profile-commands.md)——独立的 `~/.damon/` 目录，让你可以从单个安装运行独立的 agent（不同的 SOUL、技能、记忆、会话、凭据）。**在 Docker 下运行时，不建议使用 Hermes 内置的多 profile 功能。**
...
- **独立生命周期** — 可独立升级、重启、暂停或回滚每个 agent（`docker restart damon-work` 不会影响 `damon-personal`）。
...
1. 以 root 身份运行 `/etc/cont-init.d/01-damon-setup`（即 `docker/stage2-hook.sh`）：可选的 UID/GID 重映射、修复卷所有权、首次启动时初始化 `.env` / `config.yaml` / `SOUL.md`、同步内置技能。
2. 运行 `/etc/cont-init.d/02-reconcile-profiles`（即 `damon_cli.container_boot`）：遍历 `$HERMES_HOME/profiles/<name>/`，在 `/run/service/gateway-<profile>/` 下重建各 profile 的 gateway s6 服务槽，并仅自动启动上次记录状态为 `running` 的 profile（参见 [Per-profile gateway 监管](#per-profile-gateway-supervision)）。
3. 启动静态的 `main-hermes` 和 `dashboard` s6-rc 服务。
4. 将容器的 CMD 作为主程序 exec（`/opt/damon/docker/main-wrapper.sh`），根据用户传给 `docker run` 的参数进行路由：
   - 无参数 → `hermes`（默认）
...
当使用 Docker 作为执行环境时（不是上述方法，而是 agent 在 Docker 沙箱内运行命令——参见 [配置 → Docker 后端](./configuration.md#docker-backend)），Hermes 为所有工具调用复用单个长期运行的容器，并自动将技能目录（`~/.damon/skills/`）和技能声明的所有凭据文件以只读卷的形式绑定挂载到该容器中。技能脚本、模板和引用在沙箱内无需手动配置即可使用，由于容器在 Hermes 进程的整个生命周期内持续存在，你安装的任何依赖或写入的文件都会在下次工具调用时保留。
...
当工具必须在每次容器启动时立即可用且无需重新安装延迟时，构建一个继承自 `nousresearch/damon-agent` 并在层中安装该工具的新镜像：
...
入口点脚本和 `/opt/data` 语义原样继承，本页其余内容仍然适用。拉取更新的上游 `nousresearch/damon-agent` 时记得重新构建镜像。
...
如果某个工具可能对大多数 Hermes Agent 用户有用，考虑将其贡献到上游，而不是在私有派生镜像中维护。在 [damon-agent 仓库](https://github.com/NousResearch/damon-agent)提交 issue 或 pull request，描述该工具及其使用场景。被纳入官方镜像的工具惠及所有用户，并避免了维护下游 fork 的开销。
...
然后在 `~/.damon/config.yaml` 中，使用**容器名称**作为主机名：
...
1. 两个容器是否在同一 Docker 网络上（`docker network inspect damon-net`）
...
容器的 stage2 hook 通过 `s6-setuidgid` 在每个受监管的服务内将权限降至非 root 用户 `hermes`（UID 10000）。如果宿主机的 `~/.damon/` 由不同 UID 拥有，请设置 `HERMES_UID`/`HERMES_GID` 以匹配宿主机用户，或确保数据目录可写：
...
```sh
docker logs --tail 50 damon          # 最近日志
docker run -it --rm nousresearch/damon-agent:latest version     # 验证版本
docker stats damon                    # 资源使用情况
```