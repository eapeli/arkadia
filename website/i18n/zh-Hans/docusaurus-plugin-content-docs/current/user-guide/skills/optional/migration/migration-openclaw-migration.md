...
| 相关 skill | [`damon-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-damon-agent) |
...
- 将 OpenClaw skill 复制到 `~/.damon/skills/openclaw-imports/`
- 可选地将 OpenClaw 工作区指令文件复制到所选 Hermes 工作区
- 将兼容的工作区资产（如 `workspace/tts/`）镜像到 `~/.damon/tts/`
...
请勿猜测更短的路径，如 `~/.damon/skills/openclaw-migration/...`。

运行辅助脚本前：

1. 优先使用 `~/.damon/skills/migration/openclaw-migration/` 下的已安装路径。
...
- 在 `~/.damon/skills/openclaw-imports/` 下可用的 OpenClaw skill
