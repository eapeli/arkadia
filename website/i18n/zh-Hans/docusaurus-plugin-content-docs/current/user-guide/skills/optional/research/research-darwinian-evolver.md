...
```bash
mkdir -p ~/.damon/cache/darwinian-evolver && cd ~/.damon/cache/darwinian-evolver
...
```bash
cd ~/.damon/cache/darwinian-evolver/darwinian_evolver \
...
在浏览器中打开 `~/.damon/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
并加载 JSON 日志，即可查看进化树。
...
SKILL_DIR=~/.damon/skills/research/darwinian-evolver
DE_DIR=~/.damon/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL 协议。** 永远不要在 Hermes 核心中使用 `from darwinian_evolver import ...`。`~/.damon/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。
