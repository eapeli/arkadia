...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/autonomous-ai-agents/damon-agent` |
| 版本 | `2.1.0` |
...
**文档：** https://damon-agent.nousresearch.com/docs/
...
平台文档：https://damon-agent.nousresearch.com/docs/user-guide/messaging/
...
在交互式聊天会话中输入这些命令。新命令会不定期上线；如果以下内容看起来过时，请在会话内运行 `/help` 获取权威列表，或查看[实时斜杠命令参考](https://damon-agent.nousresearch.com/docs/reference/slash-commands)。命令注册表的权威来源是 `damon_cli/commands.py` — 每个消费方（自动补全、Telegram 菜单、Slack 映射、`/help`）均从中派生。
...
~/.damon/sessions/         Session transcripts
...
Profiles 使用 `~/.damon/profiles/<name>/`，布局相同。
...
完整配置参考：https://damon-agent.nousresearch.com/docs/user-guide/configuration
...
完整提供商文档：https://damon-agent.nousresearch.com/docs/integrations/providers
...
某些 shell hook 集成在触发前需要明确加入允许列表。通过 `~/.damon/shell-hooks-allowlist.json` 管理——在 hook 首次尝试运行时以交互方式提示。
...
用户文档：https://damon-agent.nousresearch.com/docs/user-guide/features/cron
...
- **遥测：** `~/.damon/skills/.usage.json` 中的 sidecar 保存每个 skill 的 `use_count`、`view_count`、`patch_count`、`last_activity_at`、`state`、`pinned`。

配置：`curator.*`（`enabled`、`interval_hours`、`min_idle_hours`、`stale_after_days`、`archive_after_days`、`backup.*`）。
用户文档：https://damon-agent.nousresearch.com/docs/user-guide/features/curator
...
用户文档：https://damon-agent.nousresearch.com/docs/user-guide/features/kanban
...
- **Gateway 崩溃循环**：重置失败状态：`systemctl --user reset-failed damon-gateway`
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `hermes config edit` 或[配置文档](https://damon-agent.nousresearch.com/docs/user-guide/configuration) |
| 可用工具 | `hermes tools list` 或[工具参考](https://damon-agent.nousresearch.com/docs/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](https://damon-agent.nousresearch.com/docs/reference/slash-commands) |
| Skill 目录 | `hermes skills browse` 或[Skill 目录](https://damon-agent.nousresearch.com/docs/reference/skills-catalog) |
| 提供商设置 | `hermes model` 或[提供商指南](https://damon-agent.nousresearch.com/docs/integrations/providers) |
| 平台设置 | `hermes gateway setup` 或[消息文档](https://damon-agent.nousresearch.com/docs/user-guide/messaging/) |
| MCP 服务器 | `hermes mcp list` 或[MCP 指南](https://damon-agent.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `hermes profile list` 或[Profiles 文档](https://damon-agent.nousresearch.com/docs/user-guide/profiles) |
| Cron 任务 | `hermes cron list` 或[Cron 文档](https://damon-agent.nousresearch.com/docs/user-guide/features/cron) |
| 记忆 | `hermes memory status` 或[记忆文档](https://damon-agent.nousresearch.com/docs/user-guide/features/memory) |
| 环境变量 | `hermes config env-path` 或[环境变量参考](https://damon-agent.nousresearch.com/docs/reference/environment-variables) |
| CLI 命令 | `hermes --help` 或[CLI 参考](https://damon-agent.nousresearch.com/docs/reference/cli-commands) |
| Gateway 日志 | `~/.damon/logs/gateway.log` |
| 会话文件 | `~/.damon/sessions/` 或 `hermes sessions browse` |
| 源代码 | `~/.damon/damon-agent/` |

---

## 贡献者快速参考

面向偶尔贡献者和 PR 作者。完整开发者文档：https://damon-agent.nousresearch.com/docs/developer-guide/
...
配置：`~/.damon/config.yaml`（设置）、`~/.damon/.env`（API key）。
...
所有处理器必须返回 JSON 字符串。路径使用 `get_damon_home()`，永远不要硬编码 `~/.hermes`。

### 添加斜杠命令

1. 在 `damon_cli/commands.py` 的 `COMMAND_REGISTRY` 中添加 `CommandDef`
...
- 测试自动将 `HERMES_HOME` 重定向到临时目录——永远不会触及真实的 `~/.damon/`
- 推送任何变更前运行完整套件
...
- POSIX 文件模式（0o600 等）→ `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")`（参见 `tests/damon_cli/test_auth_toctou_file_modes.py`）
- `signal.SIGALRM` → 仅 Unix（参见 `tests/conftest.py::_enforce_test_timeout`）
...
- 所有路径使用 `damon_constants` 中的 `get_damon_home()`（profile 安全）
