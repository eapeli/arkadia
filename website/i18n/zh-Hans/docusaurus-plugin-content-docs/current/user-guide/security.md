...
审批系统支持三种模式，通过 `~/.damon/config.yaml` 中的 `approvals.mode` 配置：
...
| `tee` 写入 `/etc/`、`~/.ssh/`、`~/.damon/.env` | 通过 tee 覆盖敏感文件 |
| `>` / `>>` 写入 `/etc/`、`~/.ssh/`、`~/.damon/.env` | 通过重定向覆盖敏感文件 |
...
| `pkill`/`killall` damon/gateway | 防止自我终止 |
...
通过"always"批准的命令会保存到 `~/.damon/config.yaml`：
...
在 `~/.damon/.env` 中以逗号分隔的值设置允许的用户 ID：
...
在 `~/.damon/config.yaml` 中控制未授权私信的处理方式：
...
**存储：** 配对数据存储于 `~/.damon/pairing/`，按平台分为独立的 JSON 文件：
...
- **持久模式**（`container_persistent: true`）：从 `~/.damon/sandboxes/docker/<task_id>/` 绑定挂载 `/workspace` 和 `/root`
...
路径相对于 `~/.damon/`。文件在容器内挂载到 `/root/.damon/`。
...
4. **安全存储密钥** — 将 API 密钥保存在具有适当文件权限的 `~/.damon/.env` 中
...
9. **监控日志** — 检查 `~/.damon/logs/` 中的未授权访问尝试
...
为获得最高安全性，请在独立的机器或虚拟机上运行 gateway。在 `config.yaml` 中设置 `terminal.backend: ssh`，然后通过 `~/.damon/.env` 中的环境变量提供主机详情：
...
Hermes 内置了一个公告扫描器，用于标记活跃 venv 中与已知受损版本目录匹配的 Python 包（例如 2026 年 5 月的 `mistralai 2.4.6` 供应链投毒事件）。实现位于 `damon_cli/security_advisories.py`。
...
许多功能（Mistral TTS、ElevenLabs、Honcho 记忆、Bedrock、Slack、Matrix 等）依赖并非每个用户都需要的 Python 包。Hermes 在首次使用时**懒加载**安装这些包，而非在 `damon-agent[all]` 下急切安装。实现位于 `tools/lazy_deps.py`。
