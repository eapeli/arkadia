...
- **已配置 LLM 提供商**——至少在 `~/.damon/.env` 中配置了 OpenAI、Anthropic 或其他受支持提供商的 API 密钥
...
2. **发送 `/newbot`**——BotFather 会询问两件事：
   ...
   - **用户名**——必须以 `bot` 结尾（例如 `myteam_damon_bot`）
...
sudo damon gateway install --system   # 仅 Linux：开机启动的系统服务
```
...
控制机器人在使用工具时显示的详细程度。在 `~/.damon/config.yaml` 中：
...
通过编辑 `~/.damon/SOUL.md` 自定义机器人的沟通方式：
...
| 内容 | 位置 |
|------|----------|
| Gateway 日志 | `journalctl --user -u damon-gateway`（Linux）或 `~/.damon/logs/gateway.log`（macOS） |
| Cron 任务输出 | `~/.damon/cron/output/{job_id}/{timestamp}.md` |
| Cron 任务定义 | `~/.damon/cron/jobs.json` |
| 配对数据 | `~/.damon/pairing/` |
| 会话历史 | `~/.damon/sessions/` |
