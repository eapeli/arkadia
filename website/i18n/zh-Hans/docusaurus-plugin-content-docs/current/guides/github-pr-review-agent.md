...
:::tip 没有消息通知？没关系
使用 `deliver: "local"` 将审查结果保存到 `~/.damon/cron/output/`。在接入通知之前用于测试非常方便。
:::
...
创建 `~/.damon/skills/code-review/SKILL.md`：
...
- **[构建 Plugin](/guides/build-a-damon-plugin)** — 将审查逻辑封装为可共享的 plugin
