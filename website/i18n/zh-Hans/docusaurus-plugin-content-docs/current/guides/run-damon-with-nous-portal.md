...
3. 在 `~/.damon/config.yaml` 中设置 `model.provider: nous`
...
对于多人共用一台机器的团队场景，每个人有自己的 Portal 账号 → 每个 home 目录保存各自的 `~/.damon/auth.json` → 用户之间不共享 token。这是正确的边界划分。
...
如果某个模型确实不可用，请[提交 issue](https://github.com/NousResearch/damon-agent/issues)——大多数缺失是我们可以更新的路由配置问题。
