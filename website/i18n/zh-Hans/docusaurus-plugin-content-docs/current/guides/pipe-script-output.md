...
- 需要将结果推送给你的 cron 脚本
- 从终端发送一次性消息
- 将任意工具的输出管道到任意平台（`make | damon send --to slack:#builds`）
...
3. **管道 stdin** — `echo hi | damon send --to telegram`
...
if [ "$ram_pct" -ge 85 ]; then
  damon send --to telegram --subject "⚠ MEMORY WARNING" \
...
if ./scripts/deploy.sh; then
  damon send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | damon send \
...
```bash
./train.py --epochs 200 && \
  damon send --to telegram "training done" || \
  damon send --to telegram "training failed (exit $?)"
```
...
**通常不需要。** 对于所有基于 bot token 的平台——Telegram、Discord、Slack、Signal、SMS、WhatsApp Cloud API 等——`hermes send` 直接使用 `~/.damon/.env` 和 `~/.damon/config.yaml` 中的凭据调用平台的 REST 接口。它是一个独立的子进程，消息投递完成后即退出。
...
列表数据来源于 `~/.damon/channel_directory.json`，gateway 运行期间每隔几分钟刷新一次。如果看到"尚未发现频道"，请先启动一次 gateway（`hermes gateway start`）以填充缓存。
