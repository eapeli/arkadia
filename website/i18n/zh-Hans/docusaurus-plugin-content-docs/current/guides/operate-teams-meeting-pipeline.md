...
Hermes 内置 cron 调度器。`--no-agent` 模式以脚本作为任务执行（而非使用 LLM），`--script` 必须指向 `~/.damon/scripts/` 下的文件。首先创建脚本：
...
创建 `/etc/systemd/system/damon-teams-pipeline-maintain.service`：
...
以及 `/etc/systemd/system/damon-teams-pipeline-maintain.timer`：
...
确保 cron 环境中包含 `MSGRAPH_*` 凭据。最简单的方法：在 crontab 调用的包装脚本顶部 source `~/.damon/.env`。
