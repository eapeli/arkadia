...
完成本指南需要**租户管理员权限**（或由管理员代为授予同意）。请记录收集到的值——最终需要填入 `~/.damon/.env`。
...
3. **Description：**`damon-graph-secret`。**Expires：**根据你的轮换策略选择合适的值（通常为 6-24 个月）。
4. 点击 **Add**。
...
2. 用新值更新 `~/.damon/.env` 中的 `MSGRAPH_CLIENT_SECRET`。
