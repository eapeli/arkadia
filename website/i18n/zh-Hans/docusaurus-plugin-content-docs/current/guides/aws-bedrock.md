...
- **boto3** — 通过 `pip install damon-agent[bedrock]` 安装
- **IAM 权限** — 至少需要：
...
运行 `hermes model` 后，你的 `~/.damon/config.yaml` 将包含以下内容：
...
**[sample-damon-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-damon-agent-on-aws-with-bedrock)** — 自动创建 VPC、IAM 角色、EC2 实例并配置 Bedrock。一键即可在任意区域完成部署。