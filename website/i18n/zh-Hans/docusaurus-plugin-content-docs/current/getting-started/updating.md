...
hermes update            # 执行 pip install --upgrade damon-agent
```

或手动执行：

```bash
pip install --upgrade damon-agent    # 或：uv pip install --upgrade damon-agent
```
...
1. **配对数据快照** — 保存一份轻量级的更新前状态快照（涵盖 `~/.damon/pairing/`、飞书评论规则及其他运行时修改的状态文件）。可通过 [快照与回滚](../user-guide/checkpoints-and-rollback.md) 中描述的快照恢复流程进行恢复，或从 Hermes 写入 `~/.damon/` 目录旁的最新快速快照 zip 文件中提取。
...
- 更新运行期间，所有输出会同步镜像到 `~/.damon/logs/update.log`。如果终端消失，重新连接后检查日志，确认更新是否完成以及 gateway 重启是否成功：
...
与 [GitHub releases 页面](https://github.com/NousResearch/damon-agent/releases) 上的最新版本进行比较。
...
卸载程序会提供选项，让你保留配置文件（`~/.damon/`）以便将来重新安装。
