...
# ~/.damon/damon-agent，在 Windows 上为 %LOCALAPPDATA%\hermes\damon-agent）：
...
（如果安装时未包含 `[all]`，请先运行 `cd ~/.damon/damon-agent && uv pip install -e ".[acp]"`。）
