...
- `damon_cli/runtime_provider.py` — 凭据解析，`_resolve_custom_runtime()`
- `damon_cli/auth.py` — provider 注册表，`resolve_provider()`
- `damon_cli/model_switch.py` — 共享 `/model` 切换流水线（CLI + gateway）
