...
| **MiniMax** | M2.7、M1 |
| **xAI** | Grok-4、Grok-3 |
| **Hermes** | Hermes-4-70B、Hermes-4-405B（对话，见[下方说明](#a-note-on-damon-4)） |
...
由于所有请求都通过一个经 OAuth 认证的 Portal 会话路由，你不会积累一个包含十几个长期 API 密钥的 `.env` 文件。磁盘上唯一的凭证是 `~/.damon/auth.json` 中的 refresh token（刷新令牌），Hermes 会在每次请求时从中生成短期 JWT——详见下方[令牌处理](#token-handling)。
...
2. 将 refresh token 存储至 `~/.damon/auth.json`
3. 让你从精选列表中选择一个 Nous 模型（也可跳过以保留当前模型）
4. 在 `~/.damon/config.yaml` 中将 Nous 设为推理提供商（当你选择模型时）
...
运行 `hermes setup --portal` 后，`~/.damon/config.yaml` 将如下所示：
...
OAuth refresh token 单独存储在 `~/.damon/auth.json`（不在 `config.yaml` 中——凭证与配置有意分开存放）。
...
如果某个模型确实缺失，请[提交 issue](https://github.com/NousResearch/damon-agent/issues)——我们将 Portal 目录同步至 Hermes，缺口通常意味着可以更新的路由配置。
