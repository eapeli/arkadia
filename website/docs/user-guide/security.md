...
The approval system supports three modes, configured via `approvals.mode` in `~/.damon/config.yaml`:
...
Configure the timeout in `~/.damon/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.damon/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.damon/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` damon/gateway | Self-termination prevention |
...
Commands approved with "always" are saved to `~/.damon/config.yaml`:
...
Set allowed user IDs as comma-separated values in `~/.damon/.env`:
...
Control how unauthorized direct messages are handled in `~/.damon/config.yaml`:
...
**Storage:** Pairing data is stored in `~/.damon/pairing/` with per-platform JSON files:
...
Container resources are configurable in `~/.damon/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.damon/sandboxes/docker/<task_id>/`
...
4. **Store secrets securely** — keep API keys in `~/.damon/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.damon/logs/` for unauthorized access attempts
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.damon/.env`:
...
TERMINAL_SSH_KEY=~/.ssh/damon_agent_key
```
...
Hermes ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `damon_cli/security_advisories.py`.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Hermes installs these **lazily** on first use rather than eagerly under `damon-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
