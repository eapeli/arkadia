...
1. **SQLite database** (`~/.damon/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.damon/config.yaml`:
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`hermes insights`](/reference/cli-commands#damon-insights).
...
|------|------|-------------|
| SQLite database | `~/.damon/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.damon/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.damon/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.damon/sessions/`. They are no longer written or
...
Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.damon/config.yaml`:
