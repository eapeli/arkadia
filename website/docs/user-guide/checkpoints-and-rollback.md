...
Or enable globally in `~/.damon/config.yaml`:
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.damon/checkpoints/store/` — your real project `.git` is never touched. Every project the agent works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.
...
- Once per conversation turn (per directory), it:
  ...
  - Initialises or reuses the **single shared shadow store** at `~/.damon/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/damon/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.damon/checkpoints/ at startup
...
```text
Checkpoint base: /home/you/.damon/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/damon-agent                                        20       2h ago  live
...
Clear with: damon checkpoints clear-legacy
```
...
~/.damon/checkpoints/
  ...
  │   ├── refs/damon/<hash> # per-project branch tip
...
Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.damon/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.damon/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:
