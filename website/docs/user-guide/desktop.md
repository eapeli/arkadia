...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#damon-cron).
...
The [manual update process](https://damon-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.
...
| `--damon-root PATH` | Override the Hermes source root the app uses (sets `HERMES_DESKTOP_HERMES_ROOT`)          |
...
Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.damon/.env` (the secrets file, mode 0600):
...
Running the dashboard as a systemd service? Give the unit `EnvironmentFile=%h/.damon/.env` so the credentials are in the environment at boot.
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.damon/.env` and that the dashboard process actually loaded them.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Hermes Desktop to a remote backend](./features/web-dashboard.md#connecting-damon-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Hermes Desktop](../reference/environment-variables.md#web-dashboard--damon-desktop).
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.damon/damon-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
