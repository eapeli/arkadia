...
- save provider credentials into `~/.damon/.env`
...
```bash
SCRIPT="$(find ~/.damon/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `~/.damon/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.damon/.env`
