...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.damon/.env` (the skill will prompt for this on first load).
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/damon-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/damon-op.sock"
