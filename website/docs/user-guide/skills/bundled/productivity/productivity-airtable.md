...
4. Store the token in `~/.damon/.env` (or via `hermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.damon/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
