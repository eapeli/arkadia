...
Copies your current profile's `config.yaml`, `.env`, `SOUL.md`, and skills into the new profile. Same API keys, model, and capabilities, but fresh sessions and memory. Edit `~/.damon/profiles/work/.env` for different API keys, or `~/.damon/profiles/work/SOUL.md` for a different personality.
...
The alias works with every damon subcommand — it's just `hermes -p <name>` under the hood.
...
```bash
# Edit coder's tokens
nano ~/.damon/profiles/coder/.env

# Edit assistant's tokens
nano ~/.damon/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates damon-gateway-coder systemd/launchd service
assistant gateway install     # creates damon-gateway-assistant service
```
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.damon/profiles/coder/SOUL.md
```
...
Profiles use the `HERMES_HOME` environment variable. When you run `coder chat`, the wrapper script sets `HERMES_HOME=~/.damon/profiles/coder` before launching hermes. Since 119+ files in the codebase resolve paths via `get_damon_home()`, Hermes state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
