...
When the model wants one of these, codex spawns the `damon_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Hermes' default runtime), and the result is returned to codex like any other MCP response.
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Hermes' own `hermes auth login codex` writes to `~/.damon/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
- Migrates user MCP servers from `~/.damon/config.yaml` to `~/.codex/config.toml`.
...
You can also set it manually in `~/.damon/config.yaml`:
```yaml
model:
...
(Hermes will preserve your override on re-migration as long as it lives outside the `# managed by damon-agent` markers.)
...
To route specific aux tasks to a cheaper / different model, set explicit overrides in `~/.damon/config.yaml`:
...
```toml
# managed by damon-agent — `hermes codex-runtime migrate` regenerates this section
...
CODEX_HOME=~/.damon/profiles/work/codex damon chat
```
...
When the model calls `web_search` (or another exposed Hermes tool), codex spawns the `damon_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `damon-tools` callback)
...
If you find a bug, [open an issue](https://github.com/NousResearch/damon-agent/issues) with the output of `hermes logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
...
```
        ...
        └──────────────────────────────────┘           │
                                                        ...
                                                        ▼
        ┌──────────────────────────────────────────────────────────┐
        │  damon_tools_mcp_server.py (subprocess on demand)        │
        │   web_search, web_extract, browser_*, vision_analyze,    │
        │   image_generate, skill_view, skills_list, text_to_speech│
        └──────────────────────────────────────────────────────────┘
```

For implementation details, see [PR #24182](https://github.com/NousResearch/damon-agent/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
