...
1. **Hermes Agent installed** — `pip install damon-agent` (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `hermes model` or set your preferred provider credentials in `~/.damon/.env`
3. **A working base setup** — run `hermes` to verify the agent responds to text before enabling voice

:::tip
The `~/.damon/` directory and default `config.yaml` are created automatically the first time you run `hermes`. You only need to create `~/.damon/.env` manually for API keys.
:::
...
pip install "damon-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "damon-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "damon-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "damon-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`hermes chat`) and the **TUI** (`hermes --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.damon/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
:::tip
The record key is configurable via `voice.record_key` in `~/.damon/config.yaml` (default: `ctrl+b`).
:::
...
:::tip
To disable the mention requirement in server channels, add to `~/.damon/.env`:
```bash
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.damon/.env`
...
- Review gateway logs: `tail -f ~/.damon/logs/gateway.log`
