...
Hermes runs with a curated `damon-acp` toolset designed for editor workflows. It includes:
...
```bash
uvx --from 'damon-agent[acp]==<version>' damon-acp
```
...
- Installs Node.js 22 LTS into `~/.damon/node/` if missing
...
- Configure Hermes provider credentials first with `hermes model`, or set them in `~/.damon/.env` / `~/.damon/config.yaml`.
- Install `uv` so the registry launcher can run `uvx --from 'damon-agent[acp]==<version>' damon-acp`.
...
```text
/path/to/damon-agent/acp_registry
```
...
The upstream registry PR copies those files into the top-level `damon-agent/` directory in `agentclientprotocol/registry`.

The registry entry uses a `uvx` distribution that points directly at the `damon-agent` PyPI release:
...
or by editing `~/.damon/.env`. Registry clients can also trigger Hermes' terminal auth flow, which runs the same interactive provider/model setup.
