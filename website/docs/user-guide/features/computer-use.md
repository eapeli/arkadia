...
4. Start a session with the toolset enabled:
   ```
   damon -t computer_use chat
   ```
   or add `computer_use` to your enabled toolsets in `~/.damon/config.yaml`.
...
Pair with `approvals.mode: manual` in `~/.damon/config.yaml` if you want every action confirmed.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/damon-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
