...
```bash
# After setting up a custom endpoint via damon model:
hermes auth list
# Shows:
...
| Hermes PKCE OAuth | `~/.damon/auth.json` | Yes (Anthropic) |
...
2. **`damon_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`damon_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.damon/auth.json` under the `credential_pool` key:
