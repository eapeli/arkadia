...
| CLI | Saved to `~/.damon/audio_cache/` | MP3 |
...
tts:
  ...
  piper:
    ...
    # voices_dir: ''                            # default: ~/.damon/cache/piper-voices/
...
tts:
  ...
  gemini:
    voice: Algieba
    persona_prompt_file: ~/.damon/tts/butler-voice.md
```
...
On the first TTS call for a voice that isn't cached locally, Hermes runs `python -m piper.download_voices <name>` and downloads the model (~20-90MB depending on quality tier) into `~/.damon/cache/piper-voices/`. Subsequent calls reuse the cached model.
...
Drop this in `~/.damon/plugins/my-tts/`:
...
**Mistral API (Voxtral Transcribe)** — Requires `MISTRAL_API_KEY`. Uses Mistral's [Voxtral Transcribe](https://docs.mistral.ai/capabilities/audio/speech_to_text/) models. Supports 13 languages, speaker diarization, and word-level timestamps. Install with `pip install damon-agent[mistral]`.
...
Drop this in `~/.damon/plugins/my-stt/`:
