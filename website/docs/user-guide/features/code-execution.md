...
1. The agent writes a Python script using `from damon_tools import ...`
2. Hermes generates a `damon_tools.py` stub module with RPC functions
...
from damon_tools import web_search, web_extract
...
```python
from damon_tools import search_files, read_file
import json
...
```python
from damon_tools import search_files, read_file, patch
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.damon/config.yaml`:
...
DEBUG`, or check `~/.damon/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted HERMES_* var(s)` if a script behaves
as though a `HERMES_*` variable is missing.

Hermes always writes the script and the auto-generated `damon_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
