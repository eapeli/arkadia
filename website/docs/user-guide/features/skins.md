...
/skin mytheme        # switch to a custom skin from ~/.damon/skins/mytheme.yaml
```

Or set the default skin in `~/.damon/config.yaml`:
...
Create YAML files under `~/.damon/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.damon/skins/mytheme.yaml
...
[Hermes Mod](https://github.com/cocktailpeanut/damon-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Hermes Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/damon-mod/master/nous.png)
...
- Saves directly to `~/.damon/skins/`
- Activates a skin by updating `~/.damon/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/damon-mod.git
cd damon-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.damon/skins/`.
...
- Built-in skins load from `damon_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.damon/skins/` take precedence over built-in skins with the same name.
