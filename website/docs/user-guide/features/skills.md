...
All skills live in **`~/.damon/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.
...
```bash
curl -fsSL https://damon-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Hermes to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.damon/plans/` relative to the active workspace/backend working directory.
...
/home/user/.damon/cache/chart-q4-2025.png
...
When a missing value is encountered, Hermes asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `hermes setup` or `~/.damon/.env` locally instead.
...
```text
~/.damon/skills/                  # Single source of truth
...
Add `external_dirs` under the `skills` section in `~/.damon/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.damon/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
...
```text
~/.damon/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.damon/skill-bundles/<slug>.yaml`** and look like this:
...
# Re-scan ~/.damon/skill-bundles/ and report changes
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.damon/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.damon/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.damon/pending/skills/` and are
...
[Skills Hub](https://damon-agent.nousresearch.com/docs) page — instead of a
...
```bash
hermes skills tap add my-org/damon-skills
hermes skills search deploy
hermes skills install my-org/damon-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.damon/.hub/taps.json`:
...
Taps are stored in `~/.damon/.hub/taps.json` (created on demand).

## Bundled skill updates (`hermes skills reset`)

Hermes ships with a set of bundled skills in `skills/` inside the repo. On install and on every `hermes update`, a sync pass copies those into `~/.damon/skills/` and records a manifest at `~/.damon/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).
