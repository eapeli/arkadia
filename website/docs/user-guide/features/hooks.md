...
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.damon/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.damon/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.damon/hooks/` containing two files:
...
```yaml
# ~/.damon/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.damon/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.damon/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.damon/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path
...
```yaml
# ~/.damon/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.damon/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/damon-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.damon/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."
...
1. A file at `~/.damon/BOOT.md` with natural-language startup instructions.
...
Create `~/.damon/BOOT.md`. Write it as if you were giving instructions to a human assistant:
...
**`~/.damon/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.damon/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.damon/hooks/boot-md/handler.py`**

```python
"""Run ~/.damon/BOOT.md on every gateway startup."""
...
Delete `~/.damon/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.damon/hooks/`
...
See the **[Build a Plugin guide](/guides/build-a-damon-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`damon_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.damon/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.damon/agent-hooks/` (by convention) | `~/.damon/plugins/<name>/` | `~/.damon/hooks/<name>/` |
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.damon/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.damon/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.damon/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.damon/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.damon/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.damon/agent-hooks/inject-cwd-context.sh
...
hooks:
  subagent_stop:
    - command: "~/.damon/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.damon/agent-hooks/log-orchestration.sh
log=~/.damon/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Hermes sees it, then persists the decision to `~/.damon/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.
...
- Keep scripts inside `~/.damon/agent-hooks/` so the path is easy to audit.
