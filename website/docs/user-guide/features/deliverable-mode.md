...
persona in `~/.damon/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.damon/config.yaml` (switchable per session
via `/personality`).
...
Install MCP servers via `~/.damon/config.yaml` under the `mcp_servers`
