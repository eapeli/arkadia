...
sudo damon gateway install --system   # Linux: boot-time system service for servers
...
1. loads jobs from `~/.damon/cron/jobs.json`
...
A file lock at `~/.damon/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.damon/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.damon/cron/output/`), but no message is sent to the delivery target.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.damon/scripts/` (same sandboxing rule as the pre-run script gate).
...
Hermes will write the check script to `~/.damon/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.damon/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.damon/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.damon/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.damon/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Hermes reads Job 1's most recent output from `~/.damon/cron/output/{job1_id}/*.md`
...
```bash
#!/bin/bash
# ~/.damon/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.damon/scripts/.feed-changed.last"
...
:::tip
Hermes's own `~/.damon/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/NousResearch/damon-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.damon/cron/jobs.json`. Output from job runs is saved to `~/.damon/cron/output/{job_id}/{timestamp}.md`.
