...
The default `damon-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'damon-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install damon-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- `ptyprocess` — installed by the `pty` extra (`pip install 'damon-agent[web,pty]'`, or `[all]` covers both)
...
To point [Hermes Desktop](#connecting-damon-desktop-to-a-remote-backend) at a dashboard running on another machine instead of its own bundled backend, see the remote-backend section below.
...
```ini
[Service]
EnvironmentFile=%h/.damon/.env
ExecStart=/path/to/venv/bin/python -m damon_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.damon/.env` containing:
...
Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.damon/skills/` and grouped by category.
...
- **Restart gateway** — credentials are written to `~/.damon/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
This re-reads `~/.damon/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
| `GET /api/damon/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
- **CLI — `hermes dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `hermes setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.damon/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  damon dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes HERMES_DASHBOARD_OAUTH_CLIENT_ID to ~/.damon/.env
  ```
...
**1. Log in and register the dashboard.** `hermes dashboard register` uses your existing Nous login to provision an OAuth client and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.damon/.env` for you:
...
**1. Set credentials in `~/.damon/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/damon/   # required
      client_id: damon-dashboard                              # required
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-hermes.json` — it defines a `hermes` realm, a **public PKCE client** (`damon-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
`http://localhost:8080/realms/damon/.well-known/openid-configuration` (issuer
...
export HERMES_DASHBOARD_OIDC_CLIENT_ID="damon-dashboard"
...
> admin console (Clients → damon-dashboard → Settings). The same pattern works
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `damon_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `damon_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `damon_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `damon_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
```python
# ~/.damon/plugins/dashboard-auth-myidp/__init__.py
from damon_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# 1. Set the dashboard login credentials in ~/.damon/.env (secrets file, 0600).
cat >> ~/.damon/.env <<'EOF'
HERMES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HERMES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
If you run the dashboard as a systemd service, `~/.damon/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.damon/.env`, so the credentials are in the environment at boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `damon_cli/web_dist/` which the FastAPI server serves as a static SPA.
