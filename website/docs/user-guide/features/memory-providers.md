...
Or set manually in `~/.damon/config.yaml`:
...
**Config:** `$HERMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$HERMES_HOME/honcho.json` > `~/.damon/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/damon-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
See the [config reference](https://github.com/NousResearch/damon-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.damon/.env
```
...
echo "MEM0_API_KEY=your-key" >> ~/.damon/.env
```
...
| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `damon-user` | User identifier |
...
echo "HINDSIGHT_API_KEY=your-key" >> ~/.damon/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p damon ui start`
...
See [plugin README](https://github.com/NousResearch/damon-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Config:** `config.yaml` under `plugins.damon-memory-store`
...
echo "RETAINDB_API_KEY=your-key" >> ~/.damon/.env
```
...
echo 'SUPERMEMORY_API_KEY=***' >> ~/.damon/.env
```
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `damon-{identity}` → `damon-coder`) to isolate memories per Hermes profile
...
| **Requires** | `pip install damon-memori` + `damon-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install damon-memori
damon-memori install
...
| **Memori** | Cloud | Free/Paid | 5 | `damon-memori` | Tool-aware memory + structured recall |
