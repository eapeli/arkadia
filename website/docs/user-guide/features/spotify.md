...
Hermes can control Spotify directly — playback, queue, search, playlists, saved tracks/albums, and listening history — using Spotify's official Web API with PKCE OAuth. Tokens are stored in `~/.damon/auth.json` and refreshed automatically on 401; you only log in once per machine.
...
4. Saves it to `~/.damon/.env` so future runs skip this step
5. Continues straight into the OAuth consent flow

After you approve, tokens are written under `providers.spotify` in `~/.damon/auth.json`. The active inference provider is NOT changed — Spotify auth is independent of your LLM provider.
...
| Field | Value |
|-------|-------|
| App name | anything (e.g. `damon-agent`) |
...
Removes tokens from `~/.damon/auth.json`. To also clear the app config, delete `HERMES_SPOTIFY_CLIENT_ID` (and `HERMES_SPOTIFY_REDIRECT_URI` if you set it) from `~/.damon/.env`, or run the wizard again.
...
**`INVALID_CLIENT: Invalid redirect URI`** — the redirect URI in your Spotify app settings doesn't match what Hermes is using. The default is `http://127.0.0.1:43827/spotify/callback`. Either add that to your app's allowed redirect URIs, or set `HERMES_SPOTIFY_REDIRECT_URI` in `~/.damon/.env` to whatever you registered.
...
Or set them permanently in `~/.damon/.env`:
...
| File | Contents |
|------|----------|
| `~/.damon/auth.json` → `providers.spotify` | access token, refresh token, expiry, scope, redirect URI |
| `~/.damon/.env` | `HERMES_SPOTIFY_CLIENT_ID`, optional `HERMES_SPOTIFY_REDIRECT_URI` |
