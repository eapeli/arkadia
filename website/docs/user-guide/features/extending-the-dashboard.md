...
1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.damon/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.damon/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.damon/dashboard-themes
```

```yaml
# ~/.damon/dashboard-themes/neon.yaml
name: neon
label: Neon
...
The CSS is injected as a single scoped `<style data-damon-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
```yaml
# ~/.damon/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
```bash
mkdir -p ~/.damon/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.damon/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.damon/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
```json
// ~/.damon/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.damon/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
A reference plugin (`example-dashboard` in [`damon-example-plugins`](https://github.com/NousResearch/damon-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
```python
# ~/.damon/plugins/my-plugin/dashboard/plugin_api.py
...
Backend routes run inside the dashboard process, so they can import from the damon-agent codebase directly:
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.damon/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.damon/plugins/<name>/dashboard/` | `project` — only when `HERMES_ENABLE_PROJECT_PLUGINS` is set |
...
The [`strike-freedom-cockpit`](https://github.com/NousResearch/damon-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `damon-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/NousResearch/damon-example-plugins.git

# Theme
cp damon-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.damon/dashboard-themes/

# Plugin
cp -r damon-example-plugins/strike-freedom-cockpit ~/.damon/plugins/
```
...
Check that the file is in `~/.damon/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.damon/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.damon/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Tail `~/.damon/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.damon/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
