...
hermes -s damon-agent-dev,github-auth
...
Every installed skill in `~/.damon/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.damon/config.yaml`:
...
Resume this session with:
  damon --resume 20260225_143052_a1b2c3
...
CLI sessions are stored in Hermes's SQLite state database under `~/.damon/state.db`. The database keeps:
