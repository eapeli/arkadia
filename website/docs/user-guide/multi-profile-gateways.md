...
(`damon-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
`~/.local/bin/damon-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: damon-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    damon gateway "$action"
  else
    damon -p "$profile" gateway "$action"
  fi
}
...
```bash
damon-gateways start      # start every configured profile
damon-gateways stop       # stop every configured profile
damon-gateways restart    # restart all
damon-gateways status     # status across all
damon-gateways list       # delegates to `hermes gateway list`
```
...
| Linux    | `~/.config/systemd/user/damon-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.hermes.gateway.plist` /
`damon-gateway.service`.
...
```bash
# Default profile
tail -f ~/.damon/logs/gateway.log
tail -f ~/.damon/logs/gateway.error.log

# Named profile
tail -f ~/.damon/profiles/<name>/logs/gateway.log
tail -f ~/.damon/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.damon/logs/gateway.log ~/.damon/profiles/*/logs/gateway.log
```
...
damon-gateways status          # full status across every profile
launchctl list | grep damon    # macOS — PIDs and labels
systemctl --user list-units 'damon-gateway-*'   # Linux — units
```
...
The default profile uses `~/.damon/` directly with the same three files.
...
caffeinate -i -w $(cat ~/.damon/gateway.pid) &   # awake while default gateway runs
...
`damon-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
```bash
ps -ef | grep "damon_cli.*-p <profile>"
cat ~/.damon/profiles/<profile>/gateway.pid
...
# Linux
systemctl --user restart damon-gateway-<profile>.service
```
