...
- Messaging gateways: the directory set by `terminal.cwd` in `~/.damon/config.yaml`
...
git worktree add ../repo-feature feature/damon-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/damon-experiment` checked out in that directory
...
git worktree add ../repo-experiment-a feature/damon-a
git worktree add ../repo-experiment-b feature/damon-b
```
...
- Works on its own branch (`feature/damon-a` vs `feature/damon-b`).
...
- Hermes checkpoint data under `~/.damon/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.
...
- **Name branches after the experiment**
  - e.g. `feature/damon-checkpoints-docs`, `feature/damon-refactor-tests`.
- **Commit frequently**
...
- Check out an isolated branch (e.g. `damon/damon-<hash>`).
