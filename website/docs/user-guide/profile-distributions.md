...
- **You just want to back up a profile on your own machine.** Use [`hermes profile export` / `import`](../reference/profile-commands.md#damon-profile-export) — that's what those are for.
...
# Edit ~/.damon/profiles/research-bot/SOUL.md
...
Create `~/.damon/profiles/research-bot/distribution.yaml`:
...
```bash
cd ~/.damon/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
5. Copies distribution-owned files into `~/.damon/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
```bash
cp ~/.damon/profiles/research-bot/.env.EXAMPLE ~/.damon/profiles/research-bot/.env
...
Path:    ~/.damon/profiles/research-bot
...
```bash
# You
cd ~/.damon/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/damon-polymarket-trader.git
git push -u origin main --tags

# Anyone
hermes profile install github.com/you/damon-polymarket-trader --alias
```
...
```yaml
# ~/.damon/profiles/research-bot/local/my-overrides.yaml
...
# Iterate locally in ~/.damon/profiles/forked-research-bot/
...
hermes profile install ~/.damon/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
hermes profile delete research-bot-test --yes
hermes profile install ~/.damon/profiles/research-bot --name research-bot-test
```
...
- [`hermes profile export` / `import`](../reference/profile-commands.md#damon-profile-export) — local backup / restore (not distribution)
