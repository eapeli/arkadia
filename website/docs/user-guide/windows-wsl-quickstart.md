...
The damon gateway (and anything else you want to keep running) is easier to manage with systemd. On modern WSL, enable it once inside your distro:
...
- Your Hermes install (`~/.damon/`) — Linux side. The installer already does this.
...
2. For the target, use your distro name (replace `Ubuntu` if needed):
   ...
   ```text
   wt.exe -w 0 -p "Ubuntu" wsl.exe -d Ubuntu --cd ~ -- bash -ic "damon"
   ```
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-hermes#wsl2-bridge-damon-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Hermes in WSL.
