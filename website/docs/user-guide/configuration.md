...
All settings are stored in the `~/.damon/` directory for easy access.
...
2. **`~/.damon/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.damon/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Leaving these unset keeps the legacy defaults (`HERMES_API_TIMEOUT=1800`s, `HERMES_API_CALL_STALE_TIMEOUT=300`s, native Anthropic 900s). Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/NousResearch/damon-agent/blob/main/cli-config.yaml.example).
...
damon_home = Path(os.environ["HERMES_HOME"])
...
**`docker_env`** vs **`docker_forward_env`**: the former injects literal `KEY=value` pairs you specify in the config (the values live in your `config.yaml` or are passed as a JSON dict via `TERMINAL_DOCKER_ENV='{"DEBUG":"1"}'`). The latter forwards values from your shell or `~/.damon/.env`, so the actual secret never appears in the config file. Use `docker_forward_env` for tokens and `docker_env` for static knobs the container needs.
...
- `damon-agent=1` — marks it as Hermes-managed
- `damon-task-id=<sanitized task_id>` — keys the per-task reuse probe
- `damon-profile=<sanitized profile name>` — scopes reuse and reaping to the active Hermes profile

On startup, Hermes runs `docker ps --filter label=damon-task-id=<id> --filter label=damon-profile=<profile>` and **attaches to the existing container** when it finds one. If the container is `exited` (e.g. after a Docker daemon restart), it's `docker start`'d and reused — filesystem state and any installed packages survive, but in-container background processes do not.
...
| Orphan reaper at next startup | Sweeps **Exited** damon-labeled containers older than `2 × lifetime_seconds` (default 600s = 10 min), scoped to the current profile. **Running containers are never touched** — sibling-process safety. Set `docker_orphan_reaper: false` to disable. |
...
- **Switching profiles** isolates containers from each other — a container labeled `damon-profile=work` is invisible to a Hermes process running under `damon-profile=research`. The orphan reaper is profile-scoped too, so cross-profile containers don't get reaped accidentally, but they also won't get cleaned up automatically until you start Hermes again under their original profile.
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.damon/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.damon/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.damon/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `damon-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/damon-agent` (HPC convention) → `~/.damon/sandboxes/singularity`.
...
- The remote sandbox may have been torn down by the time you go looking; the local `~/.damon/cache/remote-syncs/…` copy is the authoritative record of what the agent changed.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.damon/cache/documents:/output" # Gateway-visible exports
```
...
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.damon/cache/documents/report.txt`
...
Hermes resolves each listed variable from your current shell first, then falls back to `~/.damon/.env` if it was saved with `hermes config set`.
...
```yaml
worktree: true    # Always create a worktree (same as damon -w)
...
Or via environment variable (in `~/.damon/.env`):
...
quick_commands:
  status:
    type: exec
    command: systemctl status damon-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.damon/damon-agent && git pull && pip install -e .
  gpu:
    type: exec
...
**Exa:** Set `EXA_API_KEY` in `~/.damon/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.damon/browser_recordings/
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: damon chat --checkpoints). Default: false (opt-in).
...
| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.damon/SOUL.md` or `$HERMES_HOME/SOUL.md` |
...
| **Messaging gateway** | `terminal.cwd` from `~/.damon/config.yaml`; if unset, home directory `~` |
...
`MESSAGING_CWD` and direct `TERMINAL_CWD` entries in `~/.damon/.env` are legacy compatibility fallbacks. New configurations should use `terminal.cwd`.
