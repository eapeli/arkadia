...
```powershell
iex (irm https://raw.githubusercontent.com/NousResearch/damon-agent/main/scripts/install.ps1)
```
...
```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/NousResearch/damon-agent/main/scripts/install.ps1))) -NoVenv -SkipSetup -Branch main
```
...
| `-InstallDir` | `%LOCALAPPDATA%\hermes\damon-agent` | Override code location |
...
On first launch (and on demand when a missing tool is detected), Hermes runs a small Python bootstrapper — `damon_cli/dep_ensure.py` — that checks for and lazily installs the non-Python dependencies it needs. On Windows, the relevant ones are:
...
5. **Clones the repo** to `%LOCALAPPDATA%\hermes\damon-agent` and creates a virtualenv inside it.
...
9. **Adds `%LOCALAPPDATA%\hermes\damon-agent\venv\Scripts` to User PATH and sets `HERMES_HOME=%LOCALAPPDATA%\hermes`** — exposes the `hermes` command (and points it at your data dir) after you open a new terminal.
...
The fix is in `damon_cli/stdio.py::configure_windows_stdio()`, called early in every entry point (`cli.py::main`, `damon_cli/main.py::main`, `gateway/run.py::main`). It:
...
| Path | Contents |
|---|---|
| `%LOCALAPPDATA%\hermes\damon-agent\` | Git checkout + venv. `venv\Scripts\hermes.exe` is the command added to User PATH. Safe to `Remove-Item -Recurse` and reinstall. |
...
On native Windows the installer sets `HERMES_HOME=%LOCALAPPDATA%\hermes`, so your data and the disposable install live under the **same** `%LOCALAPPDATA%\hermes` root: the install/runtime is the `damon-agent\`, `git\`, `node\`, and `bin\` subdirectories, while your data files sit directly in `%LOCALAPPDATA%\hermes`. Reinstalling only replaces the `damon-agent\` checkout, so your data survives — but because the two share a root, **don't** `Remove-Item -Recurse %LOCALAPPDATA%\hermes` if you want to keep your data; delete the `damon-agent\` subdirectory instead. Your data directory is identical in shape to a Linux `~/.hermes`, so you can mirror it between machines.
...
The installer adds `%LOCALAPPDATA%\hermes\damon-agent\venv\Scripts` to your **User PATH** via `[Environment]::SetEnvironmentVariable`. Existing terminals don't pick this up — open a new PowerShell window (or Windows Terminal tab) after installation. Close-and-reopen, don't `$env:PATH += …` by hand unless you know what you're doing.

Verify:

```powershell
Get-Command damon        # should print C:\Users\<you>\AppData\Local\hermes\damon-agent\venv\Scripts\hermes.exe
hermes --version
```
...
That's the clean path — removes the schtasks entry, Startup folder shortcut, `hermes.cmd` shim, deletes `%LOCALAPPDATA%\hermes\damon-agent\`, and trims the User PATH. It leaves the rest of `%LOCALAPPDATA%\hermes\` alone (your config, auth, skills, sessions, logs) in case you're reinstalling.
