...
- A strong shared secret to use as the `clientState` value. Generate with `openssl rand -hex 32` and put it in `~/.damon/.env` as `MSGRAPH_WEBHOOK_CLIENT_STATE`.

## Quick Start

Minimum `~/.damon/config.yaml`:
...
Or via env vars in `~/.damon/.env` (auto-merged on startup):
