...
# devtunnel
devtunnel create damon-line --allow-anonymous
devtunnel port create damon-line -p 8646 --protocol https
devtunnel host damon-line
```
...
**Bot receives nothing in groups.** Check `LINE_ALLOWED_GROUPS` includes the `C...` group ID. To find a group ID, send a test message and grep `~/.damon/logs/gateway.log` for `LINE: rejecting unauthorized source` — the rejected source dict has the IDs.
