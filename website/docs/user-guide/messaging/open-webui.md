...
flowchart LR
    ...
    B["damon-agent<br/>gateway API server<br/>port 8642"]
...
- ensures `~/.damon/.env` contains `API_SERVER_ENABLED`, `API_SERVER_HOST`, `API_SERVER_KEY`, `API_SERVER_PORT`, and `API_SERVER_MODEL_NAME`
...
`hermes config set` auto-routes the flag to `config.yaml` and the secret to `~/.damon/.env`. If the gateway is already running, restart it so the change takes effect:

```bash
hermes gateway stop && damon gateway
```
...
# {"object":"list","data":[{"id":"damon-agent", ...}]}
```
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **damon-agent** for the default profile). Start chatting!
...
Your agent model should now appear in the model dropdown (named after your profile, or **damon-agent** for the default profile).
...
2. Edit your damon-agent connection
...
- **Check model listing**: `curl -H "Authorization: Bearer your-secret-key" http://localhost:8642/v1/models` should return a list with `damon-agent`
