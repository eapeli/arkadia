...
Or set environment variables directly in `~/.damon/.env`:
...
**Pre-authorize specific users** (in `~/.damon/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.damon/.env`):
```bash
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.damon/config.yaml` (default: `true`). There is no corresponding environment variable.
