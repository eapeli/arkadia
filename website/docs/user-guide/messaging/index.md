...
sudo damon gateway install --system   # Linux only: install a boot-time system service
...
Configure per-platform overrides in `~/.damon/gateway.json`:
...
Control how much tool activity is displayed in `~/.damon/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.damon/config.yaml`:
...
journalctl --user -u damon-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo damon gateway install --system
sudo damon gateway start --system
sudo damon gateway status --system
journalctl -u damon-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `hermes update`. When `hermes update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart damon-gateway` command (it never blocks on an interactive password prompt).
...
After that, `hermes update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo damon update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/damon-gateway`:

```
hermes ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed damon-gateway*, /usr/bin/systemctl --no-ask-password start damon-gateway*, /usr/bin/systemctl --no-ask-password restart damon-gateway*
```
:::
...
If you run multiple Hermes installations on the same machine (with different `HERMES_HOME` directories), each gets its own systemd service name. The default `~/.hermes` uses `damon-gateway`; other installations use `damon-gateway-<hash>`. The `hermes gateway` commands automatically target the correct service for your current `HERMES_HOME`.
:::
...
hermes gateway stop                  # Stop the service
hermes gateway status                # Check status
tail -f ~/.damon/logs/gateway.log   # View logs
```
...
|----------|---------|--------------|
| CLI | `damon-cli` | Full access |
| Telegram | `damon-telegram` | Full tools including terminal |
| Discord | `damon-discord` | Full tools including terminal |
| WhatsApp | `damon-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `damon-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `damon-slack` | Full tools including terminal |
| Google Chat | `damon-google_chat` | Full tools including terminal |
| Signal | `damon-signal` | Full tools including terminal |
| SMS | `damon-sms` | Full tools including terminal |
| Email | `damon-email` | Full tools including terminal |
| Home Assistant | `damon-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `damon-mattermost` | Full tools including terminal |
| Matrix | `damon-matrix` | Full tools including terminal |
| DingTalk | `damon-dingtalk` | Full tools including terminal |
| Feishu/Lark | `damon-feishu` | Full tools including terminal |
| WeCom | `damon-wecom` | Full tools including terminal |
| WeCom Callback | `damon-wecom-callback` | Full tools including terminal |
| Weixin | `damon-weixin` | Full tools including terminal |
| BlueBubbles | `damon-bluebubbles` | Full tools including terminal |
| QQBot | `damon-qqbot` | Full tools including terminal |
| Yuanbao | `damon-yuanbao` | Full tools including terminal |
| Microsoft Teams | `damon-teams` | Full tools including terminal |
| API Server | `damon-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `damon-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.damon/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
