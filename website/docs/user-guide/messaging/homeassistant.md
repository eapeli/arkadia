...
Configure which events the agent sees in `~/.damon/config.yaml` under the Home Assistant platform's `extra` section:
...
The adapter reads credentials from `~/.damon/.env` (auto-merged at startup) or
