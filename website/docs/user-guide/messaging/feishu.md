...
Rules live in `~/.damon/feishu_comment_rules.json` (pairing grants in `~/.damon/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.damon/feishu_seen_message_ids.json`.
...
Feishu / Lark uses the `damon-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
