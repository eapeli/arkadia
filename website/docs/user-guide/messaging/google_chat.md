...
`~/.damon/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a damon restart)
...
- Principal: `damon-chat-bot@<your-project>.iam.gserviceaccount.com`
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/damon-chat-events`.
...
Add the Google Chat section to `~/.damon/.env`:
...
`~/.damon/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.damon/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.damon/google_chat_user_token.json` (if present from
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.damon/google_chat_user_tokens/<sanitized_email>.json` (filesystem
