...
```bash
# devtunnel (Microsoft)
devtunnel create damon-bot --allow-anonymous
devtunnel port create damon-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host damon-bot
...
Alternatively, configure via `~/.damon/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.damon/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create damon-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
...
- Store credentials in `~/.damon/.env` with permissions `600` (`chmod 600 ~/.damon/.env`)
