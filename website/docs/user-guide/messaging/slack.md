...
1. Generate the manifest:
   ...
   ```
   This writes `~/.damon/slack-manifest.json` and prints paste-in
   instructions.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `damon-socket` (the name doesn't matter)
...
[`COMMAND_REGISTRY`](https://github.com/NousResearch/damon-agent/blob/main/damon_cli/commands.py)
...
3. Paste the new contents of `~/.damon/slack-manifest.json`
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.damon/config.yaml`.
...
- Tokens should be stored in `~/.damon/.env` (file permissions `600`)
