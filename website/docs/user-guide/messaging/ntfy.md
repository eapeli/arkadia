...
- A topic name (any unique string — `damon-myname-2026` works fine)
...
Add these to `~/.damon/.env`:

```
NTFY_TOPIC=damon-myname-2026
NTFY_ALLOWED_USERS=damon-myname-2026
NTFY_HOME_CHANNEL=damon-myname-2026
```
...
- Or **pick a long, unguessable topic name** (`damon-7d4f9c8b-2026`) and treat it as the shared secret. This is the lightest setup but the topic name leaks via any logs or screenshots.
...
1. Pick a topic name: `damon-myname-2026`
2. On your phone: install the [ntfy app](https://ntfy.sh/docs/subscribe/phone/), tap **+**, enter `damon-myname-2026`
3. On the host:
   ```bash
   echo 'NTFY_TOPIC=damon-myname-2026' >> ~/.damon/.env
   echo 'NTFY_ALLOWED_USERS=damon-myname-2026' >> ~/.damon/.env
   damon gateway restart
   ```
