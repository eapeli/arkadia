...
4. Choose a **username** — this must be unique and end in `bot` (e.g., `my_damon_bot`)
...
Add the following to `~/.damon/.env`:
...
You can also set it manually in `~/.damon/.env`:
...
If you'd rather have the **agent itself** handle audio — for diarization, a custom transcription tool, or just archiving the recording — set `stt.enabled: false` in `~/.damon/config.yaml`:
...
```
[The user sent a voice message: /home/<user>/.damon/cache/audio/<hash>.ogg]
```
...
Add the URLs under `platforms.telegram.extra` in `~/.damon/config.yaml`:
...
grep -E "Using custom Telegram base_url|Using Telegram local_mode" ~/.damon/logs/gateway.log | tail
```
...
```bash
tail -f ~/.damon/logs/gateway.log | grep -iE "telegram|cache"
```

You should see a `[Telegram] Cached user voice at /home/<user>/.damon/cache/audio/...` line and **no** "too large" rejection. Combined with `stt.enabled: false` (above), the path to the original audio file then lands in the agent's inbound message for downstream processing.
...
Add topics under `platforms.telegram.extra.dm_topics` in `~/.damon/config.yaml`:
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.damon/config.yaml`:
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.damon/.env`. |
...
Configure the response timeout via `agent.clarify_timeout` in `~/.damon/config.yaml` (default `600` seconds). If you don't respond within the timeout, the agent unblocks with a sentinel message and adapts rather than hanging.
...
Configure in `~/.damon/config.yaml`:
