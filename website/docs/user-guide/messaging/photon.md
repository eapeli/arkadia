...
Runtime credentials are written to `~/.damon/.env`
...
(device token, dashboard project id) lives in `~/.damon/auth.json` under
...
**Pre-authorize specific numbers** (in `~/.damon/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.damon/.env`):
...
- **`sidecar deps : ✗ run damon photon install-sidecar`** — Node is
