...
5. Save the account credentials automatically to `~/.damon/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.damon/.env`:
...
- Tokens are saved per account+peer to `~/.damon/weixin/accounts/<account_id>.context-tokens.json`
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `pip install damon-agent[messaging]`. Alternatively, open the URL printed above the QR |
