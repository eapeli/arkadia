...
Discord behavior is controlled through two files: **`~/.damon/.env`** for credentials and env-level toggles, and **`~/.damon/config.yaml`** for structured settings. Environment variables always take precedence over config.yaml values when both are set.
...
The `discord` section in `~/.damon/config.yaml` mirrors the env vars above. Config.yaml settings are applied as defaults — if the equivalent env var is already set, the env var wins.
...
**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.damon/.env` and restart the gateway.
...
**Fix**: Set this in `~/.damon/config.yaml` and restart the gateway:
...
```bash
# ~/.damon/.env — works alongside or instead of DISCORD_ALLOWED_USERS
...
```bash
# ~/.damon/.env — env vars win over config.yaml
