...
**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage damon the same way you manage any other NixOS service.
:::
...
nix run github:NousResearch/damon-agent -- setup
nix run github:NousResearch/damon-agent -- chat

# Or install persistently
nix profile install github:NousResearch/damon-agent
hermes setup
hermes chat
```

After `nix profile install`, `hermes`, `damon-agent`, and `damon-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `hermes setup` walks you through provider selection, `hermes gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.damon/`.
...
```bash
nix profile install github:NousResearch/damon-agent#messaging
```

For all optional extras (voice, all providers, all platforms):

```bash
nix profile install github:NousResearch/damon-agent#full
```
...
```bash
git clone https://github.com/NousResearch/damon-agent.git
cd damon-agent
nix build
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    damon-agent.url = "github:NousResearch/damon-agent";
  };

  outputs = { nixpkgs, damon-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        damon-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
# configuration.nix
{ config, ... }: {
  services.damon-agent = {
    enable = true;
    settings.model.default = "anthropic/claude-sonnet-4";
    environmentFiles = [ config.sops.secrets."damon-env".path ];
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o damon /dev/stdin /var/lib/damon/env
```

```nix
services.damon-agent.environmentFiles = [ "/var/lib/damon/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `hermes` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `hermes` in your shell creates a separate `~/.damon/` directory.
:::
...
- For developers working on the damon codebase, set `HERMES_DEV=1` to bypass container routing and run the local checkout directly
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo damon chat` manually.
:::
...
systemctl status damon-agent

# Watch logs (Ctrl+C to stop)
journalctl -u damon-agent -f
...
```nix
services.damon-agent.configFile = /etc/damon/config.yaml;
```
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."damon-env".path ]` |
| Give the agent a personality | `${services.damon-agent.stateDir}/.damon/SOUL.md` | manage the file directly |
...
| Override the damon package | `package` | `inputs.damon-agent.packages.${system}.default.override { ... }` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Hermes reads this file on every startup, so changes take effect with a `systemctl restart damon-agent` — no container recreation needed.
...
```nix
{
  sops = {
    ...
    secrets."damon-env" = { format = "yaml"; };
  };

  services.damon-agent.environmentFiles = [
    config.sops.secrets."damon-env".path
  ];
}
```
...
```nix
{
  age.secrets.damon-env.file = ./secrets/damon-env.age;

  services.damon-agent.environmentFiles = [
    config.age.secrets.damon-env.path
  ];
}
```
...
{
  services.damon-agent = {
    authFile = config.sops.secrets."damon/auth.json".path;
...
The agent identity file is separate: Hermes loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.damon-agent.stateDir}/.damon/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.

```nix
{
  services.damon-agent.documents = {
...
```nix
{
  services.damon-agent.mcpServers = {
    filesystem = {
      command = "npx";
      args = [ "-y" "@modelcontextprotocol/server-filesystem" "/data/workspace" ];
    };
...
```nix
{
  services.damon-agent.mcpServers.remote-api = {
...
```nix
{
  services.damon-agent.mcpServers.my-oauth-server = {
...
```bash
# Container mode
docker exec -it damon-agent \
  damon mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u damon HERMES_HOME=/var/lib/damon/.hermes \
  damon mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
scp ~/.damon/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/damon/.damon/mcp-tokens/
...
```nix
{
  services.damon-agent.mcpServers.analysis = {
    command = "npx";
...
When damon runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:
...
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it damon-agent damon config set ...` is also blocked)
...
When container mode is enabled, damon runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-damon-agent-0.1.0  ──►  /nix/store/... (ro)
~/.hermes -> /var/lib/damon/.hermes       (symlink bridge, per hostUsers)
/var/lib/damon/                    ──►  /data/          (rw)
...
|---|---|---|---|---|
| `systemctl restart damon-agent` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the damon package itself do **not** trigger recreation.
...
If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/damon-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current damon package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.
...
For plugins that are just a source tree with `plugin.yaml` + `__init__.py` (e.g., [damon-lcm](https://github.com/stephenschoettler/damon-lcm)):

```nix
services.damon-agent.extraPlugins = [
...
For pip-packaged plugins that register via `[project.entry-points."damon_agent.plugins"]` (e.g., [rtk-hermes](https://github.com/ogallotti/rtk-hermes)):

```nix
services.damon-agent.extraPythonPackages = [
...
The package's `site-packages` is added to PYTHONPATH in the damon wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in damon-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.damon-agent.extraDependencyGroups = [ "messaging" ];
```
...
```nix
{
  inputs.damon-agent.url = "github:NousResearch/damon-agent";
  outputs = { damon-agent, nixpkgs, ... }: {
    nixpkgs.overlays = [ damon-agent.overlays.default ];
    # Then:
    #   pkgs.damon-agent.override { extraPythonPackages = [...]; }
    #   pkgs.damon-agent.override { extraDependencyGroups = [ "hindsight" ]; }
  };
}
```
...
```nix
services.damon-agent.settings.plugins.enabled = [
  "damon-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core damon dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
| Check | What it tests |
|---|---|
| `package-contents` | `hermes` and `damon-agent` binaries exist and `hermes version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `hermes --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `HERMES_MANAGED=true damon config set ...` prints the NixOS error |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the damon-agent service |
| `package` | `package` | `damon-agent` | The damon-agent package to use |
| `user` | `str` | `"damon"` | System user |
| `group` | `str` | `"damon"` | System group |
...
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the damon user's per-user profile so terminal commands, skills, and cron jobs all see them |
...
```
/var/lib/damon/                     # stateDir (owned by hermes:hermes, 0750)
├── .damon/                         # HERMES_HOME
...
cd /etc/nixos && nix flake update damon-agent
...
```bash
systemctl status damon-agent
docker ps -a --filter name=damon-agent
docker inspect damon-agent --format='{{.State.Status}}'
docker exec -it damon-agent bash
docker exec damon-agent readlink /data/current-package
docker exec damon-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop damon-agent
docker rm -f damon-agent
sudo rm /var/lib/damon/.container-identity
sudo systemctl start damon-agent
```
...
```bash
# Native mode
sudo -u damon cat /var/lib/damon/.damon/.env

# Container mode
docker exec damon-agent cat /data/.damon/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec damon-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u damon-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `hermes version` shows old version | Container not restarted | `systemctl restart damon-agent` |
| Permission denied on `/var/lib/hermes` | State dir is `0750 hermes:hermes` | Use `docker exec` or `sudo -u hermes` |
| `nix-collect-garbage` removed damon | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "damon-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
...
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart damon-agent` |
