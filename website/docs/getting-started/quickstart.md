...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `damon_cli/models.py` may change between releases) | `hermes model` → MiniMax (OAuth) |
...
- **Secrets and tokens** → `~/.damon/.env`
- **Non-secret settings** → `~/.damon/config.yaml`
...
# ~/.damon/damon-agent on Linux/macOS or %LOCALAPPDATA%\hermes\damon-agent on Windows):
...
Hermes ships with a catalog of bundled skills already installed in `~/.damon/skills/`. You can add more from the Skills Hub, or write your own.
...
(If you installed without `[all]`, run `cd ~/.damon/damon-agent && uv pip install -e ".[acp]"` first.)
