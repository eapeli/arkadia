...
hermes update            # runs pip install --upgrade damon-agent
```

Or manually:

```bash
pip install --upgrade damon-agent    # or: uv pip install --upgrade damon-agent
```
...
1. **Pairing-data snapshot** — a lightweight pre-update state snapshot is saved (covers `~/.damon/pairing/`, Feishu comment rules, and other state files that get modified at runtime). Recoverable via the snapshot restore flow described under [Snapshots and rollback](../user-guide/checkpoints-and-rollback.md), or by extracting the most recent quick-snapshot zip Hermes wrote next to your `~/.damon/` directory.
...
- All output is mirrored to `~/.damon/logs/update.log` while the update runs. If your terminal disappears, reconnect and inspect the log to see whether the update finished and whether the gateway restart succeeded:

```bash
tail -f ~/.damon/logs/update.log
```
...
Compare against the latest release at the [GitHub releases page](https://github.com/NousResearch/damon-agent/releases).
...
nix flake update damon-agent

# Or rebuild with the latest
nix profile upgrade damon-agent
```
...
The uninstaller gives you the option to keep your configuration files (`~/.damon/`) for a future reinstall.

### pip installs

```bash
pip uninstall damon-agent
...
```bash
hermes gateway stop
# Linux: systemctl --user disable damon-gateway
