...
Set your provider with `hermes model` or by editing `~/.damon/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
- [Use MCP with Hermes](../guides/use-mcp-with-hermes.md#wsl2-bridge-damon-in-wsl-to-windows-chrome)
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Hermes Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.damon/`.
...
**Solution:** Hermes auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.damon/config.yaml`:
...
:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.damon/.env` for conflicting entries.
:::
...
cat ~/.damon/logs/gateway.log | tail -50
```
...
- Check gateway logs: `cat ~/.damon/logs/gateway.log | tail -50`
...
Configure in `~/.damon/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "damon-agent[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
tmux new -s damon 'hermes gateway run'
# Reattach later: tmux attach -t hermes

# Option 3: Background via nohup
nohup damon gateway run > ~/.damon/logs/gateway.log 2>&1 &
```
...
cd ~/.damon/damon-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.damon/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
There is no hard limit. Each profile is just a directory under `~/.damon/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Hermes can route subagents to a different model automatically. Set this in `~/.damon/config.yaml`:
...
2. On the **source machine**, create a full backup:
   ...
   damon backup
   ```
   This creates a zip of your entire `~/.damon/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/damon-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/damon-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   damon import ~/damon-backup-<timestamp>.zip
   ```
...
```bash
rsync -av --exclude='damon-agent' ~/.damon/ newmachine:~/.damon/
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/damon-agent/issues)
