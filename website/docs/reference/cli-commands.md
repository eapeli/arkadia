...
| `--ignore-user-config` | Ignore `~/.damon/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
...
| `hermes secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.damon/.env`. |
...
| `hermes checkpoints` | Inspect / prune / clear `~/.damon/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
...
| `--ignore-user-config` | Ignore `~/.damon/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
...
Per-run overrides (no mutation to `~/.damon/config.yaml`):
...
# or:
HERMES_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 damon -z "…"
```
...
:::warning damon model vs /model — know the difference
...
:::tip WSL users
Use `hermes gateway run` instead of `hermes gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s damon 'hermes gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-damon-gateway-start-fails) for details.
:::
...
hermes slack manifest --write      # write to ~/.damon/slack-manifest.json
...
echo "message" | damon send --to <target>
hermes send --list [platform]
```

Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.damon/.env` + `~/.damon/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.
...
echo "RAM 92%" | damon send --to telegram:-1001234567890
...
Pull API keys from an external secret manager at process startup instead of storing them in `~/.damon/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Hermes venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.damon/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
Multi-profile, multi-project collaboration board. Each install can host many boards (one per project, repo, or domain); each board is a standalone queue with its own SQLite DB and dispatcher scope. New installs start with one board called `default`, whose DB is `~/.damon/kanban.db` for back-compat; additional boards live at `~/.damon/kanban/boards/<slug>/kanban.db`. The gateway-embedded dispatcher sweeps every board per tick.
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.damon/kanban/current`). |
...
Board resolution order (highest precedence first): `--board <slug>` flag → `HERMES_KANBAN_BOARD` env var → `~/.damon/kanban/current` file → `default`.

All actions are also available as a slash command in the gateway (`/kanban …`), with the same argument surface — including `boards` subcommands and the `--board` flag.

For the full design — comparison with Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise, eight collaboration patterns, four user stories, concurrency correctness proof — see `docs/damon-kanban-v1-spec.pdf` in the repository or the [Kanban user guide](/user-guide/features/kanban).
...
Subscriptions persist to `~/.damon/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart.
...
Create a zip archive of your Hermes configuration, skills, sessions, and data. The backup excludes the damon-agent codebase itself.

| Option | Description |
|--------|-------------|
| `-o`, `--output <path>` | Output path for the zip file (default: `~/damon-backup-<timestamp>.zip`). |
...
- The `damon-agent` code itself (this is a user-data backup, not a repo snapshot).

### Examples

```bash
hermes backup                           # Full backup to ~/damon-backup-*.zip
...
Inspect and manage the shadow git store at `~/.damon/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.
...
### Examples
```bash
hermes import ~/damon-backup-20260423.zip           # Prompts before overwriting existing config
hermes import ~/damon-backup-20260423.zip --force   # Overwrite without prompting
```
...
View, tail, and filter Hermes log files. All logs are stored in `~/.damon/logs/` (or `<profile>/logs/` for non-default profiles).
...
Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.damon/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.damon/skill-bundles/` and report added/removed bundles |
...
| `backup` | Take a manual tar.gz snapshot of `~/.damon/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.damon/skills/` from a snapshot (defaults to newest) |
...
Inspect shell-script hooks declared in `~/.damon/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.damon/shell-hooks-allowlist.json`.
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Hermes](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-damon-as-an-mcp-server).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Hermes Plugin](../guides/build-a-damon-plugin.md).
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.damon/` (by default a single restore-point archive is written to `~/.damon/backups/pre-migration-*.zip` before apply; restorable with `hermes import`). |
...
Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. Requires `pip install damon-agent[web]` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`pip install 'damon-agent[web,pty]'`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](/user-guide/features/web-dashboard) for full documentation.
...
Pulls the latest `damon-agent` code and reinstalls dependencies in your venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install.

**pip installs:** `hermes update` detects pip-based installations automatically — it queries PyPI for the latest release and runs `pip install --upgrade damon-agent` instead of `git pull`. PyPI releases track tagged versions (major/minor releases), not every commit on `main`. Use `--check` to see if a newer PyPI release is available without installing.
...
- **Pairing data snapshot.** Even when `--backup` is off, `hermes update` takes a lightweight snapshot of `~/.damon/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `hermes backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `hermes.service` warning.** If Hermes detects a pre-rename `hermes.service` systemd unit (instead of the current `damon-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
