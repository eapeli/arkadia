...
```
# Layer 1: Agent Identity (from ~/.damon/SOUL.md)
...
`SOUL.md` lives at `~/.damon/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_damon_md(cwd_path)       # 1. .hermes.md / HERMES.md (walks to git root)
...
- `~/.damon/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.damon/MEMORY.md` and `~/.damon/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
