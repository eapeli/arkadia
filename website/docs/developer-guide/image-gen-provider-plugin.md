...
2. **User** — `~/.damon/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
...
A bundled plugin is complete at this point. User plugins at `~/.damon/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `hermes plugins enable <name>`).
...
`my_backend_imggen_package` must expose a top-level `register` function. See [Distribute via pip](/guides/build-a-damon-plugin#distribute-via-pip) in the general plugin guide for the full setup.
