...
- Building a custom/local tool without modifying Hermes core? Start with [Build a Hermes Plugin](../guides/build-a-damon-plugin.md)
...
`$HERMES_HOME/damon-agent` (usually `~/.damon/damon-agent`). That keeps your
...
system `python3 -m damon_cli.main` can pick up unrelated system Python
packages.
...
- **Profile-safe paths**: Never hardcode `~/.hermes` — use `get_damon_home()` from `damon_constants` for code paths and `display_damon_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/damon-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
- Use [GitHub Issues](https://github.com/NousResearch/damon-agent/issues)
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/damon-agent/blob/main/LICENSE).
