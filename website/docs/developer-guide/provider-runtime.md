...
- `damon_cli/runtime_provider.py` — credential resolution, `_resolve_custom_runtime()`
- `damon_cli/auth.py` — provider registry, `resolve_provider()`
- `damon_cli/model_switch.py` — shared `/model` switch pipeline (CLI + gateway)
...
- `tests/damon_cli/test_fallback_cmd.py` — the `/fallback` CLI command
