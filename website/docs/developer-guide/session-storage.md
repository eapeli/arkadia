...
Hermes Agent uses a SQLite database (`~/.damon/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `damon_state.py`


## Architecture Overview

```
~/.damon/state.db (SQLite, WAL mode)
...
Multiple damon processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.damon/state.db
...
Default path: `~/.damon/state.db`

This is derived from `damon_constants.get_damon_home()` which resolves to
`~/.damon/` by default, or the value of `HERMES_HOME` environment variable.
