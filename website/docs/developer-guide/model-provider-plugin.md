...
|---|---|---|
| Credential resolution | `damon_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `damon_cli/main.py` | Accepts `acme-inference` |
| `hermes model` picker | `damon_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `hermes doctor` | `damon_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `hermes setup` | `damon_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `damon_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.damon/plugins/model-providers/gmi/__init__.py`:
...
```bash
export HERMES_HOME=/tmp/damon-plugin-test
...
acme-inference = "acme_damon_plugin:register"
```

…where `acme_damon_plugin:register` is a function that calls `register_provider(profile)`. The general PluginManager picks up entry-point plugins during `discover_and_load()`. For `kind: model-provider` pip plugins, you still need to declare the kind in your manifest (or rely on the source-text heuristic).

See [Building a Hermes Plugin](/guides/build-a-damon-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a Hermes Plugin](/guides/build-a-damon-plugin) — general plugin authoring
