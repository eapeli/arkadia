...
```text
hermes acp / damon-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'damon-agent[acp]==<version>' damon-acp`, pointed at the `damon-agent` PyPI release.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["damon-acp"])
...
- `acp_adapter/auth.py`
- `damon_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Hermes provider/credentials. It also always advertises a terminal setup auth method (`damon-setup`, args `--setup`) so first-run registry clients can open Hermes' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.damon/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `damon-acp` toolset definition
- `damon_cli/main.py` — `hermes acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `damon-acp` script
