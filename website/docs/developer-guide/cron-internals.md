...
| `damon_cli/cron.py` | CLI `hermes cron` subcommands |
...
Jobs are stored in `~/.damon/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
```python
# ~/.damon/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.damon/cron/output/` |
