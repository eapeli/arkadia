...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          damon_home (str): Active HERMES_HOME path. Use for storage.
        """
...
| `save_config(values, damon_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
```python
def save_config(self, values: dict, damon_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(damon_home) / "my-provider.json"
...
All storage paths **must** use the `damon_home` kwarg from `initialize()`, not hardcoded `~/.hermes`:

```python
# CORRECT — profile-scoped
from damon_constants import get_damon_home
data_dir = get_damon_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.damon/my-provider").expanduser()
```
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: damon my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the damon my-provider argparse tree.
