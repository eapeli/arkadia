...
After the first login, credentials are stored under `~/.damon/auth.json` and refreshed automatically before they expire.
...
See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/NousResearch/damon-agent/issues/26923).
...
3. xAI redirects back to Hermes and the tokens are saved to `~/.damon/auth.json`.
...
After login, `~/.damon/config.yaml` will contain:
...
This is **not** a stale-token problem — re-running `hermes model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/damon-agent/issues/26847)).
