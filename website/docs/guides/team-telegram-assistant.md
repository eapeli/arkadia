...
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.damon/.env`
...
2. **Send `/newbot`** — BotFather will ask you two things:
   ...
   - **Username** — must end in `bot` (e.g., `myteam_damon_bot`)
...
sudo damon gateway install --system   # Linux only: boot-time system service
```
...
# View live logs
journalctl --user -u damon-gateway -f
...
```bash
hermes gateway stop && damon gateway start
```
...
3. **You approve it** on the server:
   ```bash
   damon pairing approve telegram XKGH5N7P
   ```
...
**Option 2:** Set it manually in `~/.damon/.env`:
...
Control how much detail the bot shows when using tools. In `~/.damon/config.yaml`:
...
Customize how the bot communicates by editing `~/.damon/SOUL.md`:
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u damon-gateway` (Linux) or `~/.damon/logs/gateway.log` (macOS) |
| Cron job output | `~/.damon/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.damon/cron/jobs.json` |
| Pairing data | `~/.damon/pairing/` |
| Session history | `~/.damon/sessions/` |
