...
[Download the Hermes Desktop installer](https://damon-agent.nousresearch.com/) for macOS or Windows, then open it. On first launch it finishes setting itself up (usually under a minute).
...
Prefer to review first? Download [`install.sh`](https://damon-agent.nousresearch.com/install.sh), inspect it, then run it.
...
- **[Run Hermes Agent with Nous Portal](/guides/run-damon-with-nous-portal)** — Full Portal walkthrough: models, Tool Gateway, and verification
