...
- Piping any tool's output anywhere (`make | damon send --to slack:#builds`)
...
3. **Piped stdin** — `echo hi | damon send --to telegram`
...
`~/.damon/.env` and `~/.damon/config.yaml`. It's a standalone subprocess
...
The listing is built from `~/.damon/channel_directory.json`, which the
