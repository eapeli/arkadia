...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.damon/` (by default a single restore-point archive is written before apply, under `~/.damon/backups/pre-migration-*.zip`; restorable with `hermes import`). |
...
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.damon/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.damon/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.damon/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.damon/memories/MEMORY.md` | All daily files merged into main memory. |
...
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.damon/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.damon/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.damon/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.damon/skills/openclaw-imports/` |
...
| Provider API keys | `models.providers.*.apiKey` | `~/.damon/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |
...
| TTS assets | `~/.damon/tts/` (file copy) |
...
These are saved to `~/.damon/migration/openclaw/<timestamp>/archive/` for manual review:
...
2. **Review archived files** — anything in `~/.damon/migration/openclaw/<timestamp>/archive/` needs manual attention.
...
5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart damon-gateway`
...
Imported skills land in `~/.damon/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.
