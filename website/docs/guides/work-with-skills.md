...
What happens:
1. The skill directory is copied to `~/.damon/skills/`
...
For how to ship skills in your own plugin, see [Build a Hermes Plugin → Bundle skills](/guides/build-a-damon-plugin#bundle-skills).
...
```bash
mkdir -p ~/.damon/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.damon/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
The skill appears automatically — no registration needed. Drop it in `~/.damon/skills/` and it's live.
...
**Use categories.** Organize skills into subdirectories (`~/.damon/skills/devops/`, `~/.damon/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.
