...
Hermes ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.damon/scripts/`. First create the script:

```bash
mkdir -p ~/.damon/scripts
cat > ~/.damon/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec damon teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.damon/scripts/maintain-teams-subscriptions.sh
```
...
Create `/etc/systemd/system/damon-teams-pipeline-maintain.service`:
...
Type=oneshot
User=hermes
EnvironmentFile=/etc/damon/env
ExecStart=/usr/local/bin/hermes teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/damon-teams-pipeline-maintain.timer`:
...
sudo systemctl enable --now damon-teams-pipeline-maintain.timer
systemctl list-timers damon-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/hermes teams-pipeline maintain-subscriptions >> /var/log/damon/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.damon/.env` at the top of a wrapper script that crontab calls.
