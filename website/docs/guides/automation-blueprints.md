...
hermes cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/damon-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/damon-agent --state open --json number,title,labels,author,createdAt --limit 30
...
hermes cron create "0 9 * * 1" \
  "Scan the NousResearch/damon-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/damon-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (damon_cli/commands.py, damon_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (damon_cli/config.py) — may need docs/user-guide/configuration.md update
...
hermes cron create "0 6 * * *" \
  "Run a dependency security audit on the damon-agent project.

1. cd ~/.damon/damon-agent && source .venv/bin/activate
...
```python title="~/.damon/scripts/check-uptime.py"
...
hermes cron create "every 30m" \
  ...
  --script ~/.damon/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
hermes cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the damon-agent codebase.
