...
3. Sets `model.provider: nous` in `~/.damon/config.yaml`
...
For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.damon/auth.json` → no token sharing across users. This is the right boundary.
...
If a model is genuinely unavailable, [open an issue](https://github.com/NousResearch/damon-agent/issues) — most gaps are routing config we can update.
