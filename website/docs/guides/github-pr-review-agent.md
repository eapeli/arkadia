...
- **Gateway running** for cron jobs:
  ```bash
  damon gateway install   # Install as a service
  # or
  damon gateway           # Run in foreground
  ```
...
Use `deliver: "local"` to save reviews to `~/.damon/cron/output/`. Great for testing before wiring up notifications.
:::
...
```
Run: gh pr list --repo NousResearch/damon-agent --state open --limit 3
```
...
Run: gh pr diff 3888 --repo NousResearch/damon-agent
```
...
```bash
mkdir -p ~/.damon/skills/code-review
```

Create `~/.damon/skills/code-review/SKILL.md`:
...
- **[Build a Plugin](/guides/build-a-damon-plugin)** — wrap the review logic into a shareable plugin
