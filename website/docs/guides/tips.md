...
Want Hermes to have a stable default voice? Edit `~/.damon/SOUL.md` (or `$HERMES_HOME/SOUL.md` if you use a custom Hermes home). Hermes now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.
...
:::tip
On messaging platforms, sessions auto-reset after idle time (default: 24 hours) or daily at 4 AM. Adjust per-platform in `~/.damon/config.yaml` if you need longer sessions.
:::
...
TERMINAL_DOCKER_IMAGE=damon-sandbox:latest
```
