...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.damon/.env` at the end.
...
3. **Description:** `damon-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.damon/.env`:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.damon/.env` with the new value.
