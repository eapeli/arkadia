...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.damon/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.damon/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.damon/.env` |
...
| `local` | Write access to `~/.damon/cron/output/` |
...
2. `~/.damon/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
...
```bash
ls ~/.damon/scripts/your-script.py   # Must exist
hermes cron edit <job_id> --script ~/.damon/scripts/your-script.py
```
...
Jobs are stored in `~/.damon/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.damon/cron/jobs.json
chmod 600 ~/.damon/cron/jobs.json   # Your user should own it
```
...
2. Check `~/.damon/logs/agent.log` for scheduler messages and `~/.damon/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/damon-agent](https://github.com/NousResearch/damon-agent) with:
