...
After the first login, credentials are stored under `~/.damon/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.damon/.env
```
...
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.damon/auth.json` under the `minimax-oauth` key.
...
After login, `~/.damon/config.yaml` will contain entries similar to:
