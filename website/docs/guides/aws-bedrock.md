...
- **boto3** — install with `pip install damon-agent[bedrock]`
...
pip install damon-agent[bedrock]
...
After running `hermes model`, your `~/.damon/config.yaml` will contain:
...
**[sample-damon-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-damon-agent-on-aws-with-bedrock)** — creates VPC, IAM role, EC2 instance, and configures Bedrock automatically. Deploy in any region with one click.
