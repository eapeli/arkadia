...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.damon/hooks/<name>/` |
...
```bash
mkdir -p ~/.damon/plugins/calculator
cd ~/.damon/plugins/calculator
```
...
```bash
HERMES_PLUGINS_DEBUG=1 damon plugins list
```
...
The same logs are always written to `~/.damon/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:
...
- **Wrong directory layout** — must be `~/.damon/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.damon/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.damon/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.damon/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
auditable in `~/.damon/logs/agent.log`. Plugins load after built-in
...
def _my_command(args):
    """Handler for damon my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: damon my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for damon my-plugin."""
...
Hermes has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.damon/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
Model Context Protocol (MCP) servers register their own tools into Hermes without any Python plugin. Declare them in `~/.damon/config.yaml`:
...
Drop a manifest + handler into `~/.damon/hooks/<name>/`:
...
```toml
# pyproject.toml
[project.entry-points."damon_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install damon-plugin-calculator
# Plugin auto-discovered on next damon startup
```
