            ...
            while api_call_count < self.max_iterations:
                ...
                try:
                    ...
                    try:
                        ...
                        _is_rate_limit_error = None
                        resolve_provider_client = None
                    if (
                        _is_rate_limit_error is not None
                        and _is_rate_limit_error(e)
                        and not getattr(self, "_fallback_attempted", False)
                        and resolve_provider_client is not None
                    ):
                        self._fallback_attempted = True
                        try:
                            fallback_client, _ = resolve_provider_client(
                                "auto", model=self.model, raw_codex=True
                            )
                        except Exception:
                            fallback_client = None
                        if fallback_client is not None:
                            self.client = fallback_client
                            print("♻️ Rate-limit detectado: trocando de provedor para tentar continuar.")
                            continue
