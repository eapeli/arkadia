"""CLI entry point for the damon-agent ACP adapter.

Loads environment variables from ``~/.damon/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from DAMON_HOME (default ``~/.damon``)."""
    from damon_cli.env_loader import load_damon_dotenv

    damon_home = get_damon_home()
    loaded = load_damon_dotenv(damon_home=damon_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", damon_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="damon-acp",
        description="Run Damon Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Damon version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Damon provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.damon/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import DamonACPAgent  # noqa: F401

    print("Damon ACP check OK")


def _run_setup() -> None:
    from damon_cli.main import main as damon_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "damon", "model"]
        damon_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``damon postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting damon-agent ACP adapter")
    ...
    import acp
    from .server import DamonACPAgent
