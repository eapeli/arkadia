"""ACP auth helpers — detect and advertise Damon authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "damon-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Damon runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Damon can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Damon.
    ...
    not have Damon provider credentials configured yet, so Damon always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Damon using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Damon provider",
            description=(
                "Open Damon' interactive model/provider setup in a terminal. "
                "Use this when Damon has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
