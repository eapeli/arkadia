"""ACP session manager — maps ACP sessions to Damon AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.damon/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Damon itself is running in WSL.

    Windows ACP clients can launch ``damon acp`` inside WSL while still sending
    ...
    """
    from damon_pkg.constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Damon from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["damon-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Damon agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Damon AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Damon runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.damon/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``DAMON_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_damon_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=damon_home / "state.db")
            return self._db_instance
        except Exception:
