...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/damon-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/damon-agent.git"
...
#   code at /usr/local/lib/damon-agent, command at /usr/local/bin/hermes,
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.damon/damon-agent"
            echo "                   default (root, Linux): /usr/local/lib/damon-agent"
            echo "  --damon-home PATH  Data directory (default: ~/.hermes, or \$HERMES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hermes installs the code under"
            echo "  /usr/local/lib/damon-agent and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HERMES_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HERMES_HOME/damon-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs damon setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HERMES_HOME/damon-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HERMES_HOME/damon-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/damon-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HERMES_HOME/damon-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$HERMES_HOME/damon-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HERMES_HOME/damon-agent/.git" ]; then
            INSTALL_DIR="$HERMES_HOME/damon-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/damon-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/damon-agent"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://damon-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # The runtime update path (damon_cli/managed_uv.py) looks in the same
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/damon-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/damon-uv-installer.$$.sh")"
...
install_node() {
    ...
    log_info "Extracting to ~/.damon/node/..."
    ...
    # Place into ~/.damon/node/ and symlink binaries into the same bin dir
    # the damon command uses (get_command_link_dir): /usr/local/bin for root
    ...
    log_success "Node.js $installed_ver installed to ~/.damon/node/"
    HAS_NODE=true
}
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                local stash_name
                stash_name="damon-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
...
setup_path() {
    log_info "Setting up damon command..."

    if [ "$USE_VENV" = true ]; then
        HERMES_BIN="$INSTALL_DIR/venv/bin/hermes"
    else
        HERMES_BIN="$(which damon 2>/dev/null || echo "")"
    ...
    # Create a user-facing shim for the damon command.
...
EOF
    ...
    log_success "Installed damon launcher → $command_link_display_dir/hermes"
    ...
    # Export for current session so damon works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create .env at ~/.damon/.env (top level, easy to find)
    if [ ! -f "$HERMES_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HERMES_HOME/.env"
            log_success "Created ~/.damon/.env from template"
        else
            touch "$HERMES_HOME/.env"
            log_success "Created ~/.damon/.env"
        fi
    else
        log_info "~/.damon/.env already exists, keeping it"
    fi
    ...
    # Create config.yaml at ~/.damon/config.yaml (top level, easy to find)
    if [ ! -f "$HERMES_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HERMES_HOME/config.yaml"
            log_success "Created ~/.damon/config.yaml from template"
        fi
    else
        log_info "~/.damon/config.yaml already exists, keeping it"
    fi
...
-->
SOUL_EOF
        log_success "Created ~/.damon/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.damon/"

    # Seed bundled skills into ~/.damon/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        ...
        log_info "Syncing bundled skills to ~/.damon/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.damon/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HERMES_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$HERMES_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.damon/skills/"
            fi
        fi
    fi
}
...
run_setup_wizard() {
    ...
    # Run damon setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m damon_cli.main setup < /dev/tty
    else
        python -m damon_cli.main setup < /dev/tty
    fi
}
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                HERMES_CMD="$(get_damon_command_path)"
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: damon gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: damon gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.damon/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: damon gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: damon gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'damon' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'damon' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'damon' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
postinstall_mode() {
    ...
    HERMES_CMD="$(command -v damon 2>/dev/null || echo "")"
    if [ -n "$HERMES_CMD" ]; then
        log_info "Running damon setup..."
        "$HERMES_CMD" setup
    else
        log_warn "hermes command not found on PATH"
        log_info "Try: python -m damon_cli.main setup"
    fi
}
