"""Windows UTF-8 bootstrap for Damon entry points.
...
should be imported at the very top of every Damon entry point
(``damon``, ``damon-agent``, ``damon-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import damon_bootstrap``
# (or ``from damon_bootstrap import apply_windows_utf8_bootstrap``) at
