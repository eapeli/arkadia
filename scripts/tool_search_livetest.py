...
# Force-isolate the test environment BEFORE any damon imports.
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.damon/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.damon/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="damon_ts_live_"))
    damon_home = home_dir / ".hermes"
    damon_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, damon_home / "auth.json")
    ...
    if real_env_file.exists():
        shutil.copy(real_env_file, damon_home / ".env")
        ...
        load_damon_dotenv(damon_home=str(Path.home() / ".hermes"))
    ...
    (damon_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return damon_home
