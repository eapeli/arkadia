"""
Damon MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Damon-specific extra)

Usage:
    damon mcp serve
    damon mcp serve --verbose
...
logger = logging.getLogger("damon.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using DAMON_HOME."""
    try:
        from damon_pkg.constants import get_damon_home
        return get_damon_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("DAMON_HOME", Path.home() / ".damon")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from damon_pkg.constants import get_damon_home
        directory_file = get_damon_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("DAMON_HOME", Path.home() / ".damon")
...
class EventBridge:
    ...
    This is the Damon equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from damon_pkg.constants import get_damon_home
            db_file = get_damon_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("DAMON_HOME", Path.home() / ".damon")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Damon MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "damon",
        instructions=(
            "Damon Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Damon MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
