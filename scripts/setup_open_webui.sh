...
# - ensures ~/.damon/.env has API server settings
...
HERMES_ENV_FILE="${HERMES_ENV_FILE:-$HOME/.damon/.env}"
...
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.damon/logs/openwebui.log
StandardError=append:%h/.damon/logs/openwebui.error.log
...
main() {
  ...
  damon gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    ...
    nohup damon gateway run >/dev/null 2>&1 &
    sleep 6
  fi
