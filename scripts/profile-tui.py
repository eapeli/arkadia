...
~30 Hz (matching xterm key-repeat), summarizes ~/.damon/perf.log on exit.
...
bypasses the damon_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  HERMES_PERF_LOG     (default ~/.damon/perf.log)
...
try:
    ...
    def get_damon_home() -> Path:  # type: ignore[misc]
...
DEFAULT_LOG = Path(os.environ.get("HERMES_PERF_LOG", str(get_damon_home() / "perf.log")))
DEFAULT_STATE_DB = get_damon_home() / "state.db"
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "damon-ink" / "src"
