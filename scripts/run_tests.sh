#!/usr/bin/env bash
# Canonical test runner for damon-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.damon/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.damon/damon-agent/venv"; do
...
if [ -f "$HOME/.damon/pytest_live_guard.py" ]; then
