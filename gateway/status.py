...
The PID file lives at ``{DAMON_HOME}/gateway.pid``.  DAMON_HOME defaults to
``~/.damon`` but can be overridden via the environment variable.  This means
separate DAMON_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting DAMON_HOME."""
    home = get_damon_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("DAMON_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "damon" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Damon gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different DAMON_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``damon.service`` + ``damon-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``damon gateway stop`` on Windows would be misclassified as an
