...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_damon_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.damon/config.yaml (primary user-facing config)
    3. ~/.damon/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_damon_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``damon config set gateway.streaming.*``
