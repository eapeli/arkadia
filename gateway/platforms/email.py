"""
Email platform adapter for the Damon gateway.

Allows users to interact with Damon by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "damon-agent" "version" "{_damon_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Damon Agent")
        ...
        msg_id = f"<damon-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
