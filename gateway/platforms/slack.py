...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_damon_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Damon does not currently need to route them into the agent loop.
            ...
            # /model work directly without /damon prefix). A single regex
            ...
            # manifest (see `damon slack manifest`). In Socket Mode, Slack
            ...
            from damon_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/damon$")

            @self._app.command(_slash_pattern)
            async def handle_damon_command(ack, command):
            ...
            for _action_id in (
                "damon_approve_once",
                "damon_approve_session",
                "damon_approve_always",
                "damon_deny",
            ):
            ...
            for _action_id in (
                "damon_confirm_once",
                "damon_confirm_always",
                "damon_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Damon handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Damon session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "damon_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "damon_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "damon_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "damon_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "damon_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "damon_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "damon_confirm_once": "once",
            "damon_confirm_always": "always",
            "damon_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "damon_approve_once": "once",
            "damon_approve_session": "session",
            "damon_approve_always": "always",
            "damon_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/damon <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/damon
        ...
        if slash_name in {"damon", ""}:
            # Legacy /damon <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from damon_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/damon   ``).
        ...
        # "Running /cmd…" ack shown by handle_damon_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/damon <question>" must produce public replies so
