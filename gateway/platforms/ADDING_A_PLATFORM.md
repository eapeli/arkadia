...
There are two ways to add a platform to the Damon gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.damon/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Damon code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `damon gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `damon_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Damon core.
...
"damon-your-platform": {
    ...
    "tools": _DAMON_CORE_TOOLS,
    "includes": []
},
```

And add it to the `damon-gateway` composite:

```python
"damon-gateway": {
    "includes": [..., "damon-your-platform"]
}
```
...
## 12. Status Display (`damon_cli/status.py`)
...
## 13. Gateway Setup Wizard (`damon_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ damon_cli/ toolsets.py \
  --include="*.py" -l | sort -u
