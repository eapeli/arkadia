...
# Audio file extensions Damon recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Damon' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Damon-created Telegram private-chat
...
from damon_pkg.constants import get_default_damon_root, get_damon_dir, get_damon_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.damon/.env manually."
)
...
# Default location: {DAMON_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_damon_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; DamonAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_damon_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_damon_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_damon_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_damon_dir("cache/screenshots", "browser_screenshots")
_DAMON_HOME = get_damon_home()
_DAMON_ROOT = get_default_damon_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "DAMON_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "DAMON_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "DAMON_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "DAMON_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _DAMON_HOME / "image_cache",
    _DAMON_HOME / "audio_cache",
    _DAMON_HOME / "video_cache",
    _DAMON_HOME / "document_cache",
    _DAMON_HOME / "browser_screenshots",
    ...
    _DAMON_HOME / "cache" / "images",
    _DAMON_HOME / "cache" / "audio",
    _DAMON_HOME / "cache" / "videos",
    _DAMON_HOME / "cache" / "documents",
    _DAMON_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Damon profile and shared Damon root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for damon_root in (_DAMON_HOME, _DAMON_ROOT):
        denied.append(damon_root / ".env")
        denied.append(damon_root / "auth.json")
        denied.append(damon_root / "credentials")
        denied.append(damon_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Damon
    secrets (``~/.damon/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``DAMON_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Damon-managed cache, under an operator-allowlisted root
    (``DAMON_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.damon/.env,
    # ~/.damon/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?damon\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+damon[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Damon commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("DAMON_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "DAMON_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "DAMON_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Damon sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.damon/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from damon_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          DAMON_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          DAMON_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          DAMON_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("DAMON_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("DAMON_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("DAMON_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _damon_run_generation is set on
