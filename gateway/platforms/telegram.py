...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("DAMON_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "DAMON_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "DAMON_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Damon-created private-chat topic lanes are marked with
    ...
    # older PTB/clients. Streaming edits stay on Damon' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Damon must skip rich delivery up front and use the legacy MarkdownV2
    ...
    def prefers_fresh_final_streaming(
        ...
        instead of Damon' current MarkdownV2 edit path.
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `damon update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Damon or OpenClaw instance is running "
            "with this token, then restart the gateway with 'damon gateway restart'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Damon process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("DAMON_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("DAMON_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("DAMON_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("DAMON_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("DAMON_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("DAMON_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            try:
                ...
                from damon_cli.commands import telegram_menu_commands
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``damon update --gateway``
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``damon model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from damon_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from damon_cli.models import PROVIDER_GROUPS
    ...
    # Scripts live in ~/.damon/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".damon" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@damon_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Damon bot profiles. A message like
