"""Relay/connector support package for the damon gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``damon gateway enroll`` writes them to
    ``~/.damon/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``DAMON_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def self_provision_if_managed() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.damon/.env``
    ...
    try:
        from damon_cli.auth import resolve_nous_access_token
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'damon'}"
