...
def test_main_wrapper_preserves_docker_workdir() -> None:
    ...
    assert "_damon_orig_cwd" in text, (
    ...
    # first exec / if-block.
    activate_idx = text.index("/opt/damon/.venv/bin/activate")
    restore_idx = text.index('cd "$_damon_orig_cwd"')
    exec_idx = text.index("if [ $# -eq 0 ]")
    assert activate_idx < restore_idx < exec_idx, (
        "cd $_damon_orig_cwd must appear after venv activation and "
...
def test_dashboard_run_resets_home_before_dropping_privileges() -> None:
    ...
    assert "exec s6-setuidgid damon hermes dashboard" in text
