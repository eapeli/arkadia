...
* ``--ignore-user-config`` → skip ``~/.damon/config.yaml`` in ``load_cli_config()``
...
class TestIgnoreUserConfigEnvGate:
    ...
    When the env var is set, user config at ``<damon_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._damon_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_damon_home", tmp_path)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``damon_cli/main.py`` must set
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real damon parser accepts both flags."""
