...
from damon_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
from damon_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import damon_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "damon_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
