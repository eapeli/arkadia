...
``damon_cli.main`` skips eager plugin discovery at argparse-setup time
...
def _live_subcommand_names() -> set[str]:
    ...
    built-in-only set.
    """
    from damon_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["damon", "-w", "--tui"], None),
        (["damon", "version"], "version"),
        (["damon", "--tui", "chat"], "chat"),
        (["damon", "-w", "logs"], "logs"),
        (["damon", "chat", "hello world"], "chat"),
        (["damon", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["damon", "-m", "gpt5", "chat"], "chat"),
        (["damon", "--model", "gpt5", "chat", "hi"], "chat"),
        (["damon", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["damon", "-z", "hello world"], None),
        (["damon", "-z", "hello", "chat"], "chat"),
        (["damon", "--model=gpt5", "chat"], "chat"),     # inline form
        (["damon", "--", "chat"], "chat"),               # -- terminator
        (["damon", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["damon", "some-plugin-cmd"], "some-plugin-cmd"),
        (["damon", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["damon"],                          # bare → chat
        ["damon", "--help"],                # top-level help
        ["damon", "-h"],
        ["damon", "version"],               # known built-in
        ["damon", "logs"],
        ["damon", "gateway", "run"],
        ["damon", "--tui"],
        ["damon", "-w", "--tui"],
        ["damon", "chat", "hi"],
        ["damon", "help"],                  # accepted built-in-ish
        ["damon", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["damon", "meet", "join"],          # potential google_meet plugin
        ["damon", "honcho", "status"],      # potential memory plugin
        ["damon", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    assert not missing_from_declaration, (
        ...
        f"damon_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
