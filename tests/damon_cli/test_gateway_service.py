...
import damon_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "damon-gateway.service"
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``/tmp/pytest-of-.../damon_test`` HERMES_HOME — silently breaking
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        ``/tmp/damon-e2e-41264`` carries none of the pytest markers but
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="HERMES_HOME=/tmp/damon-e2e-41264"\n'
            "WorkingDirectory=/tmp/damon-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="HERMES_HOME=/var/tmp/damon-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="HERMES_HOME={tmp_path}/damon-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None

    def test_detects_tmp_home_in_launchd_plist(self):
        plist = (
            "<dict>\n  <key>HERMES_HOME</key>\n"
            "  <string>/tmp/damon-e2e-99999</string>\n</dict>\n"
        )
        assert (
    ...
    def test_accepts_unit_without_damon_home(self):
        unit = "[Service]\nExecStart=/usr/bin/python -m damon_cli.main gateway run\n"
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_damon_home_for_target_user", lambda home: "/home/alice/.hermes")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup damon gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_damon_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: damon gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup damon gateway" in out
...
class TestSystemUnitHermesHome:
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HERMES_HOME", "/root/.damon/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HERMES_HOME=/home/alice/.damon/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_damon_home(self, monkeypatch):
        # Custom HERMES_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HERMES_HOME", "/opt/damon-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HERMES_HOME=/opt/damon-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's HERMES_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        damon_home = str(gateway_cli.get_damon_home().resolve())
        assert f'HERMES_HOME={damon_home}' in unit


class TestHermesHomeForTargetUser:
    """Unit tests for _damon_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("HERMES_HOME", raising=False)

        result = gateway_cli._damon_home_for_target_user("/home/alice")
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._damon_home_for_target_user("/home/alice")
        assert result == "/home/alice/.damon/profiles/coder"
...
class TestProfileArg:
    ...
    def test_default_damon_home_returns_empty(self, tmp_path, monkeypatch):
        ...
        result = gateway_cli._profile_arg(str(damon_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.damon/profiles/mybot should return '--profile mybot'."""
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        ...
        custom_home = tmp_path / "custom" / "damon"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.damon/profiles/mybot/subdir should NOT match — too deep."""
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_damon_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_damon_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".hermes" / "damon-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".hermes" / "damon-agent")
    ...
    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        ...
        original = str(tmp_path / "alice" / ".hermes" / "damon-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".hermes" / "damon-agent"
        ...
        monkeypatch.setattr(gateway_cli, "get_damon_home", lambda: root_home / ".hermes")
        ...
        assert "WorkingDirectory=/home/alice/.damon/damon-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "damon-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "damon-gateway"])
...
class TestLegacyHermesUnitDetection:
    """Tests for _find_legacy_damon_units / has_legacy_damon_units.
    ...
    renamed to ``damon-gateway.service``. After PR #5646 (signal recovery
    ...
    (``damon-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Hermes Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m damon_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_damon_service_in_user_scope(self, tmp_path, monkeypatch):
        ...
        results = gateway_cli._find_legacy_damon_units()
        ...
        assert gateway_cli.has_legacy_damon_units() is True

    def test_detects_legacy_damon_service_in_system_scope(self, tmp_path, monkeypatch):
    ...
    def test_ignores_profile_unit_damon_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``damon-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "damon-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "damon-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "damon-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_damon_units() is False

    def test_ignores_unrelated_damon_service(self, tmp_path, monkeypatch):
        ...
        (user_dir / "hermes.service").write_text(
            ...
            "ExecStart=/opt/other-damon/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_damon_units() == []
        assert gateway_cli.has_legacy_damon_units() is False
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m damon_cli.main gateway run
          - python path/to/damon_cli/main.py gateway run
          - damon gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m damon_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/damon/damon_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/hermes gateway run --replace",
            "ExecStart=/venv/bin/python /opt/damon/gateway/run.py",
        ]
...
class TestRemoveLegacyHermesUnits:
    """Tests for remove_legacy_damon_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_damon_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_damon_units(
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo damon gateway migrate-legacy" in out
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (damon-gateway-coder.service)
        ...
        profile_unit = user_dir / "damon-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "damon-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_damon_units(interactive=True)
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        if project_root is None:
            import damon_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "damon_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_damon_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "damon_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_damon_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_damon_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_damon_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "damon-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "damon-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "damon-gateway.service",
        )
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "damon-gateway")
        ...
        assert "sudo systemctl start damon-gateway" in out
        assert "sudo damon gateway uninstall --system" in out
        assert "hermes gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart damon-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop damon-gateway" in out
...
class TestServiceWorkingDirIsStable:
    ...
    def test_stable_working_dir_uses_damon_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_damon_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_damon_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_damon_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_damon_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_damon_home(self, tmp_path, monkeypatch):
        import re
