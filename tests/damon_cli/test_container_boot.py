"""Tests for damon_cli.container_boot — the cont-init.d-time
...
from damon_cli.container_boot import (
    ReconcileAction,
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under damon_home/profiles/<name>/."""
    p = damon_home / "profiles" / name
...
def _seed_default_root(
    ...
    *,
    ...
    if state is not None:
        (damon_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (damon_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (damon_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        damon_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_reconcile_log_rotates_when_size_exceeded(
    ...
    from damon_cli import container_boot
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        damon_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ("gateway", "run"),
        ("/init", "/opt/damon/docker/main-wrapper.sh", "gateway", "run"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/init", "/opt/damon/docker/main-wrapper.sh", "gateway", "run", "--no-supervise"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/init", "/opt/damon/docker/main-wrapper.sh", "dashboard"),
        (
            "/init",
            "/opt/damon/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "127.0.0.1",
            "--no-open",
        ...
        ("/init", "/opt/damon/docker/main-wrapper.sh", "damon", "dashboard"),
    ],
)
def test_is_dashboard_container_true_for_dashboard_argv(
    ...
    from damon_cli.container_boot import _is_dashboard_container
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("gateway", "run"),
        ("/init", "/opt/damon/docker/main-wrapper.sh", "gateway", "run"),
        ("/init", "/opt/damon/docker/main-wrapper.sh", "damon", "gateway", "run"),
        ("chat",),
...
def test_main_skips_reconcile_in_dashboard_container(
    ...
    monkeypatch.setattr(
        ...
        lambda: ("/init", "/opt/damon/docker/main-wrapper.sh", "dashboard"),
    )
...
def test_main_reconciles_in_gateway_container(
    ...
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/damon/docker/main-wrapper.sh", "gateway", "run"),
    )
