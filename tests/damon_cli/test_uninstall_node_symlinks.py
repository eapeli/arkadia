"""Tests for damon_cli.uninstall.remove_node_symlinks.
...
def _make_damon_node(damon_home: Path) -> Path:
    """Create a fake $HERMES_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = damon_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_damon_node(fake_home):
    damon_home = fake_home / ".hermes"
    node_bin = _make_damon_node(damon_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(damon_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(damon_home) == []


def test_removes_dangling_symlink_into_damon_node(fake_home):
