"""Tests for the Hermes plugin system (damon_cli.plugins)."""
...
from damon_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<damon_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<damon_home>/plugins/``; we derive
    ``<damon_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        ...
        import os
        damon_home_str = os.environ.get("HERMES_HOME")
        if damon_home_str:
            damon_home = Path(damon_home_str)
        else:
            damon_home = base.parent
        damon_home.mkdir(parents=True, exist_ok=True)
        cfg_path = damon_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.damon/plugins/ are discovered."""
        plugins_dir = tmp_path / "damon_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "damon_test"))
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("damon_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        damon_home = tmp_path / "damon_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under damon_plugins.<name>."""
        plugins_dir = tmp_path / "damon_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "damon_test"))

        # Clean up any prior namespace module
        sys.modules.pop("damon_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "damon_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="damon_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import damon_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("damon_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        damon_home = tmp_path / "damon_test"
        plugins_dir = damon_home / "plugins"
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("damon_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("damon_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("damon_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("damon_cli.plugins.PluginContext.dispatch_tool.__module__", "damon_cli.plugins"):
...
class TestPluginDebugLogging:
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        from damon_cli import plugins as plugins_mod
