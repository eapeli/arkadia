...
class TestGatewayLifecyclePattern:
    ...
    def test_damon_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "systemctl restart damon-gateway",
        "systemctl stop damon-gateway.service",
        "systemctl start damon-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill damon gateway process",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_damon_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade damon then run damon gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify damon gateway stop/restart refuse when _HERMES_GATEWAY=1."""
