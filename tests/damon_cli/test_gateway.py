...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /damon-gateway.service"`` and runs ``systemctl --user daemon-reload``
...
def test_run_gateway_refuses_root_in_official_docker(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "opt" / "damon"
    ...
    assert "/opt/damon/docker/entrypoint.sh" in out
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("damon_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("damon_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "damon-gateway.service"
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "damon-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "damon-gateway.service"
    system_unit = tmp_path / "system" / "damon-gateway.service"
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo damon gateway install --system --run-as-user alice" in out
    assert "sudo damon gateway start --system" in out
...
def test_scan_gateway_pids_detects_windows_damon_exe_case_variants(monkeypatch):
    monkeypatch.setattr(gateway, "is_windows", lambda: True)
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "damon_cli.gateway"
