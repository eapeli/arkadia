...
import damon_cli.doctor as doctor
import damon_cli.gateway as gateway_cli
from damon_cli import doctor as doctor_mod
from damon_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "HERMES_HOME", damon_home)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_damon_home(self, tmp_path, provider=""):
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_damon_home(tmp_path, provider)
...
def test_run_doctor_accepts_damon_provider_ids_that_catalog_aliases(
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from damon_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("damon_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "damon_cli.doctor", raising=False)
        from damon_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        (damon_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "HERMES_HOME", damon_home)
        monkeypatch.setattr(doctor_mod, "get_damon_home", lambda: damon_home)
        ...
        return buf.getvalue(), damon_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, damon_home = self._run_config_section(
        ...
        assert "HERMES_MAX_ITERATIONS=90" in (damon_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        env_after = (damon_home / ".env").read_text(encoding="utf-8")
