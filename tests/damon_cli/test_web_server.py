"""Tests for damon_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_damon_home):
    ...
    real bundled plugins — kanban, damon-achievements, model providers
    ...
    from damon_cli import web_server
...
class TestSessionTokenInjection:
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import damon_cli.web_server as ws
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_damon_home):
        ...
        import damon_state
        from damon_pkg.constants import get_damon_home
        from damon_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(damon_state, "DEFAULT_DB_PATH", get_damon_home() / "state.db")
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        import damon_cli.web_server as web_server
    ...
    def test_dashboard_update_capability_detects_generic_container(self, monkeypatch):
        import damon_pkg.constants as damon_constants
        import damon_cli.web_server as web_server

        monkeypatch.setattr(damon_constants, "is_container", lambda: True)
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("damon_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from damon_cli.web_server import app
    ...
    def test_update_damon_returns_docker_guidance_without_spawning(self, monkeypatch):
        import damon_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn damon update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_damon_action", fail_spawn)
        web_server._ACTION_PROCS.pop("damon-update", None)
        web_server._ACTION_RESULTS.pop("damon-update", None)

        resp = self.client.post("/api/damon/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "damon-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/damon-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/damon-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/damon-agent:latest" in line for line in status_data["lines"])

    def test_update_damon_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import damon_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn damon update")
    ...
    def test_update_damon_spawns_on_non_docker_install(self, monkeypatch):
        import damon_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_damon_action", fake_spawn)
        web_server._ACTION_PROCS.pop("damon-update", None)
        web_server._ACTION_RESULTS.pop("damon-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "damon-update"}
        assert calls == [(["update"], "damon-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("damon-update", None)
        web_server._ACTION_RESULTS.pop("damon-update", None)
        web_server._ACTION_PROCS["damon-update"] = proc

        resp = self.client.get("/api/actions/damon-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "damon-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["damon-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from damon_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from damon_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "damon_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_damon_action", fake_spawn)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from damon_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from damon_cli.config import save_env_value
        from damon_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://damon-agent.nousresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from damon_cli.config import load_config, load_env
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "damon_pair123_bot",
                "deep_link": "https://t.me/newbot/HermesSetupBot/damon_pair123_bot",
                "qr_payload": "https://t.me/newbot/HermesSetupBot/damon_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "damon_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/HermesSetupBot/damon_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/HermesSetupBot/damon_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "damon_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_damon_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "damon_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/HermesSetupBot/damon_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/HermesSetupBot/damon_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "damon_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_damon_action", fail_spawn_action)
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "damon_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/HermesSetupBot/damon_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/HermesSetupBot/damon_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "damon_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "damon_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/HermesSetupBot/damon_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/HermesSetupBot/damon_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "damon_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/HermesSetupBot/damon_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/HermesSetupBot/damon_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/damon_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import damon_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "damon-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from damon_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from damon_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "damon-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        import damon_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from damon_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from damon_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_damon_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from damon_pkg.constants import get_damon_home
        get_damon_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from damon_pkg.constants import get_damon_home
        import damon_cli.profiles as profiles_mod

        damon_home = get_damon_home()
        damon_home.mkdir(parents=True, exist_ok=True)
        (damon_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = damon_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_damon_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_damon_for_default_profile(self):
        from damon_pkg.constants import get_damon_home
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/damon/bin/hermes")
        ...
        assert wrapper_path.read_text() == '#!/bin/sh\nexec /opt/damon/bin/hermes -p writer "$@"\n'
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_damon_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_damon_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_damon_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_damon_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_damon_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_damon_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_damon_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_damon_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_damon_home() / "profiles" / "builder"
        token = set_damon_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_damon_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import damon_cli.skills_config as skills_config
        import damon_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import damon_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from damon_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from damon_cli.web_server import _denormalize_config_from_web
        from damon_cli.config import save_config
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from damon_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.damon/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``damon_cli/web_server.py``).
    ...
    def _setup_test_client(self, monkeypatch, _isolate_damon_home):
        ...
        monkeypatch.setattr(
            damon_state, "DEFAULT_DB_PATH", get_damon_home() / "state.db"
        )
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in damon_cli/web_server.py."
        )
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_damon_home, _install_example_plugin):
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, damon-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (damon-achievements is loaded by default).
        resp = self.client.get("/api/plugins/damon-achievements/overview")
        assert resp.status_code == 401
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import damon_cli.main as main_mod
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_damon_home
    ):
        import damon_cli.main as main_mod
        ...
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/damon-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/damon-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"damon-ws-ok" in buf:
                    break
            assert b"damon-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from damon_cli.pty_bridge import PtyUnavailableError
        ...
        import damon_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from damon_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import damon_cli.main as cli_main
    import damon_cli.web_server as ws
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_damon_home):
        import threading
    ...
    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_damon_home):
        import threading
        import cron.scheduler as sched
