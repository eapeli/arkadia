"""Comprehensive tests for damon_cli.profiles module.
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.damon/profiles)
    * HERMES_HOME  -> tmp_path/.hermes  (so get_damon_home() agrees)
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["damon", "test", "tmp", "root", "sudo"])
...
class TestGetProfileDir:
    ...
    def test_default_returns_damon_home(self, profile_env):
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes damon-agent, .worktrees,
        ...
        (default_home / "damon-agent" / ".git").mkdir(parents=True)
        (default_home / "damon-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "damon-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "damon-agent").exists()
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("damon_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("damon_cli.profiles._cleanup_gateway_service"), \
             patch("damon_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
...
class TestGetActiveProfileName:
    ...
    def test_default_damon_home_returns_default(self, profile_env):
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("damon")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("damon_cli.profiles.shutil.which", lambda name: "/opt/damon/bin/hermes")
        from damon_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/damon/bin/hermes -p mybot" in content
    ...
    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from damon_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from damon_cli.profiles import remove_wrapper_script
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from damon_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from damon_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from damon_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("damon_cli.profiles.check_alias_collision", return_value="skip"):
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert cfg["hosts"]["damon_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["damon_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "hermes.ssi_health": {"workspace": "damon", "enabled": True}
            }
        }))
        ...
        assert "hermes.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["damon_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["damon_heimdall"]["workspace"] == "damon"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                ...
                "damon_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        assert cfg["hosts"]["damon_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("damon-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".damon_history",
        ...
        excluded_prefixes = [
            "default/damon-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.damon_history",
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_damon_home()."""
    ...
    def test_default_damon_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_damon_home()
    ...
    def test_default_damon_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_damon_home() returns HERMES_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (HERMES_HOME under ~/.hermes), profiles root is still ~/.damon/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        ...
        from damon_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from damon_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".hermes"
