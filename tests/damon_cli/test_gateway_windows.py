"""Tests for damon_cli.gateway_windows."""

from pathlib import Path

import pytest

import damon_cli.gateway as gateway
import damon_cli.gateway_windows as gateway_windows
import damon_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda damon_home: "")
    monkeypatch.setattr("damon_cli.config.get_damon_home", lambda: str(damon_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "damon_cli.main"]
    assert cwd == str(damon_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_damon_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr("damon_cli.config.get_damon_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("damon_cli.config.get_damon_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_damon_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    damon_home = tmp_path / "damon-home"
    damon_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(damon_home.resolve()))}" in content
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Hermes\\damon-agent\\venv\\Scripts\\python.exe",
        r"C:\\Hermes\\damon-agent",
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("damon_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("damon_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    monkeypatch.setattr("damon_cli.gateway._get_restart_drain_timeout", lambda: 1.0)
