"""Tests for the Command Installation check in damon doctor."""
...
import damon_cli.doctor as doctor_mod
...
def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    ...
    damon_bin = venv_bin_dir / "damon"
    damon_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    damon_bin.chmod(0o755)
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, damon_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "damon"
        cmd_link.symlink_to(damon_bin)
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "damon"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == damon_bin.resolve()
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        damon_bin = project / ".venv" / "bin" / "damon"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "damon"
        cmd_link.symlink_to(damon_bin)
