"""Tests for Codex auth — tokens stored in Hermes auth store (~/.damon/auth.json)."""
...
def _setup_damon_auth(damon_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_damon_auth(damon_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("HERMES_HOME", str(damon_home))
    ...
    monkeypatch.setattr("damon_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    damon_home = tmp_path / "damon"
    _setup_damon_auth(damon_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("HERMES_HOME", str(damon_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((damon_home / "auth.json").read_text())
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    _save_codex_tokens({"access_token": "damon-at", "refresh_token": "damon-rt"})
    ...
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "damon-at"


def test_resolve_returns_damon_auth_store_source(tmp_path, monkeypatch):
    damon_home = tmp_path / "damon"
    _setup_damon_auth(damon_home)
    monkeypatch.setenv("HERMES_HOME", str(damon_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run damon auth".
    """
    from damon_cli.auth import (
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from damon_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from damon_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "damon_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("damon_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("damon_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch damon_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("damon_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
