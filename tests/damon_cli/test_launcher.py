...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `damon_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "damon"
    called = []

    fake_main_module = types.ModuleType("damon_cli.main")

    def fake_main():
        called.append("damon_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "damon_cli.main", fake_main_module)
