...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.damon/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.hermes            (has its own auth.json fixture)
    * Profile     -> tmp_path/.damon/profiles/coder   (active, HERMES_HOME points here)
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from damon_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    damon_home = tmp_path / "classic"
    damon_home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(damon_home))

    _write(damon_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    ...
    _write(damon_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from damon_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: HERMES_HOME is set to a custom path that is NOT under
    # ~/.damon/profiles/ — get_default_damon_root() returns HERMES_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from damon_cli.auth import read_credential_pool, write_credential_pool
