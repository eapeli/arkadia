...
Complements tests/damon_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/damon_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/damon_cli/test_kanban_db.py.
    monkeypatch.setenv("HERMES_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.damon/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_parse_duration_accepts_formats():
    from damon_cli.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: damon_cli.main doesn't propagate sub-command exit codes
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "damon_cli.main", "kanban",
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    ...
    r = _sp.run(
        [_sys.executable, "-m", "damon_cli.main", "kanban",
...
def test_build_worker_context_renders_author_with_safe_framing(kanban_home):
    ...
    (e.g. "damon-system", "operator") can't be misread as a system
    directive above the comment body. Defense-in-depth — see #22452."""
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="t", assignee="worker")
        kb.add_comment(conn, tid, author="damon-system", body="some note")
        ctx = kb.build_worker_context(conn, tid)

        # No bold-author rendering anywhere in the context.
        assert "**damon-system**" not in ctx
        # Explicit provenance prefix is present.
        assert "comment from worker `damon-system` at " in ctx
...
def test_default_spawn_auto_loads_kanban_worker_skill(kanban_home, monkeypatch):
    ...
    damon subprocess (which would hang trying to call an LLM).
    """
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from damon_cli import kanban as kb_cli
...
def test_gateway_dispatcher_watcher_respects_config_flag_off(monkeypatch):
    ...
    import damon_cli.config as _cfg_mod
