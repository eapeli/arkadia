...
When HERMES_HOME is set to the damon root (e.g. systemd hardcodes
...
def _run_apply_profile_override(
    tmp_path, monkeypatch, *, damon_home: str | None, active_profile: str | None,
    ...
    or None if unset.
    """
    damon_root = tmp_path / ".hermes"
    damon_root.mkdir(parents=True, exist_ok=True)

    if active_profile is not None:
        (damon_root / "active_profile").write_text(active_profile)

    if active_profile and active_profile != "default":
        (damon_root / "profiles" / active_profile).mkdir(parents=True, exist_ok=True)

    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    if damon_home is not None:
        monkeypatch.setenv("HERMES_HOME", damon_home)
    else:
        monkeypatch.delenv("HERMES_HOME", raising=False)

    monkeypatch.setattr(sys, "argv", argv or ["damon", "gateway", "start"])

    from damon_cli.main import _apply_profile_override
...
class TestApplyProfileOverrideHermesHomeGuard:
    ...
    Verifies that HERMES_HOME pointing to the damon root does NOT suppress
    the active_profile check, while HERMES_HOME already pointing to a
    profile directory IS trusted as-is.
    """

    def test_damon_home_at_root_with_active_profile_is_redirected(
        self, tmp_path, monkeypatch
    ):
        """HERMES_HOME=/root/.hermes + active_profile=coder must redirect
        HERMES_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets HERMES_HOME to the damon root
    ...
    def test_damon_home_already_profile_dir_is_trusted(self, tmp_path, monkeypatch):
        ...
        """
        damon_root = tmp_path / ".hermes"
        profile_dir = damon_root / "profiles" / "coder"
        profile_dir.mkdir(parents=True, exist_ok=True)

        (damon_root / "active_profile").write_text("other")

        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(profile_dir))
        monkeypatch.setattr(sys, "argv", ["damon", "gateway", "start"])
    ...
    def test_damon_home_unset_reads_active_profile(self, tmp_path, monkeypatch):
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        user_home = tmp_path / "home" / "damon"
        profile_dir = user_home / ".hermes" / "profiles" / "elias"
        profile_dir.mkdir(parents=True, exist_ok=True)
        (root_home / ".hermes").mkdir(parents=True, exist_ok=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SUDO_USER", "damon")
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setattr(os, "geteuid", lambda: 0, raising=False)
        monkeypatch.setattr(sys, "argv", ["damon", "-p", "elias", "gateway", "install", "--system"])
        ...
        assert os.environ.get("HERMES_HOME") == str(profile_dir)
        assert sys.argv == ["damon", "gateway", "install", "--system"]

    def test_damon_home_unset_default_profile_no_redirect(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setattr(sys, "argv", ["damon", "gateway", "start"])
        (damon_root / "active_profile").write_text("default")
    ...
    def test_profile_after_chat_subcommand_is_still_consumed(self, tmp_path, monkeypatch):
        ...
        result = _run_apply_profile_override(
            ...
            damon_home=None,
            active_profile="coder",
            argv=["damon", "chat", "-p", "coder", "-q", "hello"],
        )

        assert result is not None
        assert result.endswith("coder")
        assert sys.argv == ["damon", "chat", "-q", "hello"]
    ...
    def test_top_level_profile_after_value_flag_is_consumed(self, tmp_path, monkeypatch):
        ...
        result = _run_apply_profile_override(
            ...
            damon_home=None,
            active_profile="coder",
            argv=["damon", "-m", "gpt-5", "--profile", "coder", "chat"],
        )

        assert result is not None
        assert result.endswith("coder")
        assert sys.argv == ["damon", "-m", "gpt-5", "chat"]
    ...
    def test_top_level_profile_after_continue_flag_is_consumed(self, tmp_path, monkeypatch):
        ...
        result = _run_apply_profile_override(
            ...
            damon_home=None,
            active_profile="coder",
            argv=["damon", "--continue", "--profile", "coder"],
        )

        assert result is not None
        assert result.endswith("coder")
        assert sys.argv == ["damon", "--continue"]
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        damon_root = tmp_path / ".hermes"
        damon_root.mkdir(parents=True, exist_ok=True)
        (damon_root / "active_profile").write_text("briefer")
        (damon_root / "profiles" / "briefer").mkdir(parents=True, exist_ok=True)
        ...
        monkeypatch.setenv("HERMES_HOME", str(damon_root))
        monkeypatch.setenv("HERMES_S6_SUPERVISED_CHILD", "1")
        monkeypatch.setattr(sys, "argv", ["damon", "gateway", "run"])

        from damon_cli.main import _apply_profile_override
        _apply_profile_override()

        assert os.environ.get("HERMES_HOME") == str(damon_root), (
    ...
    def test_supervised_named_profile_flag_still_wins(self, tmp_path, monkeypatch):
        ...
        (damon_root / "active_profile").write_text("briefer")
        (damon_root / "profiles" / "briefer").mkdir(parents=True, exist_ok=True)
        (damon_root / "profiles" / "coder").mkdir(parents=True, exist_ok=True)

        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setenv("HERMES_S6_SUPERVISED_CHILD", "1")
        monkeypatch.setattr(sys, "argv", ["damon", "-p", "coder", "gateway", "run"])
