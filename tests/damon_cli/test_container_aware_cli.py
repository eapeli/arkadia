...
def container_env(tmp_path, monkeypatch):
    ...
    container_mode = damon_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=damon-agent\n"
        "exec_user=hermes\n"
        "damon_bin=/data/current-package/bin/hermes\n"
    )
    return damon_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("damon_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "damon-agent"
    assert info["exec_user"] == "damon"
    assert info["damon_bin"] == "/data/current-package/bin/hermes"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("damon_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_damon_dev(container_env, monkeypatch):
...
def test_get_container_exec_info_not_skipped_when_damon_dev_zero(container_env, monkeypatch):
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        damon_home = Path(tmpdir) / ".hermes"
        damon_home.mkdir()
        (damon_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("damon_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_damon_home": lambda: damon_home}), \
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        ...
        "exec_user=myuser\n"
        "damon_bin=/opt/damon/bin/hermes\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "damon-custom"
    assert info["exec_user"] == "myuser"
    assert info["damon_bin"] == "/opt/damon/bin/hermes"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from damon_cli.main import _exec_in_container
