...
prefix (e.g. ``mission-control.tilos.com/damon/*`` -> local Caddy ->
...
honour it too:
  ...
  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/damon/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/damon/auth/callback`` and the user gets 404.
...
from damon_cli.dashboard_auth import clear_providers, register_provider
from tests.damon_cli.conftest_dashboard_auth import StubAuthProvider
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        ...
        assert r.headers["location"].startswith("/damon/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/damon/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/damon/auth/callback`` — the
        ...
        assert parsed.path == "/damon/auth/callback", (
...
class TestPublicUrlOverride:
    ...
    def patch_config(self, monkeypatch):
        """Replace ``damon_cli.config.load_config`` with a stub
        ...
        def _set(public_url) -> None:
            ...
            monkeypatch.setattr(
                "damon_cli.config.load_config", lambda: cfg
            )
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://example.com/damon/auth/callback"
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        from damon_cli.dashboard_auth import prefix as prefix_mod
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        pkce = next(c for c in cookies if "damon_session_pkce" in c)
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-damon_session_pkce=")
        ]
    ...
    def test_pkce_cookie_uses_host_prefix_when_direct(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Host-damon_session_pkce=")
        ]
        assert pkce_candidates, (
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from damon_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("damon_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        pkce_set = next(
            ...
            if "damon_session_pkce" in c
        )
        # Parse "__Secure-damon_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-damon_session_pkce=value"
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-damon_session_at=")
        ]
        assert at_cookies, (
