"""Tests for damon_cli/completion.py — shell completion script generation."""
...
from damon_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real damon structure."""
    p = argparse.ArgumentParser(prog="damon")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_damon_completion()" in out
        assert "complete -F _damon_completion hermes" in out
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_damon_profiles'" in out
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c damon -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from damon_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_damon_profiles()" in out
        assert 'profiles_dir="$HOME/.damon/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        ...
        out = generate_bash(_make_parser())
        # The profile case should have _damon_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_damon_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _damon_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_damon_profiles()" in out
        assert "$HOME/.damon/profiles" in out
    ...
    def test_fish_has_profiles_helper(self):
        out = generate_fish(_make_parser())
        assert "__damon_profiles" in out
        assert "$HOME/.damon/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__damon_profiles" in out
        count = out.count("(__damon_profiles)")
