...
def fresh_home(tmp_path, monkeypatch):
    ...
    # Also reset damon_constants cache so get_default_damon_root() re-reads.
    try:
        import damon_pkg.constants as damon_constants
        damon_constants._cached_default_damon_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "damon-agent", "proj_1", "a",
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "damon_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
