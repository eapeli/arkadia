"""Tests for damon_cli configuration management."""
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from damon_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="damon_cli.config"):
                config = load_config()
            ...
            # stands out at damon startup before logging is configured)
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from damon_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
class TestEnvWriteDenylist:
    ...
    def _damon_home(self, tmp_path, monkeypatch):
    ...
    def test_damon_integration_keys_still_writable(self, allowed_key):
