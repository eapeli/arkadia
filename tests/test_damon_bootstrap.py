"""Tests for damon_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Hermes entry point
(hermes, damon-agent, damon-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hermes entry point imports damon_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported damon_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("damon_bootstrap", None)
    import damon_bootstrap  # noqa: WPS433
    return damon_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Hermes entry point must import damon_bootstrap as its
    ...
    # import damon_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "damon_cli/main.py",   # damon CLI (console_script)
        "run_agent.py",          # damon-agent (console_script)
        "acp_adapter/entry.py",  # damon-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import damon_bootstrap' and that
        ...
        ``damon_bootstrap`` as a top-level module) leaves hermes
        ...
        """
        # Resolve relative to the damon-agent repo root.  Tests live
        # at tests/test_damon_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # damon start even when damon_bootstrap hasn't been
        ...
        assert first_import_name == "damon_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'damon_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import damon_bootstrap' line to be the first import."
        )
