...
def fake_hermes(tmp_path, monkeypatch):
    ...
    the damon-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setattr(fs, "_damon_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_hermes):
        ...
        assert "damon-security" in result["error"]
        # File untouched.
    ...
    def test_non_damon_path_unaffected(self, fake_hermes, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        assert "not found in active profile 'damon-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.damon/profiles/<name>/."""
        # Don't set HERMES_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_damon_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_damon_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hermes):
        """When active profile is 'damon-security', the prompt warns
