...
and the shared slash-command handlers (damon_cli/write_approval_commands.py).
...
@pytest.fixture
def damon_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="damon_wa_test_")
...
def _set_approval(subsystem, enabled):
    import damon_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(damon_home):
...
def test_invalid_subsystem_is_off(damon_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(damon_home):
...
def test_memory_gate_on_no_interactive_stages(damon_home):
...
def test_memory_gate_on_then_apply(damon_home):
...
def test_skill_gate_off_allows_create(damon_home):
...
def test_skill_gate_on_always_stages(damon_home):
...
def test_skill_gate_on_then_apply_writes_file(damon_home):
...
def test_skill_create_diff_is_full_content(damon_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(damon_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(damon_home):
...
def test_handle_approve_all(damon_home):
    from damon_cli.write_approval_commands import handle_pending_subcommand
...
def test_handle_reject(damon_home):
    from damon_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_approval_on(damon_home):
    from damon_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(damon_home):
    from damon_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_handle_mode_alias_still_works(damon_home):
...
def test_handle_approval_invalid(damon_home):
    from damon_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(damon_home):
    from damon_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(damon_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(damon_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(damon_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(damon_home, monkeypatch):
...
def test_skills_never_prompt_inline_even_with_callback(damon_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(damon_home):
