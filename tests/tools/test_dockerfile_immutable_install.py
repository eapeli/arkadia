...
def test_dockerfile_makes_opt_damon_root_owned_and_non_writable() -> None:
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "HERMES_TUI_DIR=/opt/damon/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_hermes() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+hermes:hermes\s+/opt/damon/\.venv",
        r"chown\s+-R\s+hermes:hermes\s+/opt/damon/ui-tui",
        r"chown\s+-R\s+hermes:hermes\s+/opt/damon/gateway",
        r"chown\s+-R\s+hermes:hermes\s+/opt/damon/node_modules",
    )
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/damon/.install_method) first; baking it at build time keeps the
    ...
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/damon/.install_method" in text
