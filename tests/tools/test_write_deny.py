...
class TestWriteDenyExactPaths:
    ...
    def test_damon_env(self):
        # ``.env`` under the active HERMES_HOME (profile-aware, not just
        # ``~/.hermes``) must be write-denied. The hermetic test conftest
        # points HERMES_HOME at a tempdir — resolve via get_damon_home()
        # to match the denylist.
        from damon_pkg.constants import get_damon_home
        path = str(get_damon_home() / ".env")
        assert _is_write_denied(path) is True

    def test_damon_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.damon/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "damon_root"
        ...
        from damon_pkg.constants import get_damon_home, get_default_damon_root
        assert get_damon_home() == profile_home
        assert get_default_damon_root() == root
...
class TestWriteAllowed:
    ...
    def test_damon_control_files_requested_writable(self):
        from damon_pkg.constants import get_damon_home
