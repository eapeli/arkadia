...
class TestScanMemoryContent:
    ...
    def test_damon_config_mod_blocked(self):
        result = _scan_memory_content("edit .damon/config.yaml to change settings")
        assert "Blocked" in result
        assert "damon_config_mod" in result
        result = _scan_memory_content("update .damon/SOUL.md with new personality")
        assert "Blocked" in result
        assert "damon_config_mod" in result
    ...
    def test_damon_config_mod_no_false_positives(self):
        """Merely mentioning damon config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .damon/config.yaml for settings") is None
        assert _scan_memory_content("Read .damon/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .damon/config.yaml file contains runtime options") is None
