...
guidance, while still allowing:
  ...
  - root start (id -u == 0)
  - `--user <damon-uid>` (the supported non-root start, #34648 / #34837)
...
root, and the baked image dirs are owned by the damon build UID, so an
...
PUID/PGID aliases), which remaps the damon user and chowns the volume.

The guard fires only when the current UID is neither root NOR the damon UID,
so the #34648 `--user 10000:10000` case (pinning to the damon UID itself) is
unaffected.
...
def test_guard_present_and_mentions_remediation(path: Path) -> None:
    ...
    # Must check non-root AND non-damon-uid (so --user 10000:10000 is allowed).
...
def _run_guard(text: str, *, cur_uid: int, damon_uid: int = 10000) -> subprocess.CompletedProcess:
    ...
    with tempfile.TemporaryDirectory() as d:
        script = (
            "set -e\n"
            # Stub `id`: `id -u` -> cur_uid; `id -u hermes` -> damon_uid.
            f'id() {{ if [ "$2" = damon ]; then echo {damon_uid}; else echo {cur_uid}; fi; }}\n'
            + block
...
def test_arbitrary_user_uid_is_rejected() -> None:
    ...
    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):
        proc = _run_guard(text, cur_uid=1000, damon_uid=10000)
...
def test_root_start_passes() -> None:
    ...
    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):
        proc = _run_guard(text, cur_uid=0, damon_uid=10000)
        assert proc.returncode == 0, proc.stderr
        assert "GUARD_PASSED" in proc.stdout


def test_user_pinned_to_damon_uid_passes() -> None:
    """`--user 10000:10000` (the damon UID itself) is the supported non-root
    start from #34648 / #34837 and must NOT be blocked."""
    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):
        proc = _run_guard(text, cur_uid=10000, damon_uid=10000)
        assert proc.returncode == 0, proc.stderr
        assert "GUARD_PASSED" in proc.stdout


def test_user_pinned_to_remapped_damon_uid_passes() -> None:
    """After a HERMES_UID remap the damon UID is e.g. 4242; a container pinned
    to that same UID must still pass (cur_uid == damon_uid)."""
    for text in (_read(STAGE2_HOOK), _read(MAIN_WRAPPER)):
        proc = _run_guard(text, cur_uid=4242, damon_uid=4242)
        assert proc.returncode == 0, proc.stderr
        assert "GUARD_PASSED" in proc.stdout
