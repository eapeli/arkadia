...
UDS socket lifecycle, damon_tools generation, timeout enforcement,
...
class TestHermesToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_damon_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)

    def test_generates_subset(self):
        src = generate_damon_tools_module(["terminal", "web_search"])
    ...
    def test_empty_list_generates_nothing(self):
        src = generate_damon_tools_module([])
    ...
    def test_non_allowed_tools_ignored(self):
        src = generate_damon_tools_module(["vision_analyze", "terminal"])
        self.assertIn("def terminal(", src)
        self.assertNotIn("def vision_analyze(", src)

    def test_rpc_infrastructure_present(self):
        src = generate_damon_tools_module(["terminal"])
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_damon_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"damon_rpc\")", src)
        self.assertNotIn('os.environ.get("HERMES_RPC_DIR", "/tmp/damon_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_damon_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/damon_exec_", mkdir_cmd)
        self.assertIn("HERMES_RPC_DIR=/data/data/com.termux/files/usr/tmp/damon_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/damon_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/damon_exec_", mkdir_cmd)
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import damon_pkg.constants as damon_constants; print(damon_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("damon_constants.py", result["output"])

    def test_single_tool_call(self):
        """Script calls terminal and prints the result."""
        code = """
from damon_tools import terminal
...
"""
    ...
    def test_multi_tool_chain(self):
        ...
        code = """
from damon_tools import terminal, read_file
r1 = terminal("ls")
...
"""
        ...
        # terminal won't be in damon_tools.py, so import fails
        self.assertEqual(result["status"], "error")
...
"""
    ...
    def test_web_search_tool(self):
        ...
        code = """
from damon_tools import web_search
...
"""
    ...
    def test_json_parse_helper(self):
        ...
        code = r"""
from damon_tools import json_parse
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from damon_tools import shell_quote
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_damon_rpc_socket_injected(self):
        child_env = self._get_child_env()
...
class TestExecuteCodeEdgeCases(unittest.TestCase):
    ...
    def test_none_enabled_tools_uses_all(self):
        ...
        code = (
            "from damon_tools import terminal, web_search, read_file\n"
    ...
    def test_empty_enabled_tools_uses_all(self):
        ...
        code = (
            "from damon_tools import terminal, web_search\n"
            "print('imports ok')\n"
        )
        with patch("model_tools.handle_function_call",
                    return_value=json.dumps({"ok": True})):
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_returns_code_execution_section(self):
        from tools.code_execution_tool import _load_config
        with patch("damon_cli.config.read_raw_config",
    ...
    def test_does_not_import_interactive_cli(self):
        ...
        with patch.dict("sys.modules", {"cli": mock_cli}), \
             patch("damon_cli.config.read_raw_config", return_value={}):
            result = _load_config()
