...
to docker/stage2-hook.sh, which is installed as /etc/cont-init.d/01-damon-setup
...
def test_puid_pgid_populate_damon_uid_gid(stage2_text: str) -> None:
    assert _resolve(stage2_text, {"PUID": "1000", "PGID": "10"}) == "1000:10"


def test_damon_uid_gid_take_precedence_over_aliases(stage2_text: str) -> None:
    resolved = _resolve(
        stage2_text,
...
def test_no_uid_vars_leaves_values_empty(stage2_text: str) -> None:
    # An empty resolution means the stage2 hook keeps the default damon user.
...
def test_stage2_hook_runs_config_migration_as_hermes(stage2_text: str) -> None:
    ...
    assert 's6-setuidgid damon "$INSTALL_DIR/.venv/bin/python"' in stage2_text
