...
def _restore_tool_modules():
    original_damon_home = os.environ.get("HERMES_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_damon_home is None:
            os.environ.pop("HERMES_HOME", None)
        else:
            os.environ["HERMES_HOME"] = original_damon_home
        _reset_modules(("tools", "damon_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    damon_cli = types.ModuleType("damon_cli")
    damon_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["damon_cli"] = damon_cli
    damon_home = tmp_path / "damon-home"
    os.environ["HERMES_HOME"] = str(damon_home)
    sys.modules["damon_cli.config"] = types.SimpleNamespace(
        get_damon_home=lambda: damon_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="damon-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": damon_home / "modal_snapshots.json",
