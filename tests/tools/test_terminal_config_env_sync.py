...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. damon_cli/config.py:set_config_value
...
dicts) keeps the test independent of the user's ~/.damon/config.yaml and
mirrors the pattern used in tests/damon_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    ...
    """
    from damon_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    ...
    assert not missing, (
        ...
        f"in damon_cli/config.py (set_config_value bridges through it)."
    )
