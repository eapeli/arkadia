"""Contract test: the s6-overlay stage2 hook resets ownership of damon-owned
...
by `docker exec <container> damon …` (root unless `-u` is passed) they land
root-owned and the unprivileged damon runtime then hits PermissionError on next
startup.
...
docker/stage2-hook.sh, installed as /etc/cont-init.d/01-damon-setup. This test
