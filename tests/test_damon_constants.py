"""Tests for damon_constants module."""
...
class TestGetDefaultDamonRoot:
    """Tests for get_default_damon_root() — Docker/custom deployment awareness."""

    def test_no_damon_home_returns_native(self, tmp_path, monkeypatch):
        """When HERMES_HOME is not set, returns ~/.damon."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_damon_root() == tmp_path / ".damon"

    def test_damon_home_is_native(self, tmp_path, monkeypatch):
        """When HERMES_HOME = ~/.damon, returns ~/.damon."""
        native = tmp_path / ".damon"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HERMES_HOME", str(native))
        assert get_default_damon_root() == native

    def test_damon_home_is_profile(self, tmp_path, monkeypatch):
        """When HERMES_HOME is a profile under ~/.damon, returns ~/.damon."""
        native = tmp_path / ".damon"
    ...
    @pytest.mark.skipif(
        condition="DAMON_HOME" in os.environ or "HERMES_HOME" in os.environ,
        reason="Env-sourced HOME override makes this path unreliable in the test runner",
    )
    def test_damon_home_is_docker(self, tmp_path, monkeypatch):
        """When HERMES_HOME points outside ~/.damon (Docker), returns HERMES_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(docker_home))
        assert get_default_damon_root() == docker_home

    @pytest.mark.skipif(
        condition="DAMON_HOME" in os.environ or "HERMES_HOME" in os.environ,
        reason="Env-sourced HOME override makes this path unreliable in the test runner",
    )
    def test_damon_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any HERMES_HOME outside ~/.damon is treated as the root."""
        custom = tmp_path / "my-damon-data"
        custom.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(custom))
        assert get_default_damon_root() == custom
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        ...
        profile.mkdir(parents=True)
        monkeypatch.setenv("HERMES_HOME", str(profile))
        assert get_default_damon_root() == docker_root

    def test_no_damon_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\damon, not ~/.damon."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(damon_constants.sys, "platform", "win32")

        assert get_default_damon_root() == local_appdata / "damon"

    def test_no_damon_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/damon without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("HERMES_HOME", raising=False)
        ...
        assert get_default_damon_root() == home / "AppData" / "Local" / "damon"


class TestGetDamonHome:
    """Tests for get_damon_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When HERMES_HOME is unset on Windows, use %LOCALAPPDATA%\\damon."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(damon_constants.sys, "platform", "win32")
        monkeypatch.setattr(damon_constants, "_profile_fallback_warned", False)

        assert get_damon_home() == local_appdata / "damon"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(damon_constants, "_container_detected", None)
    ...
    @pytest.mark.skipif(
        condition=not Path("/proc").exists(),
        reason="/proc is not mounted; container detection via /proc is unavailable here",
    )
    ...
    def test_detects_cgroup_kubepods(self, monkeypatch, tmp_path):
        ...
        def _fake_open(p, *a, **kw):
            ...
            if str(p).startswith("/proc/"):
                return _real_open(str(tmp_path / "void"), *a, **kw)
            return _real_open(p, *a, **kw)
        monkeypatch.setattr("builtins.open", _fake_open)
        assert is_container() is True
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(damon_constants, "_container_detected", True)
        assert is_container() is True
