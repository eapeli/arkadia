...
def server():
    with patch.dict(
        "sys.modules",
        {
            "damon_constants": MagicMock(
                get_damon_home=MagicMock(return_value="/tmp/damon_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'damon-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'damon-release' patched"
    }
