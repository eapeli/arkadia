...
def worker_loop(worker_id: int, damon_home: str, result_file: str) -> None:
...
def reclaimer_loop(damon_home: str, result_file: str) -> None:
    os.environ["HERMES_HOME"] = damon_home
    os.environ["HOME"] = damon_home
    sys.path.insert(0, WT)
    from damon_cli import kanban_db as kb
...
def main():
    home = tempfile.mkdtemp(prefix="damon_reclaim_race_")
