...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves damon_cli.kanban_db on its own
...
def main():
    home = tempfile.mkdtemp(prefix="damon_e2e_")
    ...
    # damon_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "damon")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m damon_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/damon_e2e_*/worker_*.log
    import glob
