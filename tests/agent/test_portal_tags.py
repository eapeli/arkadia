...
def test_damon_client_tag_includes_current_version():
    """The client tag must reflect damon_cli.__version__ verbatim."""
    from damon_cli import __version__
    from agent.portal_tags import damon_client_tag

    assert damon_client_tag() == f"client=damon-client-v{__version__}"


def test_damon_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import damon_client_tag

    tag = damon_client_tag()
    assert tag.startswith("client=damon-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import damon_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=damon-agent" in tags
    assert damon_client_tag() in tags
    assert len(tags) == 2
