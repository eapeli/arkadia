...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_damon_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / ".hermes"))

    damon_env = tmp_path / ".hermes" / ".env"
    damon_env.parent.mkdir(parents=True)
    damon_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.damon/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
