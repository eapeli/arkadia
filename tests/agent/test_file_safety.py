...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_damon_env(self):
        ...
        # Note: damon internal .env is in ~/.damon/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.damon/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.damon/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = damon_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._damon_home_path", return_value=damon_home):
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        damon_home = tmp_path / ".hermes"
        hub = damon_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_damon_home(self, tmp_path):
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        damon_home = tmp_path / ".hermes"
        cache = damon_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
