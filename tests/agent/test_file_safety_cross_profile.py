...
Reference: May 2026 incident — a damon-security profile session
accidentally edited skills under both ~/.damon/profiles/damon-security/skills/
AND ~/.damon/skills/ (the default profile's skills), realizing only
...
def fake_hermes(tmp_path, monkeypatch):
    ...
    sec_home = root / "profiles" / "damon-security"
    ...
    monkeypatch.setattr(damon_constants, "get_default_damon_root", lambda: root)

    # The reloads below ensure get_cross_profile_warning/classify see the patched root.
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_damon_root_path", lambda: root)
...
def _set_active_home(monkeypatch, damon_home: Path):
    """Point file_safety._damon_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_damon_home_path", lambda: damon_home)
...
class TestResolveActiveProfileName:
    ...
    def test_named_profile(self, fake_hermes, monkeypatch):
        ...
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "damon-security"
    ...
    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_damon_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["active_profile"] == "damon-security"
    ...
    def test_default_writing_security_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["target_profile"] == "damon-security"
    ...
    def test_non_damon_path_returns_none(self, fake_hermes, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_hermes["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any Hermes root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_damon_config_not_classified_as_cross_profile(self, fake_hermes, monkeypatch):
