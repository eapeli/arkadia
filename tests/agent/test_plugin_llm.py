...
""",
        ...
        from damon_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        damon_home = tmp_path / ".hermes"
        damon_home.mkdir()
        (damon_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("HERMES_HOME", str(damon_home))
        from damon_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from damon_cli.plugins import PluginContext, PluginManifest, PluginManager
