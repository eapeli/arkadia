...
Those paths look like ``…/sandboxes/<backend>/<task>/home/.damon/…`` and
...
class TestClassifySandboxMirrorTarget:
    ...
    def test_sandboxes_segment_without_home_damon_returns_none(self, tmp_path):
    ...
    def test_sandboxes_segment_with_home_but_no_damon_returns_none(self, tmp_path):
    ...
    def test_truncated_sandbox_path_returns_none(self, tmp_path):
        """``…/sandboxes/<backend>/<task>`` without ``home/.damon/<thing>`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
...
class TestSandboxMirrorIsOrthogonalToCrossProfile:
    ...
    def test_same_profile_mirror_still_flagged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_damon_root_path", lambda: tmp_path)
        monkeypatch.setattr(fs, "_damon_home_path", lambda: tmp_path / "profiles" / "group1")
