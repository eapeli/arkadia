...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("damon/damon-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned damon/* and pr-* branches."""

    def test_prunes_orphaned_damon_branch(self, git_repo):
        """damon/damon-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "damon/damon-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "damon/damon-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "damon/damon-deadbeef" in result.stdout
        ...
        assert "damon/damon-deadbeef" in orphaned
        ...
        assert "damon/damon-deadbeef" not in result.stdout
