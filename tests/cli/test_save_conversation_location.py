...
Regression: the old implementation wrote ``damon_conversation_<ts>.json``
...
The fix writes snapshots under ``~/.damon/sessions/saved/`` and prints
...
def damon_home(tmp_path, monkeypatch):
    ...
    # Clear any cached damon_home computation
    import damon_pkg.constants as damon_constants
    if hasattr(damon_constants, "_damon_home_cache"):
        damon_constants._damon_home_cache = None
    return home
...
def test_save_conversation_writes_under_damon_home(damon_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.damon/sessions/saved/, not CWD."""
    ...
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "damon_constants"]:
    ...
    cwd_leak = list(work.glob("damon_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.damon/sessions/saved/
    saved_dir = damon_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("damon_conversation_*.json"))
...
def test_save_conversation_empty_history_does_nothing(damon_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "damon_constants"]:
        sys.modules.pop(mod, None)
    import cli
