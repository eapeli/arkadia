...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._damon_home", tmp_path), \
             patch("damon_constants.display_damon_home", return_value="~/.damon/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.damon/profiles/coder/config.yaml`"
