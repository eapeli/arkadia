...
class TestExitSummaryResumeHint:
    ...
    profile boundaries — sessions live under `~/.damon-profiles/<profile>/`,
    ...
    def test_resume_hint_includes_profile_flag_for_named_profile(self, capsys):
        cli_obj = _make_cli()
        with patch("damon_cli.profiles.get_active_profile_name", return_value="dev"):
            cli_obj._print_exit_summary()
        out = capsys.readouterr().out
    ...
    def test_resume_hint_falls_back_when_profile_lookup_fails(self, capsys):
        ...
        cli_obj = _make_cli()
        with patch(
            "damon_cli.profiles.get_active_profile_name",
