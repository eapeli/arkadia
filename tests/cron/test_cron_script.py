...
def cron_env(tmp_path, monkeypatch):
    ...
    (damon_home / "scripts").mkdir()
    monkeypatch.setenv("HERMES_HOME", str(damon_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "HERMES_DIR", damon_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", damon_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", damon_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", damon_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.damon/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
