...
class TestSysPathOrdering:
    ...
    def test_damon_time_importable(self):
        """damon_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _damon_now
        assert callable(_damon_now)

    def test_damon_constants_importable(self):
        """damon_constants should be importable from cron context."""
        from damon_pkg.constants import get_damon_home
        assert callable(get_damon_home)
