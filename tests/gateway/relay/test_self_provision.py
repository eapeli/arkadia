...
def _arm(monkeypatch, *, managed=True, url="wss://connector.example/relay", token="nas-token"):
    monkeypatch.setattr("damon_cli.config.is_managed", lambda: managed)
    monkeypatch.setattr(relay, "relay_url", lambda: url)
    monkeypatch.setattr("damon_cli.auth.resolve_nous_access_token", lambda: token)
...
def test_token_failure_is_non_fatal(monkeypatch):
    ...
    monkeypatch.setattr("damon_cli.auth.resolve_nous_access_token", _boom)
