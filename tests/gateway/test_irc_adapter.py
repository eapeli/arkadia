...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        msg = _parse_irc_message(":server 001 damon-bot :Welcome to IRC")
        assert msg["prefix"] == "server"
        assert msg["command"] == "001"
        assert msg["params"] == ["damon-bot", "Welcome to IRC"]

    def test_parse_nick_collision(self):
        msg = _parse_irc_message(":server 433 * damon-bot :Nickname is already in use")
...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "damon"
        assert adapter.channel == "#damon-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_welcome(self, adapter):
        ...
        await adapter._handle_line(":server 001 damon :Welcome to IRC")
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        await adapter._handle_line(":server 433 * damon :Nickname in use")
        assert adapter._current_nick == "damon_"
        sent = writer.write.call_args[0][0]
        assert b"NICK damon_" in sent
    ...
    async def test_handle_dm(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG damon :private message")
        assert len(dispatched) == 1
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG damon :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_nick_collision_retry(self, adapter):
        ...
        await adapter._handle_line(":server 433 * damon :Nickname in use")
        assert adapter._current_nick == "damon_"
        await adapter._handle_line(":server 433 * damon_ :Nickname in use")
        assert adapter._current_nick == "damon_1"
        await adapter._handle_line(":server 433 * damon_1 :Nickname in use")
        assert adapter._current_nick == "damon_2"
