...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_damon_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("damon_constants.display_damon_home", return_value="~/.damon/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.damon/profiles/coder/cron/output/" in prompt
...
class TestSessionStoreRewriteTranscript:
    ...
    def store(self, tmp_path, monkeypatch):
        import damon_state
        monkeypatch.setattr(damon_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
        config = GatewayConfig()
