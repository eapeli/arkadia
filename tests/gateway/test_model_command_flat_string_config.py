...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_damon_home", damon_home)
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(
        "damon_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_damon_home() / config.yaml`` — point it here.
    monkeypatch.setattr("damon_constants.get_damon_home", lambda: damon_home)
    monkeypatch.setattr("damon_cli.config.get_damon_home", lambda: damon_home)
    return cfg_path
