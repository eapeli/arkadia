...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.damon/media/'
...
class TestMediaDeliveryPathValidation:
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.damon/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_damon_credentials(self, tmp_path, monkeypatch):
        """~/.damon/.env and ~/.damon/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        damon_dir = fake_home / ".hermes"
        damon_dir.mkdir(parents=True)
        env_file = damon_dir / ".env"
    ...
    def test_denylist_blocks_damon_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        damon_dir = fake_home / ".hermes"
        damon_dir.mkdir(parents=True)
        config_file = damon_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_damon_root_config_for_profiles(self, tmp_path, monkeypatch):
        ...
        damon_root = fake_home / ".hermes"
        config_file = damon_root / "config.yaml"
    ...
    def test_root_home_damon_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.damon/.env`` stays blocked under the $HOME exception — it is a
        ...
        monkeypatch.setattr("gateway.platforms.base._HERMES_HOME", damon_dir)
