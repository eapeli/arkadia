...
Covers four shapes from Phase 4 of ``.damon/plans/2026-05-21-dashboard-oauth-auth.md``:
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            nous_plugin.NousDashboardAuthProvider(
                client_id="damon-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    ...
    def test_skips_when_client_id_malformed(self, monkeypatch):
        monkeypatch.setenv("HERMES_DASHBOARD_OAUTH_CLIENT_ID", "damon-dashboard")
        ...
        assert "damon-dashboard" in nous_plugin.LAST_SKIP_REASON
...
class TestConfigYamlSource:
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``damon_cli.config.load_config``
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        monkeypatch.setattr(
            "damon_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        nous_plugin.register(ctx)
...
class TestStartLogin:
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "damon_session_pkce" in result.cookie_payload
        pkce = result.cookie_payload["damon_session_pkce"]
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["damon_session_pkce"] != b.cookie_payload[
            "damon_session_pkce"
        ]
