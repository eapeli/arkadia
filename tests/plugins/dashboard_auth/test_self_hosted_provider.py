...
class TestStartLogin:
    ...
    def test_two_calls_differ(self, provider):
        ...
        assert (
            a.cookie_payload["damon_session_pkce"]
            != b.cookie_payload["damon_session_pkce"]
        )
...
class TestPluginRegister:
    ...
    def patch_config(self, monkeypatch):
        def _set(oauth_block):
            ...
            monkeypatch.setattr("damon_cli.config.load_config", lambda: cfg)
    ...
    def test_config_load_failure_falls_through(self, monkeypatch):
        ...
        monkeypatch.setattr("damon_cli.config.load_config", _broken)
        ctx = MagicMock()
