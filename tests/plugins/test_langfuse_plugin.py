...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_damon_config(self, monkeypatch):
        ...
        # Drop any cached import of damon_cli.config.
        sys.modules.pop("damon_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "damon_cli.config" not in sys.modules, (
            "langfuse plugin imported damon_cli.config — regression toward "
