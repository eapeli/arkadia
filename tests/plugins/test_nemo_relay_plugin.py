...
def test_nemo_relay_plugin_emits_llm_tool_and_exports_atif(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "atif" / "damon-atif-s1.json").exists()
...
def test_nemo_relay_plugin_reparents_child_session_scope_for_embedded_atif(monkeypatch):
    ...
    child_push = next(
        ...
        if event[0] == "scope.push" and event[1] == "damon-session-child-session"
    )
    child_kwargs = child_push[3]
    assert child_kwargs["handle"] == ("scope", "damon-session-parent-session")
...
def test_nemo_relay_plugin_skips_embedded_child_atif_file_by_default(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "atif" / "damon-atif-parent-session.json").exists()
    assert not (tmp_path / "atif" / "damon-atif-child-session.json").exists()
...
def test_nemo_relay_plugin_can_write_embedded_child_atif_file_in_all_mode(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "atif" / "damon-atif-parent-session.json").exists()
    assert (tmp_path / "atif" / "damon-atif-child-session.json").exists()
...
""",
    ...
    assert "damon-session-s2" in scope_push_names
...
""",
    ...
    assert not (tmp_path / "direct-atif" / "damon-atif-s1.json").exists()
...
""",
    ...
    assert "atif.register" in event_names
    assert (tmp_path / "direct-atif" / "damon-atif-s1.json").exists()
...
def test_nemo_relay_downstream_unwrap_matches_real_middleware_wrapper_shape(monkeypatch):
    ...
    # ``damon_cli.middleware._run_execution_chain`` hands to a middleware
    ...
    from damon_cli import middleware
