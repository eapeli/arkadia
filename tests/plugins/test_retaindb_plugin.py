...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", damon_home=str(tmp_path / ".hermes"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", damon_home=profile_home)
        assert p._client.project == "damon-coder"
        p.shutdown()
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_HOME", str(damon_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", damon_home=str(damon_home))
        return p
