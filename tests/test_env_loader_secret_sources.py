"""Tests for the secret-source tracking in ``damon_cli.env_loader``.
...
from damon_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_damon_dotenv()`` is called at module-import time from several
    hot modules (cli.py, damon_cli/main.py, run_agent.py, ...).  The
    ...
    # cli.py, damon_cli/main.py, run_agent.py, trajectory_compressor.py,
