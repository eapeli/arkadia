...
class TestToolsetConsistency:
    ...
    def test_damon_platforms_share_core_tools(self):
        """All damon-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on damon-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["damon-cli", "damon-telegram", "damon-discord", "damon-whatsapp", "damon-slack", "damon-signal", "damon-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_damon_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("damon-whatsapp")

    def test_damon_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("damon-api-server")
