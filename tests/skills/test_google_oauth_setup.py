...
class TestHermesConstantsFallback:
    """Tests for _damon_home.py fallback when damon_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_damon_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _damon_home.py with damon_constants blocked."""
        monkeypatch.setitem(sys.modules, "damon_constants", None)
        spec = importlib.util.spec_from_file_location("_damon_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_damon_home_env_var(self, monkeypatch, tmp_path):
        """When damon_constants is missing, HERMES_HOME comes from env var."""
        monkeypatch.setenv("HERMES_HOME", str(tmp_path / "custom-hermes"))
        module = self._load_helper(monkeypatch)
        assert module.get_damon_home() == tmp_path / "custom-hermes"

    def test_fallback_defaults_to_dot_hermes(self, monkeypatch):
        """When damon_constants is missing and HERMES_HOME unset, default to ~/.hermes."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_damon_home() == Path.home() / ".hermes"

    def test_fallback_ignores_empty_damon_home(self, monkeypatch):
        """Empty/whitespace HERMES_HOME is treated as unset."""
        monkeypatch.setenv("HERMES_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_damon_home() == Path.home() / ".hermes"

    def test_fallback_display_damon_home_shortens_path(self, monkeypatch):
        """Fallback display_damon_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_damon_home() == "~/.hermes"

    def test_fallback_display_damon_home_profile_path(self, monkeypatch):
        """Fallback display_damon_home() handles profile paths under ~/."""
        monkeypatch.setenv("HERMES_HOME", str(Path.home() / ".damon/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_damon_home() == "~/.damon/profiles/coder"

    def test_fallback_display_damon_home_custom_path(self, monkeypatch):
        """Fallback display_damon_home() returns full path for non-home locations."""
        monkeypatch.setenv("HERMES_HOME", "/opt/damon-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_damon_home() == "/opt/damon-custom"

    def test_delegates_to_damon_constants_when_available(self):
        """When damon_constants IS importable, _damon_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_damon_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import damon_pkg.constants as damon_constants
        assert module.get_damon_home is damon_constants.get_damon_home
        assert module.display_damon_home is damon_constants.display_damon_home
...
class TestInstallDeps:
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "damon-agent[google]" in out
