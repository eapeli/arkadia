...
def test_no_args_starts_hermes(built_image: str) -> None:
    """``docker run <image>`` should start damon cleanly.
...
def test_bare_executable_passthrough(built_image: str) -> None:
    ...
    damon wrapper. Useful for long-lived sandbox mode and for testing.
    """
    r = subprocess.run(
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> damon batch ...`` returns a
