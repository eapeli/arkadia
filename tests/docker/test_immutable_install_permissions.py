...
def test_damon_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        r"""
        set -eu
        /opt/damon/.venv/bin/python - <<'PY'
        from pathlib import Path

        install_file = Path("/opt/damon/agent/message_sanitization.py")
        try:
