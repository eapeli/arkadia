...
``$HERMES_GIT_SHA`` build-arg to ``/opt/damon/.damon_build_sha`` and
``damon_cli/build_info.py`` reads it as a fallback.
...
* If ``/opt/damon/.damon_build_sha`` exists in the image, assert that
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's damon user (UID 10000) can bootstrap
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/damon/.damon_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/damon/.damon_build_sha",
        ],
