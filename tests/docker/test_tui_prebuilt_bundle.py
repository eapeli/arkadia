...
The fix is ``ENV HERMES_TUI_DIR=/opt/damon/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the damon user, return stdout."""
    inner = (
        "source /opt/damon/.venv/bin/activate && "
        "cd /opt/hermes && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the damon user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "damon", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_damon_tui_dir_env_is_set(built_image: str) -> None:
    ...
    assert r.stdout.strip() == "/opt/damon/ui-tui", (
        f"HERMES_TUI_DIR={r.stdout.strip()!r} (expected /opt/damon/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from damon_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/damon/ui-tui')\n"
