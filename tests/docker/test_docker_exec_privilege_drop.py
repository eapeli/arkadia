...
The shim (docker/damon-exec-shim.sh, installed at /opt/damon/bin/hermes)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> damon login` would write /opt/data/auth.json as
...
1. ``docker exec <c> damon …`` (defaulting to root) gets dropped to the
   damon user before the real binary runs.
2. ``docker exec --user damon <c> damon …`` (already non-root) short-
...
def test_shim_drops_root_to_damon_uid(sleep_container: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "damon", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
...
def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user damon already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "damon", sleep_container,
         "damon", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user damon failed: {r.stderr!r} stdout={r.stdout!r}. "
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "damon", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
...
def test_shim_opt_out_strict_truthiness(
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "damon", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/damon/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid damon hermes <args>` which resolves `hermes`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> damon login`` (root) wrote
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "damon", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the damon user "
