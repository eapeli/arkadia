...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``damon://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``damon://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"damon://blueprint/{quote(blueprint.key)}{qs}"
