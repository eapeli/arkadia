...
Architecture (two transports):
  ...
  **Local backend (UDS):**
  1. Parent generates a `damon_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `damon_tools.py` with file-based RPC stubs
...
# location/profile vars in _DAMON_CHILD_ALLOWED below; HERMES_RPC_SOCKET /
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational HERMES_* vars (_DAMON_CHILD_ALLOWED) pass by exact name.
...
def generate_damon_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the damon_tools.py stub module.
...
_RPC_DIR = os.environ.get("HERMES_RPC_DIR") or os.path.join(tempfile.gettempdir(), "damon_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated damon_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/damon_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_damon_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/damon_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with damon_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="damon_sandbox_")
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"damon_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated damon_tools module.
        ...
        tools_src = generate_damon_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "damon_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        # Ensure the damon-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from damon_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _damon_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _damon_root]
        if _existing_pp:
        ...
        from damon_pkg.constants import apply_subprocess_home_env
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.damon/.env')).
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (damon-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    # temp-dir staging and damon-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.damon/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        ...
        f"Available via `from damon_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into damon_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from damon_tools import {import_str}` "
