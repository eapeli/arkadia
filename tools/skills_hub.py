...
Used by damon_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            damon_meta = metadata.get("damon", {})
            if isinstance(damon_meta, dict):
                tags = damon_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.damon/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                damon_meta = meta_block.get("damon", {})
                if isinstance(damon_meta, dict):
                    tags = damon_meta.get("tags", [])
...
HERMES_INDEX_URL = "https://damon-agent.nousresearch.com/docs/api/skills-index.json"
HERMES_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "damon-index.json"
HERMES_INDEX_TTL = 6 * 3600  # 6 hours


def _load_damon_index() -> Optional[dict]:
...
class HermesIndexSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "damon-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "damon-index"),
            identifier=entry.get("identifier", ""),
