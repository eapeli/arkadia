...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from damon_cli.plugins import invoke_hook
    except Exception:
...
# active profile home path such as /home/damon/.damon/config.yaml. The
# resolved-absolute form is folded into the ~/.damon/ patterns at detection
...
_DAMON_ENV_PATH = (
    r'(?:~\/\.damon/|'
    r'(?:\$home|\$\{home\})/\.damon/|'
    r'(?:\$damon_home|\$\{damon_home\})/)'
    r'\.env\b'
)
# ~/.damon/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _DAMON_ENV_PATH; matches the HERMES_HOME override form as
# well as ~/.damon/.
_DAMON_CONFIG_PATH = (
    r'(?:~\/\.damon/|'
    r'(?:\$home|\$\{home\})/\.damon/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart damon gateway (kills running agents)"),
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart damon-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart damon-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(hermes|gateway|cli\.py)\b', "kill damon/gateway process (self-termination)"),
    ...
    # ~/.damon/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Hermes-managed security file (~/.damon/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_DAMON_CONFIG_PATH}|{_DAMON_ENV_PATH})', "in-place edit of Hermes config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_DAMON_CONFIG_PATH}|{_DAMON_ENV_PATH})', "in-place edit of Hermes config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_DAMON_CONFIG_PATH}|{_DAMON_ENV_PATH})', "in-place edit of Hermes config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.damon/ form so the Hermes config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/damon/.damon/config.yaml`) rather than
    ...
    command = _rewrite_resolved_damon_home(command)
    return command
...
def _rewrite_resolved_damon_home(command: str) -> str:
    """Rewrite the resolved absolute Hermes home prefix to ``~/.damon/``.
    ...
    ``~/.damon/`` so the static ``_DAMON_CONFIG_PATH`` / ``_DAMON_ENV_PATH``
    ...
    try:
        ...
        home = get_damon_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        ...
        # /home/damon/.hermes or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.damon/")
    return command
