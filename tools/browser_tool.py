...
def _get_call_llm():
    from agent.auxiliary_client import call_llm as _fn
    return _fn

from utils import env_int, is_truthy_value
from damon_cli.config import DEFAULT_CONFIG, cfg_get
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    damon_node_bin = str(damon_home / "node" / "bin")
    damon_node_root = str(damon_home / "node")
    damon_nm_bin = str(damon_home / "node_modules" / ".bin")
    return [damon_node_bin, damon_node_root, damon_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.damon/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.damon/plugins/browser/<vendor>/``) participate
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-damon_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed damon processes — this way every clean damon exit sweeps
    ...
    # Sweep orphans from other crashed damon processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live damon processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current damon PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live damon process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning damon process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced damon PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent damon instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-damon_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live damon process.
            continue
...
def _find_agent_browser() -> str:
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                damon_nm = str(get_damon_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=damon_nm)
            if not recheck:
                damon_node_bin = str(get_damon_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=damon_node_bin)
            if not recheck:
                damon_node_root = str(get_damon_home() / "node")
                recheck = shutil.which("agent-browser", path=damon_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this damon PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = damon_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    from damon_pkg.constants import get_damon_dir
    screenshots_dir = get_damon_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/damon-agent:latest")
                else:
