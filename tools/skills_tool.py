...
from damon_pkg.constants import get_damon_home as get_damon_home, display_damon_home
import os
import re
...
# All skills live in ~/.damon/skills/ (seeded from bundled skills/ on install).
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_damon_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.damon/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.damon/skills/ and external dirs.
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_damon_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from damon_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: damon plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.damon/skills/): {skill_md}")
        ...
        # Check metadata.damon.* first (agentskills.io convention), fall back to top-level
        damon_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            damon_meta = metadata.get("damon", {}) or {}

        tags = _parse_tags(damon_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            damon_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
