...
_damon_config_resolved: str | None = None
_damon_config_resolved_loaded = False


def _get_damon_config_resolved() -> str | None:
    """Return the resolved absolute path of the Hermes config file (cached)."""
    global _damon_config_resolved, _damon_config_resolved_loaded
    if _damon_config_resolved_loaded:
        return _damon_config_resolved
    _damon_config_resolved_loaded = True
    try:
        from damon_cli.config import get_config_path
        _damon_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _damon_config_resolved = str(Path("~/.damon/config.yaml").expanduser().resolve())
        except Exception:
            _damon_config_resolved = None
    return _damon_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # this file.
    damon_config = _get_damon_config_resolved()
    if damon_config and (resolved == damon_config or normalized == damon_config):
        return (
            f"Refusing to write to Hermes config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.damon/config.yaml directly or use 'hermes config' instead."
        )
    return None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.damon/…`` mirror created by a
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.damon/…`` path.
    ...
    # in a session that cd'd into ``~/.damon/profiles/other/`` is
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with damon_tools.write_file() for very "
            "large files."
        )
