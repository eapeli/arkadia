...
# display_damon_home imported lazily at call site (stale-module safety during damon update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of damon directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "damon-*")):
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``damon-agent=1`` for the current
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.damon/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for damon-tagged containers left behind by
    ...
    try:
        ...
        if background:
            ...
            try:
                ...
                # damon-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in damon-agent dev work (PRs #31329, "
