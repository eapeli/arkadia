...
def register_credential_file(
    ...
    *relative_path* is relative to ``HERMES_HOME`` (e.g. ``google_token.json``).
    ...
    """
    damon_home = _resolve_damon_home()
    ...
    host_path = damon_home / relative_path
    ...
    containment_error = validate_within_dir(host_path, damon_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.damon/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="damon-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching damon_constants.get_damon_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_damon_dir()`` for backward compatibility with old directory layouts.
    """
    from damon_pkg.constants import get_damon_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_damon_dir(new_subpath, old_name)
        if host_dir.is_dir():
