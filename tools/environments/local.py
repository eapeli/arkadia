...
_DAMON_PROVIDER_ENV_FORCE_PREFIX = "_DAMON_FORCE_"
...
_DAMON_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _inject_context_damon_home(env: dict) -> None:
    """Bridge the context-local Hermes home override into subprocess env."""
    try:
        from damon_pkg.constants import get_damon_home as get_damon_home_override

        value = get_damon_home_override()
        if value:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_DAMON_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _DAMON_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_DAMON_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_DAMON_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _DAMON_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_damon_home(sanitized)
...
def _find_bash() -> str:
    ...
    _damon_portable_git = os.path.join(_local_appdata, "damon", "git") if _local_appdata else ""
    if _damon_portable_git:
        for candidate in (
            os.path.join(_damon_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_damon_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_DAMON_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_DAMON_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _DAMON_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        run_env[path_key] = _append_missing_sane_path_entries(run_env.get(path_key, ""))

    _inject_context_damon_home(run_env)
...
class LocalEnvironment(BaseEnvironment):
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        if _IS_WINDOWS:
            ...
            try:
                ...
                cache_dir = get_damon_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "damon_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._damon_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_damon_pgid", None)
                    if pgid is None:
                        raise
