...
from tools.environments.local import _DAMON_PROVIDER_ENV_BLOCKLIST
...
def _load_damon_env_vars() -> dict[str, str]:
    """Load ~/.damon/.env values without failing Docker command execution."""
    try:
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale damon-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=damon-agent=1`` (created by this codebase)
    ...
    * (optional) ``label=damon-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a damon process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    filters = ["--filter", "label=damon-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=damon-profile={_sanitize_label_value(profile_filter)}"])
...
#   SETUID/SETGID - the image's init drops from root to the 'damon'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``damon-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.damon/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. damon-agent:latest) already use /init as PID 1
        ...
        container_name = f"damon-{uuid.uuid4().hex[:8]}"
        # Labels make damon-created containers identifiable to:
        #   * the orphan reaper (`damon-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`damon-task-id`, `damon-profile`)
        #   * operators running `docker ps --filter label=damon-agent=1`
        ...
        label_args = [
            "--label", "damon-agent=1",
            "--label", f"damon-task-id={task_label}",
            "--label", f"damon-profile={profile_name}",
        ]
        ...
        self._labels = {
            "damon-agent": "1",
            "damon-task-id": task_label,
            "damon-profile": profile_name,
        }
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _DAMON_PROVIDER_ENV_BLOCKLIST)
        damon_env = _load_damon_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = damon_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("damon-task-id", "")
        profile_label = self._labels.get("damon-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"damon-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``damon-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=damon-agent=1",
                    "--filter", f"label=damon-task-id={task_label}",
                    "--filter", f"label=damon-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"damon-cleanup-{log_id}")
        t.start()
