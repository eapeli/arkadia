...
transport, discovers their tools, and registers them into the damon-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.damon/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.damon/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            damon_home = os.path.expanduser(
                os.getenv(
                    "HERMES_HOME", os.path.join(os.path.expanduser("~"), ".hermes")
                )
            )
            candidates = [
                os.path.join(damon_home, "node", "bin", resolved_command),
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.damon/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    ...
    try:
        ...
        home_override = get_damon_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_damon_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # vanish here: OAuth token stores and any other get_damon_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.damon/.env`` loaded at startup).
    """
    try:
