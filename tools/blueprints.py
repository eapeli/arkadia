...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.damon.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.damon.blueprint`` is a mapping containing
    ...
    damon = meta.get("damon") if isinstance(meta, dict) else None
    blueprint = damon.get("blueprint") if isinstance(hermes, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.damon.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.damon.blueprint`` block) they can hand
