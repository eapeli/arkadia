...
def _damon_bin_dir() -> str:
    """Return $HERMES_HOME/bin, creating it if needed."""
    d = os.path.join(_get_damon_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    ...
    try:
        ...
        dest = os.path.join(_damon_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    damon_bin = os.path.join(_damon_bin_dir(), "tirith")
    if os.path.isfile(damon_bin) and os.access(damon_bin, os.X_OK):
        _resolved_path = damon_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return damon_bin
