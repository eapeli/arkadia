...
def has_xai_credentials() -> bool:
    ...
    2. ``~/.damon/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_damon_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.damon/.env`` first, then ``os.environ``.

    Wraps :func:`damon_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from damon_cli.config import get_env_value as _damon_get_env_value

        value = _damon_get_env_value(name)
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    to ``XAI_API_KEY`` resolved via ``damon_cli.config.get_env_value`` so keys
    stored in ``~/.damon/.env`` (the standard Hermes location) are honored —
