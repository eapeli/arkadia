#!/bin/sh
# shellcheck shell=sh
# /opt/damon/bin/hermes — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `hermes` user (UID 10000). When an operator runs `docker exec <c> damon ...`
...
# user runs `docker exec <c> damon login`, this writes
...
# Fix
# ---
# This shim sits at /opt/damon/bin/hermes and is placed earliest on PATH.
# When invoked as root, it drops to the damon user (via s6-setuidgid)
...
# (/opt/damon/.venv/bin/hermes), so the second hop cannot re-enter this
...
# state via the damon CLI. Default is to drop.

set -e

REAL=/opt/damon/.venv/bin/hermes

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "damon-shim: $REAL not found or not executable" >&2
    exit 127
fi
...
# Root, no opt-out. Drop to the damon user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "damon-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "damon-shim: re-run with --user damon or set HERMES_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the damon user's home before dropping privileges. Without
