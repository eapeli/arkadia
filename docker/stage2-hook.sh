...
#
# Wired into the image as /etc/cont-init.d/01-damon-setup by the
...
DAMON_HOME="${DAMON_HOME:-/opt/data}"
INSTALL_DIR="/opt/damon"

# Drop to damon via s6-setuidgid, but skip it when already non-root.
as_damon() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid damon "$@"; }
...
# the container starts non-root. The baked image trees (/opt/data, /opt/damon/
# .venv, ui-tui, node_modules) stay owned by the damon build UID (10000), so an
...
# into $DAMON_HOME). See #34837 for the supervision-tree side of this.
#
# The supported way to match host-side ownership is to start as root (the image
# default) and pass DAMON_UID/DAMON_GID — or the PUID/PGID aliases — which the
...
# `id -u damon` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# damon UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u damon)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-damon UID).
...
and the baked image directories are owned by the damon user (UID $(id -u damon)),
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e DAMON_UID=\$(id -u) -e DAMON_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the damon user to that UID/GID at boot and chowns the data
...
# --- Bootstrap DAMON_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid damon mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom DAMON_HOME paths whose parents only
# root can create (e.g. `DAMON_HOME=/home/damon/.damon` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for DAMON_UID/DAMON_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  DAMON_UID/
# DAMON_GID still win when both are set.  See #15290, salvages #25872.
DAMON_UID="${DAMON_UID:-${PUID:-}}"
DAMON_GID="${DAMON_GID:-${PGID:-}}"

if [ -n "${DAMON_UID:-}" ] && validate_uid_gid "$DAMON_UID" && [ "$DAMON_UID" != "$(id -u damon)" ]; then
    echo "[stage2] Changing damon UID to $DAMON_UID"
    usermod -u "$DAMON_UID" damon
fi
if [ -n "${DAMON_GID:-}" ] && validate_uid_gid "$DAMON_GID" && [ "$DAMON_GID" != "$(id -g damon)" ]; then
    echo "[stage2] Changing damon GID to $DAMON_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$DAMON_GID" damon 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised damon user (UID 10000)
...
# damon process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes damon. Idempotent across container
...
#   - damon is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G damon 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] damon already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" damon 2>/dev/null; then
        echo "[stage2] Added damon to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group damon failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When DAMON_UID is remapped or the top-level $DAMON_HOME isn't owned by
# the runtime damon UID, restore ownership to damon — but ONLY for the
# directories damon actually writes to. The full $DAMON_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of damon-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_damon_uid=$(id -u damon)
needs_chown=false
if [ "$(stat -c %u "$DAMON_HOME" 2>/dev/null)" != "$actual_damon_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $DAMON_HOME (targeted) to damon ($actual_damon_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $DAMON_HOME: chown the directory itself (not its contents)
    # so damon can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown damon:damon "$DAMON_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $DAMON_HOME failed (rootless container?) — continuing"
    # Damon-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by damon (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$DAMON_HOME/$sub" ]; then
            chown -R damon:damon "$DAMON_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $DAMON_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi

# --- Fix ownership of build trees under $INSTALL_DIR ---
# Damon-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime damon UID no longer owns them — otherwise:
...
#     the source mtime is newer than dist/ or when DAMON_TUI_FORCE_BUILD
#     is set) and writes to ui-tui/dist/. Without this chown the new
#     damon UID can't write the build output (#28851).
...
# The set mirrors the build-time `chown -R damon:damon` line in the
# Dockerfile — keep them in sync if the Dockerfile chown set changes.
# These are under $INSTALL_DIR (not $DAMON_HOME), so the bind-mount
# concern doesn't apply — recursive is fine.
#
# This MUST be gated independently of the $DAMON_HOME ownership check
# above. `usermod -u <new> damon` re-chowns the damon home dir
# ($DAMON_HOME == /opt/data) to the new UID as a side effect, so after a
# DAMON_UID/PUID remap `stat $DAMON_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/damon
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $DAMON_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime damon UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_damon_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to damon ($actual_damon_uid)"
    chown -R damon:damon \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
# Always reset ownership of $DAMON_HOME/profiles to damon on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> damon …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as damon and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$DAMON_HOME/profiles" ]; then
    chown -R damon:damon "$DAMON_HOME/profiles" 2>/dev/null || true
fi

# Always reset ownership of $DAMON_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged damon runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$DAMON_HOME/cron" ]; then
    chown -R damon:damon "$DAMON_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of damon-owned top-level state files on every boot.
# The targeted data-volume chown above only covers damon-owned
# *subdirectories*; loose state files living directly under $DAMON_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> damon …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged damon runtime then hits
...
# sweep so host-owned files in a bind-mounted $DAMON_HOME are never
...
# entries of damon_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$DAMON_HOME/$f" ]; then
        chown damon:damon "$DAMON_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the damon runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$DAMON_HOME/config.yaml" ]; then
    chown damon:damon "$DAMON_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$DAMON_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as damon user ---
# Run as damon via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $DAMON_HOME values
...
as_damon mkdir -p \
    ...
    "$DAMON_HOME/skins" \
    "$DAMON_HOME/plans" \
    "$DAMON_HOME/workspace" \
    "$DAMON_HOME/home" \
    "$DAMON_HOME/pairing" \
    "$DAMON_HOME/platforms/pairing"

# --- Install-method stamp (read by detect_install_method() in damon status) ---
# Preserved from the tini-era entrypoint (PR #27843). Must be written as
# the damon user so ownership matches the file's documented owner.
# tee is invoked directly via s6-setuidgid (no `sh -c` wrapper) for the
# same shell-metacharacter safety described above.
printf 'docker\n' | as_damon tee "$DAMON_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$DAMON_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_damon cp "$INSTALL_DIR/$src" "$DAMON_HOME/$dest"
    fi
}
...
if [ -f "$DAMON_HOME/.env" ]; then
    chown damon:damon "$DAMON_HOME/.env" 2>/dev/null || true
    chmod 600 "$DAMON_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $DAMON_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `damon update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set DAMON_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$DAMON_HOME/config.yaml" ]; then
    s6-setuidgid damon "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$DAMON_HOME/auth.json" ] && [ -n "${DAMON_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$DAMON_AUTH_JSON_BOOTSTRAP" > "$DAMON_HOME/auth.json"
    chown damon:damon "$DAMON_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$DAMON_HOME/auth.json"
fi
...
# can set DAMON_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# DAMON_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$DAMON_HOME/gateway_state.json" ] && \
        [ "${DAMON_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$DAMON_HOME/gateway_state.json"
    chown damon:damon "$DAMON_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$DAMON_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_damon "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/damon/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Damon spawns for the
...
# main-wrapper.sh propagates it into the supervised ``damon`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-damon, dashboard, etc.).
