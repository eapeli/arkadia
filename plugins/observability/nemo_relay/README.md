...
```bash
env HERMES_HOME=/tmp/damon-nemo-relay-test \
  damon plugins enable observability/nemo_relay
```
...
```bash
export HERMES_HOME=/tmp/damon-nemo-relay-test
hermes plugins enable observability/nemo_relay
...
uv run damon plugins enable observability/nemo_relay
uv run damon chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/damon_agent-*.whl
...
export HERMES_HOME=/tmp/damon-nemo-relay-docs/damon-home
mkdir -p "$HERMES_HOME"
...
```bash
export HERMES_NEMO_RELAY_ATOF_ENABLED=1
export HERMES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/damon-nemo-relay-docs/subagent/atof
export HERMES_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export HERMES_NEMO_RELAY_ATOF_MODE=overwrite
export HERMES_NEMO_RELAY_ATIF_ENABLED=1
export HERMES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/damon-nemo-relay-docs/subagent/atif
...
```bash
mkdir -p /tmp/damon-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/damon-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/damon-nemo-relay-docs/workdir/beta.txt
cd /tmp/damon-nemo-relay-docs/workdir

export HERMES_NEMO_RELAY_ATOF_ENABLED=1
export HERMES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/damon-nemo-relay-docs/parallel/atof
export HERMES_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export HERMES_NEMO_RELAY_ATOF_MODE=overwrite
export HERMES_NEMO_RELAY_ATIF_ENABLED=1
export HERMES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/damon-nemo-relay-docs/parallel/atif
...
```bash
export HERMES_NEMO_RELAY_PLUGINS_TOML=/tmp/damon-middleware-test/plugins.toml
```
...
```bash
export HERMES_HOME=/tmp/damon-middleware-test/damon-home
mkdir -p "$HERMES_HOME" /tmp/damon-middleware-test/nemo-relay
...
cat > /tmp/damon-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/damon-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/damon-middleware-test/atif"
...
export HERMES_NEMO_RELAY_PLUGINS_TOML=/tmp/damon-middleware-test/nemo-relay/plugins.toml
