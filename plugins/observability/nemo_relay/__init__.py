...
class _Settings:
    ...
    atof_filename: str = "damon-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "damon-atif-{session_id}.json"
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
            state.atif_exporter = self.nemo_relay.AtifExporter(
                ...
                extra={"source": "damon-agent", "plugin": "observability/nemo_relay"},
            )
...
def _load_settings() -> _Settings:
    ...
    return _Settings(
        ...
        atof_filename=_env("HERMES_NEMO_RELAY_ATOF_FILENAME") or "damon-atof.jsonl",
        ...
        atif_filename_template=_env("HERMES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "damon-atif-{session_id}.json",
...
def _resolve_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="damon-nemo-relay-awaitable",
        daemon=True,
    )
    thread.start()
    thread.join()
