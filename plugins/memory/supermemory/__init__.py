...
def _load_supermemory_config(damon_home: str) -> dict:
    config = _default_config()
    config_path = Path(damon_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, damon_home: str) -> None:
    config_path = Path(damon_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "damon"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "damon", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, damon_home):
        ...
        _save_supermemory_config(sanitized, damon_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from damon_pkg.constants import get_damon_home
        self._damon_home = kwargs.get("damon_home") or str(get_damon_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._damon_home)
