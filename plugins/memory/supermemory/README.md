...
|-----|---------|-------------|
| `container_tag` | `hermes` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `damon-{identity}` → `damon-coder`). |
...
```json
{
  "container_tag": "damon-{identity}"
}
```

For a profile named `coder`, this resolves to `damon-coder`. The default profile resolves to `damon-default`. Without `{identity}`, all profiles share the same container.
