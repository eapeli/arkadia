...
Handles: damon honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "damon" else ""
...
def _local_config_path() -> Path:
    ...
    """
    return get_damon_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        damon_host.get("pinUserPeer"),
        damon_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in damon_host:
        aliases_src = damon_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in damon_host:
        prefix_src = damon_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(damon_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom damon_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    damon_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        damon_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(damon_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = damon_host.get("userPeerAliases")
    prefix = damon_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(damon_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(damon_host)
    damon_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        damon_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        damon_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        current_host_key = damon_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            damon_host["apiKey"] = new_local_key
    ...
    # --- 3. Identity ---
    current_peer = damon_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        damon_host["peerName"] = new_peer

    current_ai = damon_host.get("aiPeer") or cfg.get("aiPeer", "damon")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        damon_host["aiPeer"] = new_ai

    current_workspace = damon_host.get("workspace") or cfg.get("workspace", "damon")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        damon_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, damon_host)
    ...
    if run_mapping:
        peer_target = damon_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(damon_host)
            damon_host["pinUserPeer"] = False
            if prior_aliases:
                damon_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                damon_host, current_prefix, prefix_from_root,
            ...
            if merged:
                damon_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                damon_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                damon_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = damon_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        damon_host["observationMode"] = new_obs
    else:
        damon_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(damon_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        damon_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        damon_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = damon_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        damon_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = damon_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        damon_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                damon_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(damon_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            damon_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        damon_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        damon_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        damon_host["dialecticReasoningLevel"] = new_reasoning
    else:
        damon_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = damon_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        damon_host["sessionStrategy"] = new_strat

    damon_host["enabled"] = True
    damon_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        damon_config = load_config()
        damon_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(damon_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: damon config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    damon honcho status     -- show full config")
    print("    damon honcho mode       -- change recall/observation mode")
    print("    damon honcho tokens     -- tune context and dialectic budgets")
    print("    damon honcho peer       -- update peer names")
    print("    damon honcho map <name> -- map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: damon honcho setup\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: damon honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        damon = hosts.get(_host_key(), {})
        user = damon.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = damon.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = damon.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = damon.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: damon honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: damon honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = damon.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = damon.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = damon.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("\n  Set with: damon honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    damon honcho identity --show        — show both peer representations")
        print("    damon honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  damon honcho setup")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  damon honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'damon'")
        ...
        for f in user_files:
            print("    damon honcho migrate  — this step handles it interactively")
        if has_key:
    ...
    if agent_files:
        ...
        if has_key:
            ...
            for f in agent_files:
                print(f"    damon honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  damon honcho identity <path/to/SOUL.md>")
    ...
    print("              Map a custom name:  damon honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. damon honcho setup              — configure API key (required)")
        print("  2. damon honcho migrate            — re-run this walkthrough")
    else:
        print("  1. damon honcho status             — verify Honcho connection")
        print("  2. damon                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. damon honcho identity --show    — verify AI peer representation")
        print("  4. damon honcho tokens             — tune context and dialectic budgets")
        print("  5. damon honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from damon_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to damon memory setup)",
    )
