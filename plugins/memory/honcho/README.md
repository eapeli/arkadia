...
| 2 | `~/.damon/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Hermes profile: `hermes` (default) or `damon_<profile>`.
...
| 5 | `per-repo` strategy | Git root directory name (`damon-agent`) |
...
If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-damon-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening damon in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
Host key is derived from the active Hermes profile: `hermes` (default) or `damon_<profile>` (e.g. `hermes -p coder` -> host key `damon_coder`). Older `hermes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
