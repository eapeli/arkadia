...
def resolve_active_host() -> str:
    ...
    Resolution order:
      ...
      2. Active profile name via profiles system -> ``damon.<profile>``
      3. Fallback: ``"damon"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      ...
      2. ~/.damon/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_damon_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_damon_home() / "honcho.json"
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    def _build() -> "Honcho":
        ...
        # the "go run damon honcho setup" hint they used to.
        try:
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                damon_cfg = load_config()
                honcho_cfg = damon_cfg.get("honcho", {})
