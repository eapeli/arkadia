...
Daemon startup logs: `~/.damon/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p damon ui start
```
...
Config file: `~/.damon/hindsight/config.json`
...
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `damon-{profile}` isolates memory per active Hermes profile. Empty placeholders collapse cleanly (e.g. `damon-{user}` with no user becomes `hermes`). |
...
The LLM API key is stored in `~/.damon/.env` as `HINDSIGHT_LLM_API_KEY`.
