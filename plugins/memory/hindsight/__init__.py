...
def _load_config() -> dict:
    ...
    profile_path = get_damon_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "damon": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "damon"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    ...
    profile = config.get("profile", "damon")
    return str(profile or "damon")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``damon-{user}`` with no user becomes ``hermes``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, damon_home):
        """Write config to $HERMES_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(damon_home) / "hindsight"
    ...
    def post_setup(self, damon_home: str, config: dict) -> None:
        ...
        from damon_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "damon")
        ...
        self.save_config(provider_config, damon_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(damon_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(damon_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "damon"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: damon-{profile}", "default": ""},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "damon"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "damon"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "damon", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "damon")
