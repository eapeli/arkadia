...
Let the damon agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where damon runs) ────────────────────────────┐
│                                                                      │
...
echo 'OPENAI_API_KEY=sk-...' >> ~/.damon/.env
...
On macOS, damon will **not** switch your system audio input automatically — the
