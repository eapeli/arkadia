...
:class:`~damon_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``damon_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~damon_cli.dashboard_auth.Session`.
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "damon_session_pkce": f"state={state};verifier={code_verifier}",
        }
