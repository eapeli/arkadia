...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``damon-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``damon_session_pkce``
cookie.
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"damon_session_pkce\"]``
...
def register(ctx) -> None:
    ...
    engagement layer (``damon_cli.web_server.should_require_auth`` +
