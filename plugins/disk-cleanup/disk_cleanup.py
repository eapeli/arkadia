...
Scope: strictly HERMES_HOME and /tmp/damon-*
Never touches: ~/.damon/logs/ or any system directory.
"""
...
try:
    ...
    def get_damon_home() -> Path:  # type: ignore[no-redef]
...
def get_state_dir() -> Path:
    ...
    return get_damon_home() / "disk-cleanup"
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HERMES_HOME or ``/tmp/damon-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    damon_home = get_damon_home()
    try:
        path.resolve().relative_to(damon_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/damon-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("damon-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "damon-agent", "backups", "profiles", ".worktrees",
})
...
def guess_category(path: Path) -> Optional[str]:
    ...
    damon_home = get_damon_home()
    try:
        rel = path.resolve().relative_to(damon_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "damon-agent",
        }:
            return None
        ...
        # Path isn't under HERMES_HOME (e.g. /tmp/damon-*) — fall through.
        pass
