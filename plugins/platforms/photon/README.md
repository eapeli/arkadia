...
3. **Provision the project secret** — mint a fresh project secret (the
   dashboard reveals it only once) and persist it to `~/.damon/.env` so the
...
Runtime SDK credentials live in `~/.damon/.env` (the same place every other
...
Management metadata lives in `~/.damon/auth.json` under `credential_pool`:
