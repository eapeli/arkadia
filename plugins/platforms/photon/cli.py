...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 3. Rotate the project secret and persist creds (runtime -> ~/.damon/.env,
    ...
    print("  Start the gateway:  damon gateway start")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.damon/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.damon/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  damon gateway restart")
    return 0
