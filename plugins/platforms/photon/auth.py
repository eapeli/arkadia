...
Credential storage mirrors every other Hermes channel:
    ...
    * runtime SDK creds  -> ``~/.damon/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.damon/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a damon dependency
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of damon-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.damon/auth.json`` honouring the active Hermes profile."""
    try:
        from damon_pkg.constants import get_damon_home
        return Path(get_damon_home()) / "auth.json"
    except Exception:
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.damon/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.damon/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.damon/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: damon_cli.config unavailable — skipping .env write")
        return
    try:
