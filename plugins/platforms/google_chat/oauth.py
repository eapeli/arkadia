...
try:
    from damon_pkg.constants import display_damon_home, get_damon_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where damon_constants isn't importable
...
# ``ls ~/.damon/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _damon_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _damon_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _damon_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _damon_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _damon_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install damon-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'damon-agent[google_chat]'")
        return False
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_damon_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_damon_home()}/google_chat_user_token.json"
    )
