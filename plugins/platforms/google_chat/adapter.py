...
# ``damon_plugins.platforms__google_chat.adapter`` once the plugin
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a damon MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            ...
            _damon_home = _get_damon_home()
        except (ModuleNotFoundError, ImportError):
            _damon_home = _Path.home() / ".hermes"
        self._thread_count_store = _ThreadCountStore(
            _damon_home / "google_chat_thread_counts.json"
        )
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a damon MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
...
def interactive_setup() -> None:
    ...
    The setup wizard at ``damon_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.damon/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. damon-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.damon/.env")
    print_info("Restart the gateway: damon gateway restart")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'damon-agent[google_chat]'",
