...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        try:
            ...
            damon_home = Path.home().joinpath(".hermes").resolve()
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.damon/.env`` via ``damon_cli.config``.
    """
    print()
    ...
    try:
        from damon_cli.config import get_env_var, set_env_var
    except ImportError:
        print("damon_cli.config not available; set LINE_* vars manually in ~/.damon/.env")
        return
