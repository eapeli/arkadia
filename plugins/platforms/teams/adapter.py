...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        damon_action = data.get("damon_action", "")
        session_key = data.get("session_key", "")

        if not damon_action or not session_key:
        ...
        choice = choice_map.get(damon_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="damon_approve",
                    data={**btn_data_base, "damon_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="damon_approve",
                    data={**btn_data_base, "damon_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="damon_approve",
                    data={**btn_data_base, "damon_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="damon_approve",
                    data={**btn_data_base, "damon_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print()
    print_success("Teams configuration saved to ~/.damon/.env")
