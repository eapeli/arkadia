"""
Dump command for damon CLI.

Outputs a compact, plain-text summary of the user's Damon setup
...
from damon_cli.config import get_damon_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.damon_build_sha`` by the Dockerfile's
    ``DAMON_GIT_SHA`` build-arg (see ``damon_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``damon dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from damon_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(damon_home: Path) -> int:
...
def _cron_summary(damon_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = damon_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"damon_home:      {display_damon_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `damon auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `damon auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["damon-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(damon_home)}")
    lines.append(f"  skills:             {_count_skills(damon_home)}")
