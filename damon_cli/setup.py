"""
Interactive setup wizard for Damon Agent.
...
_DOCS_BASE = "https://github.com/eapeli/damon/wiki"
...
# display_damon_home imported lazily at call sites (stale-module safety during damon update)
...
from damon_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from damon_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Damon Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Damon using environment variables or config commands:")
    print_info("  damon config set model.provider custom")
    print_info("  damon config set model.base_url http://localhost:8080/v1")
    print_info("  damon config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'damon setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from damon_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'damon setup')")


def _print_setup_summary(config: dict, damon_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'damon setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from damon_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `damon tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from damon_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'damon setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'damon setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'damon setup terminal'"))
    ...
    # Spotify (OAuth via damon auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'damon setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {damon_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('damon setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('damon setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('damon setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('damon setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('damon setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('damon config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('damon config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('damon config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('damon', Colors.GREEN)}              Start chatting")
    print(f"   {color('damon gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('damon doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `damon tools` and `damon setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``damon model``)
    ...
    provider added to ``damon model`` is automatically available here.
    ...
    # Delegate to the shared damon model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from damon_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: damon model")
    ...
    # on demand via `damon auth add`, `damon setup` vision, and
    # `damon setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``damon model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from damon_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.damon/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Damon auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via damon setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'damon setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Damon runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `damon setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `damon setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into DAMON_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("DAMON_MAX_ITERATIONS")
    ...
    print_info("  Run `damon setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # DAMON_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from damon_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_damon_home(Path(get_damon_home()))
    except Exception:
        pass
...
def _profile_name_from_damon_home(damon_home) -> str | None:
    """Return the active profile name when DAMON_HOME is a profile dir."""
    if damon_home.parent.name == "profiles":
        return damon_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Damon delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `damon update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://github.com/eapeli/damon/wiki/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under DAMON_HOME, and print
    ...
    """
    try:
        from damon_cli.slack_cli import _build_full_manifest
        from damon_pkg.constants import get_damon_home

        manifest = _build_full_manifest(
            bot_name="Damon",
            bot_description="Your Damon agent on Slack",
        )
        target = Path(get_damon_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `damon slack manifest --write` anytime to refresh after "
            "Damon adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Damon delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Damon to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from damon_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/eapeli/damon/wiki/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/eapeli/damon/wiki/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  damon config edit")
    print_info("   Open config in your editor:  damon config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from damon_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Damon from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'damon setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     damon config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_damon_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo damon gateway install --system")
                print_info("  Or run in foreground:  damon gateway")
        else:
            from damon_pkg.constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   damon gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   damon gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `damon setup tools` and `damon tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``damon_cli.auth`` — lists every supported
    ...
    """
    try:
        from damon_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from damon_cli.config
    ...
    if section_key == "model":
        ...
        from damon_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_damon.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_damon migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_damon", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Damon.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Damon to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Damon at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Damon at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Damon at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Damon at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Damon equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Damon config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Damon.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Damon's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(damon_home: Path) -> bool:
    ...
    print_info("Damon can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: damon claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=damon_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: damon claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Damon configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Damon config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Damon (use damon claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Managed Provider setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``damon setup --portal`` and ``damon portal``. This is the
    ...
    Damon session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``damon setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``damon model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``damon portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Damon Setup — Managed Provider (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Managed Provider sub.")
    print()
    print_info("  Sign up: https://#/manage-subscription")
    print()
    ...
    # setup calls, so `damon portal` == quick setup's Nous step.
    try:
        from damon_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `damon portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `damon portal`: %s", exc)
        print()
        print_error(f"  Managed Provider setup encountered an error: {exc}")
        print_info("  You can retry later with `damon portal`.")
        return
    ...
    print_info("  Run `damon portal info` to inspect routing.")
    print_info("  Run `damon` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      damon setup           — full or quick (auto-detected)
      damon setup model     — just model/provider
      damon setup tts       — just text-to-speech
      damon setup terminal  — just terminal backend
      damon setup gateway   — just messaging platforms
      damon setup tools     — just tool configuration
      damon setup agent     — just agent settings
    """
    from damon_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot Managed Provider setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Damon Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Damon Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Damon Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, damon_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Damon configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'damon setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(damon_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Damon?",
            [
                "Quick Setup (Managed Provider) — free OAuth login, no API keys, model + tools (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, damon_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {damon_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'damon config edit'")
    ...
    # Tune later with `damon setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, damon_home)


def _run_first_time_quick_setup(config: dict, damon_home, is_existing: bool):
    """Streamlined first-time setup via Managed Provider: OAuth, model, terminal & messaging.

    Routes straight to the Managed Provider provider — runs the device-code OAuth
    ...
    settings, tools); the user can customize later via ``damon setup <section>``
    or switch providers with ``damon model``.
    """
    from damon_cli.config import load_config

    # Step 1: Managed Provider — OAuth login + model selection.
    # _model_flow_nous() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Nous model picker). Provider is set to "nous" by the login/model save.
    print()
    print_header("Managed Provider")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://#/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Managed Provider setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_nous error during quick setup: %s", exc)
        print_warning(f"Managed Provider setup encountered an error: {exc}")
        print_info("You can try again later with: damon model")
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'damon setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    damon setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  damon setup gateway")
    print()

    _print_setup_summary(config, damon_home)


def _run_quick_setup(config: dict, damon_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'damon setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Damon to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'damon setup gateway'.")
