"""
Configuration management for Damon Agent.

Config files are stored in ~/.damon/ for easy access:
- ~/.damon/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.damon/.env         - API keys and secrets

This module provides:
- damon config          - Show current configuration
- damon config edit     - Open config in editor
- damon config set      - Set a specific value
- damon config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``damon config set`` (which rewrites
    ...
    ``config.yaml`` in place: damon never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.damon/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``damon logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  damon config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Damon makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Damon itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``damon update``, the TUI build.
...
#   on every plugin install / ``damon update``.
...
# * ``DAMON_HOME`` / ``DAMON_PROFILE`` / ``DAMON_CONFIG`` /
#   ``DAMON_ENV`` — Damon runtime location flags. Writing these into
...
#
# IMPORTANT: ``DAMON_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (DAMON_GEMINI_CLIENT_ID,
# DAMON_LANGFUSE_PUBLIC_KEY, DAMON_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Damon runtime location — never via dashboard env writer.
    # NOT a DAMON_* blanket: integration credentials (DAMON_GEMINI_*,
    # DAMON_LANGFUSE_*, DAMON_SPOTIFY_*, ...) ARE allowed.
    "DAMON_HOME", "DAMON_PROFILE", "DAMON_CONFIG", "DAMON_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Damon runtime location "
            "(DAMON_HOME, DAMON_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "DAMON_TOOL_PROGRESS", "DAMON_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `damon plugins enable
    # observability/langfuse` or `damon tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "DAMON_LANGFUSE_ENV",
    "DAMON_LANGFUSE_RELEASE",
    "DAMON_LANGFUSE_SAMPLE_RATE",
    "DAMON_LANGFUSE_MAX_CHARS",
    "DAMON_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from damon_cli.colors import Colors, color
from damon_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("DAMON_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_damon_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Damon is running in package-manager-managed mode.

    Two signals: the DAMON_MANAGED env var (set by the systemd service),
    or a .managed marker file in DAMON_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade damon-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Damon was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.damon/.install_method`` file (written by installers)
    2. DAMON_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``nousresearch/damon-agent`` image stamps ``docker``
    ...
    so ``damon update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_damon_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.damon/.install_method."""
    stamp = get_damon_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Damon lives in a ``uv tool`` layout.

    ``uv tool install damon-agent`` places the install at
    ``.../uv/tools/damon-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``damon-agent`` happens to be uv-tool-installed on the machine while
    the *active* Damon is a regular pip/venv install, causing
    ``damon update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/damon-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade damon-agent"
    if method == "docker":
        return "docker pull nousresearch/damon-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade damon-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade damon-agent"
        return "pip install --upgrade damon-agent"
    return "damon update"
...
# Long-form text for ``damon update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# damon_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Damon, it doesn't update
...
✗ ``damon update`` doesn't apply inside the Docker container.

Damon Agent runs as a published image (nousresearch/damon-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/damon-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate damon-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/damon-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/damon-agent/tags
  • Your config and session history live under ``$DAMON_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``damon update`` inside Docker.
...
def format_managed_message(action: str = "modify this Damon installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("DAMON_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Damon installation is managed by NixOS "
            f"(DAMON_MANAGED={env_hint}).\n"
            "Edit services.damon-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Damon installation is managed by Homebrew "
            f"(DAMON_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade damon-agent"
        )

    return (
        f"Cannot {action}: this Damon installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Damon."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from DAMON_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, damon_bin
    or None if container mode is not active, we're already inside the
    container, or DAMON_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("DAMON_DEV") == "1":
        return None

    from damon_pkg.constants import is_container
    if is_container():
        return None

    container_mode_file = get_damon_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "damon-agent")
    exec_user = info.get("exec_user", "damon")
    damon_bin = info.get("damon_bin", "/data/current-package/bin/damon")
...
# Re-export from damon_constants — canonical definition lives there.
from damon_pkg.constants import get_damon_home  # noqa: F811,E402
from pathlib import Path
...
def _resolve_damon_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the DAMON_UID / DAMON_GID env vars set by Docker deployments.

    Docker containers running Damon commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level DAMON_HOME once, but
    subdirectories created at runtime by ``ensure_damon_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("DAMON_UID", "").strip()
    gid_str = os.environ.get("DAMON_GID", "").strip()
    try:
...
def _chown_to_damon_uid(path) -> None:
    """Chown ``path`` to ``DAMON_UID:DAMON_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_damon_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_damon_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the damon group can
    share state with the gateway service.

    The mode can be overridden via the DAMON_HOME_MODE environment variable
    (e.g. DAMON_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse DAMON_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``DAMON_UID``/``DAMON_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("DAMON_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_damon_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Damon runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("DAMON_CONTAINER") or os.environ.get("DAMON_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set DAMON_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into DAMON_HOME if the user doesn't have one yet."""
...
def ensure_damon_home():
    """Ensure ~/.damon directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_damon_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_damon_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"DAMON_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Damon-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Damon
        ...
        # identity slot (SOUL.md). Empty by default. The DAMON_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Damon adds a coding operating brief
    ...
    "terminal": {
        ...
        #             DAMON_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force DAMON_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Damon auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Damon sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Damon
        ...
        #  "/home/user/.damon/cache/documents:/output"]
        ...
        # bundled Damon image, which drops to the `damon` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Damon sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Damon should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``damon chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.damon/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: damon sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Damon truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``damon kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``damon kanban decompose`` and the kanban
        ...
        # ``damon profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `damon model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `damon` (and `damon chat`) launches by default:
        ...
        # REPL and `--tui` (or DAMON_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `damon --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `damon -c` muscle memory.  Default off so existing
        # users aren't surprised.  DAMON_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `damon --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/damon`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``DAMON_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``DAMON_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``DAMON_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``DAMON_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/damon``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Damon appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.damon/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.damon/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Damon feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Damon auto-pauses the goal and
    ...
    # always goes to ~/.damon/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${DAMON_SKILL_DIR} and ${DAMON_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `damon curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `damon update`, so pruning them only
        ...
        # skipped), snapshot ~/.damon/skills/ into
        # ~/.damon/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `damon curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for damon-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Damon Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (DAMON_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.damon/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or DAMON_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `damon doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``damon_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Damon to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via DAMON_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `damon -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `damon` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `damon kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     damon-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Damon tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.damon/logs/.
    ...
    # update model picker lists without shipping a damon-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://damon-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `damon model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.damon/.env,
        ...
        # behavior: files must live under the Damon cache, under
        ...
        # user. Bridged to DAMON_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Damon
        # cache (~/.damon/cache/{documents,images,audio,video,screenshots})
        ...
        # to DAMON_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # DAMON_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to DAMON_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.damon/state.db.
    ...
    "sessions": {
        ...
        # default of ``damon sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.damon/sessions/session_{sid}.json`` on every turn
    ...
    # ``damon update`` behaviour.
    "updates": {
        # Run a full ``damon backup``-style zip of DAMON_HOME before every
        # ``damon update``.  Backups land in ``<DAMON_HOME>/backups/`` and
        # can be restored with ``damon import <path>``.  Off by default —
        # on large DAMON_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `damon update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<DAMON_HOME>/lsp/bin/`` on first use.
    ...
    # `damon tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.damon/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.damon/.env (or your shell) and never in config.yaml.
            ...
            # ~/.damon/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'damon model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Damon auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @damon:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. DAMON_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #damon)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: damon-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'damon-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Damon API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Damon API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Damon API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # DAMON_TOOL_PROGRESS and DAMON_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.damon.config`` entries, then checks
    ...
    try:
        ...
        # should never break `damon update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Damon versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``damon_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'damon setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Damon won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'damon doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("DAMON_HOME", "~/.damon")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("DAMON_TOOL_PROGRESS")
            old_mode = get_env_value("DAMON_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from DAMON_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from DAMON_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("DAMON_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from DAMON_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$DAMON_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_damon_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `damon plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `damon curator status` has no
    ...
    #   3. Creates `~/.damon/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_damon_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_damon_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from damon_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: damon config set <key> <value>")
    ...
    # metadata.damon.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _damon_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.damon/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.damon/config.yaml.
    ...
    (which change ``DAMON_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — damon auth) — OpenAI Codex
#   nous         (OAuth — damon auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.damon/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.damon/.env.
    ...
    interactive menu render (`damon tools`, `damon setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `damon tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Damon env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.damon/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.damon/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.damon/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.damon/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Damon — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Damon vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.damon/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Damon Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from damon_cli.auth import get_anthropic_key
    ...
    # Warn on stale DAMON_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("DAMON_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale DAMON_MAX_ITERATIONS={_env_ghost} "
                f"(run 'damon doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  damon config edit     # Edit config file", Colors.DIM))
    print(color("  damon config set <key> <value>", Colors.DIM))
    print(color("  damon setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: damon config set <key> <value>")
            print()
            print("Examples:")
            print("  damon config set model anthropic/claude-sonnet-4")
            print("  damon config set terminal.backend docker")
            print("  damon config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'damon config migrate' to add them")
        ...
        print("  damon config           Show current configuration")
        print("  damon config edit      Open config in editor")
        print("  damon config set <key> <value>   Set a config value")
        print("  damon config check     Check for missing/outdated config")
        print("  damon config migrate   Update config with new options")
        print("  damon config path      Show config file path")
        print("  damon config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``damon config`` UI so users
...
# =============================================================================
# Config Hot-Reload (MARK XL pattern)
# =============================================================================
# Watches config.yaml for changes and notifies registered callbacks.
# Allows live config updates without restart (toolsets, timeouts, feature flags).
# Usage:
#   from damon_cli.config import start_config_watcher, register_config_callback
#   register_config_callback(my_callback)  # callback(old_cfg, new_cfg)
#   start_config_watcher()
#   ... later ...
#   stop_config_watcher()
...
_CONFIG_WATCHER_CALLBACKS: List[Callable[[Dict, Dict], None]] = []
_CONFIG_WATCHER_LAST_MTIME: int = 0
_CONFIG_WATCHER_LOCK = threading.Lock()


def register_config_callback(callback: Callable[[Dict, Dict], None]) -> None:
    """Register a callback to be invoked when config.yaml changes.

    Callback signature: ``callback(old_config: dict, new_config: dict)``
    Called with the old and new expanded configs. Exceptions in the callback
    are logged but don't break the watcher.
    """
    with _CONFIG_WATCHER_LOCK:
        _CONFIG_WATCHER_CALLBACKS.append(callback)


def unregister_config_callback(callback: Callable[[Dict, Dict], None]) -> bool:
    """Unregister a previously registered config change callback.

    Returns True if the callback was found and removed.
    """
    with _CONFIG_WATCHER_LOCK:
        try:
            _CONFIG_WATCHER_CALLBACKS.remove(callback)
            return True
        except ValueError:
            return False
...
def _config_watcher_loop() -> None:
    """Background thread: polls config.yaml for mtime changes."""
    global _CONFIG_WATCHER_LAST_MTIME
    config_path = get_config_path()
    try:
        _CONFIG_WATCHER_LAST_MTIME = int(config_path.stat().st_mtime_ns)
    except FileNotFoundError:
        _CONFIG_WATCHER_LAST_MTIME = 0

    while not _CONFIG_WATCHER_STOP.is_set():
        # Poll interval: 2s (balance between responsiveness and CPU)
        _CONFIG_WATCHER_STOP.wait(timeout=2.0)
        if _CONFIG_WATCHER_STOP.is_set():
            break
        ...
        if current_mtime == _CONFIG_WATCHER_LAST_MTIME:
            continue

        # File changed — reload and notify
        _CONFIG_WATCHER_LAST_MTIME = current_mtime
        try:
            old_cfg = _LAST_EXPANDED_CONFIG_BY_PATH.get(str(config_path), {})
            ...
            # Notify callbacks
            with _CONFIG_WATCHER_LOCK:
                callbacks = list(_CONFIG_WATCHER_CALLBACKS)
            for cb in callbacks:
                try:
                    cb(old_cfg, new_cfg)
                except Exception as e:
                    logger.warning("Config hot-reload callback failed: %s", e)
        except Exception as e:
            logger.warning("Config hot-reload failed: %s", e)


def start_config_watcher() -> bool:
    """Start the config.yaml hot-reload watcher thread.
...
def stop_config_watcher() -> None:
    """Stop the config.yaml hot-reload watcher thread."""
    global _CONFIG_WATCHER_THREAD
    with _CONFIG_WATCHER_LOCK:
...
def is_config_watcher_running() -> bool:
    ...
    with _CONFIG_WATCHER_LOCK:
        return _CONFIG_WATCHER_THREAD is not None and _CONFIG_WATCHER_THREAD.is_alive()
