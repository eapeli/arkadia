"""Shell completion script generation for damon CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_damon_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Damon Agent bash completion
# Add to ~/.bashrc:
#   eval "$(damon completion bash)"

_damon_profiles() {{
    local profiles_dir="$HOME/.damon/profiles"
...
_damon_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_damon_profiles)" -- "$cur"))
        return
    fi
...
complete -F _damon_completion damon
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef damon
# Damon Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(damon completion zsh)"

_damon_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.damon/profiles" ]]; then
        profiles+=($HOME/.damon/profiles/*(N/:t))
    fi
...
_damon() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_damon_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'damon command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Damon Agent fish completion",
        "# Add to your config:",
        "#   damon completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __damon_profiles",
        "    echo default",
        "    if test -d $HOME/.damon/profiles",
        "        for d in $HOME/.damon/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c damon -f -s p -l profile"
        " -d 'Profile name' -xa '(__damon_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__damon_profiles)' -d 'Profile name'"
                )
