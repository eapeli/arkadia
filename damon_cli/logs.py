"""``damon logs`` — view and filter Damon log files.
...
Usage examples::
    ...
    damon logs                    # last 50 lines of agent.log
    damon logs -f                 # follow agent.log in real time
    damon logs errors             # last 50 lines of errors.log
    damon logs gateway -n 100    # last 100 lines of gateway.log
    damon logs gui -f            # follow gui.log (dashboard/pty/ws)
    damon logs desktop -f        # follow desktop.log (Electron app boot/backend)
    damon logs --level WARNING    # only WARNING+ lines
    damon logs --session abc123   # filter by session ID substring
    damon logs --component tools  # only tool-related lines
    damon logs --since 1h         # lines from the last hour
    damon logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_damon_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Damon runs — try 'damon chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_damon_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_damon_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_damon_home()}/logs/")
        return

    print(f"Log files in {display_damon_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'damon chat' to generate logs)")
