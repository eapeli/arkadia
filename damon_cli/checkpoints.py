"""`damon checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.damon/checkpoints/``.  Actions:

    damon checkpoints               # same as `status`
    damon checkpoints status        # total size, project count, breakdown
    damon checkpoints list          # per-project checkpoint counts + workdir
    damon checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    damon checkpoints clear [-f]    # nuke the entire base (asks first)
    damon checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    damon checkpoints
    damon checkpoints prune --retention-days 3 --max-size-mb 200
    damon checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: damon checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``damon checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `damon checkpoints` → status
