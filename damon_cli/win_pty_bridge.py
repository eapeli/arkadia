"""Windows ConPTY bridge for the `damon dashboard` chat tab.

Drop-in counterpart to ``damon_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``damon_cli.web_server`` consumes: ``spawn``, ``read``,
