"""``damon portal`` — the human-readable entry point for provider setup.

Running ``damon portal`` with no subcommand performs the one-shot provider
onboarding: pick a provider (OpenRouter, NovitaAI, NVIDIA NIM, etc.),
switch the inference provider, and offer to enable the Tool Gateway.
It is the friendly alias for ``damon auth add <provider>`` and runs the
same flow as the first-time quick setup.

Subcommands:
  (none)   Provider onboarding (one-shot).
  login    Explicit alias for the default one-shot onboarding.
  info     Show provider auth state + which Tool Gateway tools are routed.
  open     Open the provider subscription page in the user's default browser.
...
already in ``damon auth`` or ``damon tools``. It's the onboarding + discovery
surface for provider setup itself.
"""
...
DEFAULT_PORTAL_URL = ""
SUBSCRIPTION_URL = ""
DOCS_URL = "https://github.com/eapeli/damon/wiki/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `damon model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `damon portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `damon auth add nous --type
    oauth`. It reuses the exact wiring behind `damon setup --portal` (which in
    ...
    """
    from damon_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `damon portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `damon portal` is the
        # human-readable alias for `damon auth add nous --type oauth` /
        # `damon setup --portal`.
    ...
    print("Run `damon portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `damon portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `damon portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `damon auth add "
            "nous --type oauth`, identical to `damon setup --portal`). "
