"""Profile distributions — shareable, packaged Damon profiles via git.

A distribution is a Damon profile published as a git repository (or
...
* ``damon profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``damon skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``damon profile``, not a parallel tree):

    damon profile install <source> [--name N] [--alias] [--force] [--yes]
    damon profile update  <name>  [--force-config] [--yes]
    damon profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "damon-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            damon_requires=str(data.get("damon_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.damon_requires:
            out["damon_requires"] = self.damon_requires
        if self.author:
...
def check_damon_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Damon {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Damon distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``damon profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Damon profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Damon distribution."
        )

    # Version check up-front so we fail fast
    check_damon_requires(manifest.damon_requires, damon_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.damon).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="damon_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `damon profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`damon profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="damon_dist_update_") as tmp:
        plan = plan_install(
