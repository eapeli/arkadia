"""
Profile management for multiple isolated Damon instances.

Each profile is a fully independent DAMON_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.damon/profiles/<name>/`` by default.

The "default" profile is ``~/.damon`` itself — backward compatible,
zero migration needed.

Usage::

    damon profile create coder          # fresh profile + bundled skills
    damon profile create coder --clone  # also copy config, .env, SOUL.md, skills
    damon profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    damon -p coder chat                 # or via flag
    damon profile use coder             # set as sticky default
    damon profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See damon_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `damon profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `damon
...
# `damon skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Damon infrastructure directories that only the default profile
         (``~/.damon``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_damon_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.damon) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "damon-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "damon", "default", "test", "tmp", "root", "sudo",
})

# Damon subcommands that cannot be used as profile names/aliases
_DAMON_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the damon root, NOT to the current DAMON_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where DAMON_HOME points outside
    ``~/.damon``, profiles live under ``DAMON_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_damon_home() / "profiles"


def _get_default_damon_home() -> Path:
    """Return the default (pre-profile) DAMON_HOME path.

    In standard deployments this is ``~/.damon``.
    In Docker/custom deployments where DAMON_HOME is outside ``~/.damon``
    (e.g. ``/opt/data``), returns DAMON_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_damon_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Damon installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its DAMON_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_damon_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, damon subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _DAMON_SUBCOMMANDS:
        return f"'{canon}' conflicts with a damon subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            damon_exe = shutil.which("damon") or "damon"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(damon_exe)} -p {profile} "$@"\n')
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``damon -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``damon profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``damon profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Damon config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_damon_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``damon update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.damon)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.damon/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `damon -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Damon profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `damon update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`damon profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `damon update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `damon profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `damon -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches DAMON_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``damon profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "DAMON_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_damon_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.damon).\n"
            "To remove everything, use: damon uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``DAMON_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from damon_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set DAMON_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("DAMON_HOME")
    try:
        os.environ["DAMON_HOME"] = str(profile_dir)
        from damon_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["DAMON_HOME"] = old_home
        elif "DAMON_HOME" in os.environ:
            del os.environ["DAMON_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.damon/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: damon profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from DAMON_HOME.

    Returns ``"default"`` if DAMON_HOME is not set or points to ``~/.damon``.
    Returns the profile name if DAMON_HOME points into ``~/.damon/profiles/<name>``.
    Returns ``"custom"`` if DAMON_HOME is set to an unrecognized path.
    """
    from damon_pkg.constants import get_damon_home
    damon_home = get_damon_home()
    resolved = damon_home.resolve()

    default_resolved = _get_default_damon_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.damon itself — its parent is ~/ and its
        # directory name is ".damon", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: damon profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.damon itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.damon). "
            "Specify a different name: damon profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="damon_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"damon_{old_name}"
    legacy_old_host = f"damon.{old_name}"
    new_host = f"damon_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_damon_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("damon_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a DAMON_HOME path string.

    Called early in the CLI entry point, before any damon modules
    are imported, to set the DAMON_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
