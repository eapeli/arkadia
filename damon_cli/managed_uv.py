...
Damon owns its own uv binary at ``$DAMON_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$DAMON_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Damon keeps *its* uv binary.

    ``$DAMON_HOME/bin/uv`` on POSIX, ``$DAMON_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``damon update`` runs
    the call site from the *old*, already-imported ``damon_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``damon update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$DAMON_HOME/bin/`` instead of ``~/.local/bin/``.
    """
