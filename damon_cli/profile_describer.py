...
Used by ``damon profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``damon_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Damon Agent kanban board.
...
Rules:
  ...
  - Never write "Damon Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_damon_home())
        else:
