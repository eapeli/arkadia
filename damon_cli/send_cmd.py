"""CLI subcommand: ``damon send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"damon send: {file_path} is not a text file. --file reads the "
                ...
                f'  damon send --to telegram "MEDIA:{file_path}"\n'
                f'  damon send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  damon send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"damon send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"damon send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"damon send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"damon send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"damon send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `damon gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.damon/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_damon_env() -> None:
    """Populate ``os.environ`` from ``~/.damon/.env`` AND bridge top-level
    ...
    startup that ``damon send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.damon/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.damon/config.yaml`` into
       ...
       via ``damon config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``damon send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.damon/.env and ~/.damon/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "damon send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  damon send --to telegram \"hello\"\n"
            "  damon send --to discord:#ops --file report.md\n"
            "  damon send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "damon send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `damon send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Damon "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.damon/.env + ~/.damon/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  damon send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | damon send --to telegram:-1001234567890\n"
            "  damon send --to discord:#ops --file /tmp/report.md\n"
            "  damon send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  damon send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  damon send --list                  # all platforms\n"
            "  damon send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `damon send --list telegram`.",
    )
