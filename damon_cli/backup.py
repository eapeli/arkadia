"""
Backup and import commands for damon CLI.

`damon backup` creates a zip archive of the entire ~/.damon/ directory
(excluding the damon-agent repo and transient files).

`damon import` restores from a backup zip, overlaying onto the current
DAMON_HOME root.
"""
...
from damon_pkg.constants import get_default_damon_root, get_damon_home, display_damon_home
...
# ``damon-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/damon-agent/``
# are not accidentally excluded.
_EXCLUDED_DIRS = {
    "damon-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to damon root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``damon-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/damon-agent/`` — must be preserved.
        if part == "damon-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside DAMON_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Damon home directory."""
    damon_root = get_default_damon_root()

    if not damon_root.is_dir():
        print(f"Error: Damon home directory not found at {damon_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"damon-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"damon-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_damon_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(damon_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(damon_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``damon-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "damon-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(damon_root)
    ...
    print(f"\nRestore with: damon import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Damon backup.
    ...
    # Look for telltale files that a damon home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Damon backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.damon/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a damon dir name
        if prefix in {".damon", "damon"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_damon_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (damon_root / "config.yaml").exists()
        has_env = (damon_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Damon configuration.")
        ...
        damon_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = damon_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(damon_root.resolve())
            except ValueError:
        ...
        profiles_dir = damon_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # damon_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: damon profile list  (after installing damon)")

        # Guidance
        print()
        if not (damon_root / "damon-agent").is_dir():
            print("Note: The damon-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: damon update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  damon -p {pname} gateway install")

        print("Done. Your Damon configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and damon backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to DAMON_HOME).
...
# — `damon update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(damon_home: Optional[Path] = None) -> Path:
    home = damon_home or get_damon_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    damon_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(damon_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside DAMON_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``damon update``.
    ...
    Args:
        ...
        damon_home: Override for the Damon home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(damon_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for damon backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_damon_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, damon_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``damon_root`` to ``out_path``.
...
def _pre_update_backup_dir(damon_home: Optional[Path] = None) -> Path:
    home = damon_home or get_damon_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of DAMON_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<DAMON_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``damon update``) should continue even if the backup fails.
    """
    damon_root = damon_home or get_default_damon_root()
    if not damon_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(damon_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, damon_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `damon claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of DAMON_HOME under ``backups/`` before a
    ``damon claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``damon import <archive>``.  Writes to
    ``<DAMON_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `damon import` and the
