"""
Damon Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.damon/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Damon PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing damon-agent or damon PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Damon Agent" comment and following line
                if '# Damon Agent' in line or '# damon-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('damon' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing damon
                if 'damon' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the damon wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "damon",
        Path("/usr/local/bin/damon"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains damon_cli reference)
                content = wrapper.read_text()
                if 'damon_cli' in content or 'damon-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(damon_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``damon`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Damon home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (damon_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``damon gateway run`` processes
    ...
    try:
        from damon_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from damon_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``DAMON_HOME`` and ``DAMON_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\damon\git\cmd``, ``%LOCALAPPDATA%\damon\git\bin``,
#      ``%LOCALAPPDATA%\damon\git\usr\bin``, ``%LOCALAPPDATA%\damon\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\damon\git\`` and Node to
#      ``%LOCALAPPDATA%\damon\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``damon dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\damon\gateway-service\`` and sometimes
...
def _damon_path_markers(damon_home: Path) -> list[str]:
    """Path-entry substrings that identify Damon-owned User-PATH entries."""
    root = str(damon_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare damon-agent install dir.
    markers = [root + "\\damon-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if DAMON_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "damon".  We don't want to catch
    # unrelated entries like "cdamon-foo" or "ephermeral", so we look for
    # backslash-damon as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(damon_home: Path) -> list[str]:
    """Strip Damon-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _damon_path_markers(damon_home)
            kept: list[str] = []
...
def remove_damon_env_vars_windows() -> list[str]:
    """Delete DAMON_HOME and DAMON_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("DAMON_HOME", "DAMON_GIT_BASH_PATH"):
                try:
...
def _is_default_damon_home(damon_home: Path) -> bool:
    """Return True when ``damon_home`` points at the default (non-profile) root."""
    try:
        from damon_pkg.constants import get_default_damon_root
        return damon_home.resolve() == get_default_damon_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its DAMON_HOME directory.

    We shell out to ``damon -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current DAMON_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m damon_cli.main` so we don't depend on a `damon`
    #    wrapper that may be half-removed mid-uninstall.
    damon_invocation = [_sys.executable, "-m", "damon_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                damon_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's DAMON_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``damon uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$DAMON_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from damon_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(damon_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Damon Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Damon Chat GUI installation was found.")
        print(f"  Checked: {damon_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Damon agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(damon_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Damon agent at {damon_home / 'damon-agent'}")
        print(f"  • Your config, sessions, and secrets under {damon_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Damon agent is still installed. Run 'damon' to use the CLI,")
    print("or 'damon uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.damon/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.damon/ for future reinstall
    """
    project_root = get_project_root()
    damon_home = get_damon_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie DAMON_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_damon_home(damon_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.damon data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Damon Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {damon_home / 'config.yaml'}")
    print(f"  Secrets: {damon_home / '.env'}")
    print(f"  Data:    {damon_home / 'cron/'}, {damon_home / 'sessions/'}, {damon_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their DAMON_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Damon data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Damon code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``damon`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$DAMON_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in damon_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\damon\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(damon_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Damon-owned PATH entries in User environment")

        log_info("Removing DAMON_HOME / DAMON_GIT_BASH_PATH User env vars...")
        removed_env = remove_damon_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Damon-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing damon command...")
    ...
    #     (only when they still point into this Damon home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Damon-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(damon_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Damon-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(damon_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE DAMON_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from damon_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(damon_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.damon/, just remove the damon-agent subdir
            if damon_home in project_root.parents or project_root.parent == damon_home:
    ...
    #     under DAMON_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(damon_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(damon_home)
    ...
    # 5. Optionally remove ~/.damon/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile DAMON_HOME dirs live under
        ...
        try:
            if damon_home.exists():
                shutil.rmtree(damon_home)
                log_success(f"Removed {damon_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {damon_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {damon_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://damon-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://damon-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Damon Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m damon_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import damon_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``damon_constants`` + ``damon_cli.colors``
    (and lazily ``damon_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m damon_cli.uninstall")
    parser.add_argument(
        "--mode",
