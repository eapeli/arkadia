...
Three cookies in play:
  ...
  - damon_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/damon) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/damon`` covers
...
setter wrote ``__Secure-damon_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "damon_session_at"
SESSION_RT_COOKIE = "damon_session_rt"
PKCE_COOKIE = "damon_session_pkce"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /damon`` we want ``Path=/damon`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (damon #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/damon``)
