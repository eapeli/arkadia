...
Profile-aware location: ``$DAMON_HOME/logs/dashboard-auth.log``.
...
from ``damon_constants`` or other damon_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$DAMON_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``damon_constants.get_damon_home`` semantics: env var wins,
    else ``~/.damon``. A local copy avoids an import cycle with the
    middleware which lives below ``damon_cli``.
    """
    home = os.environ.get("DAMON_HOME") or str(Path.home() / ".damon")
