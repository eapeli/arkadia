"""
Damon Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``damon desktop`` / ``damon gui``)
     Built inside the agent checkout under ``$DAMON_HOME/damon-agent/``:
       ...
       - ``$DAMON_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Damon.app`` or ``~/Applications/Damon.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Damon`` (NSIS per-user)
...
the app name ("Damon"), separate from ``$DAMON_HOME``:
  - macOS:   ``~/Library/Application Support/Damon``
  - Windows: ``%APPDATA%\\Damon``
  - Linux:   ``$XDG_CONFIG_HOME/Damon`` (default ``~/.config/Damon``)
...
``damon uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(damon_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return damon_home / "damon-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Damon"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Damon"
...
def source_built_gui_artifacts(damon_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``damon desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``damon-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(damon_home)
    ...
    return [
        ...
        damon_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Damon" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Damon.app"),
            home / "Applications" / "Damon.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Damon).
            local_base / "Programs" / "Damon",
            # Older / alternate layout some builds used.
            local_base / "damon-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Damon")
    else:
        ...
        paths += [
            data_base / "applications" / "damon.desktop",
            data_base / "applications" / "Damon.desktop",
        ]
    return paths


def agent_is_installed(damon_home: Path) -> bool:
    """Return True when a usable Python agent install exists under DAMON_HOME.
    ...
    if (agent_root / "damon_cli").is_dir():
        return True
...
def gui_is_installed(damon_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(damon_home):
        if p.exists():
            return True
...
def gui_install_summary(damon_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = damon_home if damon_home is not None else get_damon_home()
...
def uninstall_gui(damon_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``damon-agent/damon_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$DAMON_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove damon' or "
            "'sudo dnf remove damon'). AppImage builds are a single file you "
