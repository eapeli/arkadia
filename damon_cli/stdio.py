...
on Japanese locales, etc.).  Damon's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``damon_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Damon is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``DAMON_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("DAMON_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Damon-managed Git install directories so
    ...
    # already-running shells don't see the change — which means damon
    ...
    # install without Damon-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Damon's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Damon's
    ``damon config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Damon (User env var in Windows Settings, or
    export in a PowerShell profile) and Damon picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Damon-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\damon\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``damon`` from right after install) retain their old PATH.

    Any subprocess Damon spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Damon-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "damon", "git", "cmd"),
        os.path.join(local_appdata, "damon", "git", "bin"),
        os.path.join(local_appdata, "damon", "git", "usr", "bin"),
        # Damon venv Scripts directory — host of the damon.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes damon, but harmless to include.
        os.path.join(local_appdata, "damon", "damon-agent", "venv", "Scripts"),
