...
``<root>`` is the **shared Damon root** (the parent of any active
...
``damon -p <profile>`` joins the same board as the dispatcher that
...
the profile's Damon home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``DAMON_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``DAMON_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``damon kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.damon``. In Docker / custom
deployments where ``DAMON_HOME`` points outside ``~/.damon`` (e.g.
``/opt/damon``), ``<root>`` is ``DAMON_HOME``. Legacy env-var
overrides still work:

* ``DAMON_KANBAN_DB`` — pin the database file path directly.
* ``DAMON_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``DAMON_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``DAMON_KANBAN_DB``,
``DAMON_KANBAN_WORKSPACES_ROOT``, and ``DAMON_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/damon-kanban-v1-spec.pdf`` for the full
...
# ``DAMON_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``DAMON_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("DAMON_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``DAMON_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("DAMON_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``DAMON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "DAMON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "damon_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``damon-agent``
...
def kanban_home() -> Path:
    """Return the shared Damon root that anchors the kanban board.

    Resolution order:

    1. ``DAMON_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_damon_root()``, which already returns ``<root>``
       when ``DAMON_HOME`` is ``<root>/profiles/<name>``, and returns
       ``DAMON_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``DAMON_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("DAMON_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from damon_pkg.constants import get_default_damon_root
    return get_default_damon_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``damon kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``DAMON_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``damon kanban boards
    ...
    env = os.environ.get("DAMON_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``damon kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``DAMON_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("DAMON_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``DAMON_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("DAMON_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``DAMON_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("DAMON_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``damon kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``DAMON_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``DAMON_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("DAMON_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Damon treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``DAMON_KANBAN_DB`` env → ``DAMON_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See damon_state._WAL_INCOMPAT_MARKERS for detection logic.
                from damon_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``damon kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``DAMON_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from damon_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``damon --skills ...`` alongside the built-in
    ...
    # `damon --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``damon kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `damon kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``damon kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``damon kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``damon kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``damon kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``DAMON_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Damon' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Damon-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``damon kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``DAMON_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Damon
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``damon kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Damon profile.
    ...
    if not rows:
        return False
    try:
        from damon_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from damon_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Damon profile.
        # `_default_spawn` invokes ``damon -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Damon
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `damon kanban tail` — without
...
def _module_damon_argv() -> list[str]:
    """Return the interpreter-bound Damon CLI invocation."""
    # ``damon_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``damon`` package — there is no
    # ``damon`` package to import.
    return [sys.executable, "-m", "damon_cli.main"]


def _absolute_damon_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Damon shim."""
...
def _damon_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Damon executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_damon_argv()
    return [_absolute_damon_path(path)]


def _resolve_damon_argv() -> list[str]:
    """Resolve the ``damon`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$DAMON_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("damon")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\damon.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m damon_cli.main`` — fallback for setups where
       Damon is launched from a venv and the ``damon`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_damon_bin`` for the same reason. Kept
    local (not imported from gateway) because ``damon_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("DAMON_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _damon_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _damon_path_argv(resolved_env_bin)
        return _module_damon_argv()

    damon_bin = _safe_which_no_cwd("damon") if _IS_WINDOWS else shutil.which("damon")
    if damon_bin:
        return _damon_path_argv(damon_bin)
    return _module_damon_argv()


def _kanban_worker_skill_available(damon_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``damon -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset DAMON_HOME means the worker falls back to the default root
    # home (``~/.damon``), which ships the bundled skill.
    base = _Path(damon_home) if damon_home else (_Path.home() / ".damon")
...
def _resolve_worker_cli_toolsets(damon_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``DAMON_KANBAN_TASK`` is set.
    """
    if not damon_home:
        return None
    try:
        from damon_pkg.constants import reset_damon_home_override, set_damon_home_override
        from damon_cli.config import load_config
        from damon_cli.tools_config import _get_platform_tools

        token = set_damon_home_override(damon_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for DAMON_HOME=%r (%s)",
            damon_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``damon -p <profile> chat -q ...`` subprocess.
    ...
    ``DAMON_KANBAN_DB`` / ``DAMON_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject DAMON_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `damon -p <name>` the
    # _apply_profile_override() runs *before* damon_constants is imported.
    # If DAMON_HOME is absent from the child's env, get_damon_home() falls
    # back to Path.home() / ".damon" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from damon_cli.profiles import resolve_profile_env
    try:
        env["DAMON_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via DAMON_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # DAMON_HOME never had profiles created.
        pass
    if task.tenant:
        env["DAMON_TENANT"] = task.tenant
    env["DAMON_KANBAN_TASK"] = task.id
    env["DAMON_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["DAMON_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["DAMON_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["DAMON_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["DAMON_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["DAMON_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`damon -p <name>`
    # rewrites DAMON_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_damon_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["DAMON_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["DAMON_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["DAMON_KANBAN_BOARD"] = resolved_board
    # DAMON_PROFILE is the author the kanban_comment tool defaults to.
    # `damon -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["DAMON_PROFILE"] = profile_arg

    cmd = [
        *_resolve_damon_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped DAMON_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("DAMON_HOME")):
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("DAMON_HOME"))
    if worker_toolsets:
    ...
    # `damon kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`damon` executable not found on PATH. "
            "Install Damon Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``damon kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled DAMON_PROFILE values like "damon-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Damon root exists

    Reads profile paths directly so this module has no import dependency on
    ``damon_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from damon_pkg.constants import get_default_damon_root
        default_root = get_default_damon_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``damon kanban assignees`` for the terminal.
