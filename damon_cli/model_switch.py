...
- ``damon_cli.providers``        -- canonical provider identity + overlays
- ``damon_cli.model_normalize``  -- per-provider name formatting
...
_DAMON_MODEL_WARNING = (
    "Nous Research Damon 3 & 4 models are NOT agentic and are not designed "
    "for use with Damon Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Damon 3 / Damon 4 chat families.
# The previous substring check (`"damon" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``damon-brain:qwen3-14b-ctx16k`` that just
# happen to carry "damon" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Damon-3-Llama-3.1-70B, damon-4-405b, openrouter/damon3:70b
# Negative examples it must NOT match:
#   damon-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_DAMON_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])damon[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_damon_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Damon 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_DAMON_NON_AGENTIC_RE.search(model_name))


def _check_damon_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Damon 3/4 chat model."""
    if is_nous_damon_non_agentic(model_name):
        return _DAMON_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``damon config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from damon_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'damon model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'damon doctor' — config issues detected:"
        ...
        from damon_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from damon_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from damon_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in damon_cli.runtime_provider.resolve_runtime_provider;
    ...
    damon_warn = _check_damon_model_warning(new_model)
    if damon_warn:
        warnings.append(damon_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from damon_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from damon_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from damon_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by damon provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://damon-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Damon release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from damon_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from damon_cli.models import fetch_lmstudio_models
        from damon_cli.auth import AuthError
    ...
    # --- 1. Check Damon-mapped providers ---
    from damon_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from damon_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for damon_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(damon_id)
        if (
            _alias_target
            and _alias_target != damon_id
        ...
        pconfig = PROVIDER_REGISTRY.get(damon_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (DAMON_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from damon_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(damon_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `damon model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(damon_id)
        if not model_ids:
            model_ids = curated.get(damon_id, [])
            if damon_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(damon_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Damon-only providers (nous, openai-codex, copilot, opencode-go) ---
    from damon_cli.providers import DAMON_OVERLAYS
    from damon_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Damon provider ID.
    # DAMON_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Damon IDs ("copilot").
    _mdev_to_damon = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in DAMON_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Damon slug — e.g. "github-copilot" → "copilot"
        damon_slug = _mdev_to_damon.get(pid, pid)
        if damon_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(damon_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, damon_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or damon_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(damon_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", damon_slug, exc)
        ...
        # configured.
        if not has_creds and damon_slug == "anthropic":
            try:
                ...
                if (damon_creds and damon_creds.get("accessToken")) or \
        ...
        if damon_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(damon_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(damon_slug)
                model_ids = _ids if _ids else (curated.get(damon_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(damon_slug, []) or curated.get(pid, [])
        elif damon_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `damon model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from damon_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(damon_slug, []) or curated.get(pid, [])
                if damon_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(damon_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": damon_slug,
            "name": get_label(damon_slug),
            "is_current": damon_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "damon",
        ...
        seen_slugs.add(damon_slug.lower())
        _record_builtin_endpoint(damon_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or DAMON_OVERLAYS (keeps /model in sync
    # with `damon model`).
    try:
        from damon_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Damon's canonical write key (matches
            ...
            # Damon writes ``models:`` as a dict keyed by model id
            # (see damon_cli/main.py::_save_custom_provider); older
            ...
            if api_url and api_key and discover:
                try:
                    from damon_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Damon's own writer uses; a hyphen variant is accepted
            ...
            # active model. Damon's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, damon_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`damon_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from damon_cli.models import fetch_openrouter_models
