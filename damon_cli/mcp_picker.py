"""MCP picker — interactive `damon mcp picker` (also the default `damon mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``damon mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `damon plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from damon_cli.cli_output import prompt_yes_no
from damon_cli.curses_ui import curses_single_select
from damon_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Damon session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from damon_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `damon mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: damon mcp install <name>    Picker: damon mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Damon is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Damon — run `damon update` "
...
def show_catalog() -> None:
    """`damon mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`damon mcp picker` (and default `damon mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`damon mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `damon mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
