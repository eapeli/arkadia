"""damon webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    damon webhook subscribe <name> [options]
    damon webhook list
    damon webhook remove <name>
    damon webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.damon/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: damon gateway run
"""
...
def webhook_command(args):
    """Entry point for 'damon webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: damon webhook {subscribe|list|remove|test}")
        print("Run 'damon webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (damon gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: damon webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from damon webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (damon gateway run)")
