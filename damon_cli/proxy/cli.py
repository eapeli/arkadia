"""CLI handlers for the ``damon proxy`` subcommand."""
...
from damon_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "damon proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"damon auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Damon proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Damon proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: damon proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``damon proxy <subcommand>``."""
    ...
    print(
        "damon proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  damon proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  damon proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  damon proxy providers\n"
