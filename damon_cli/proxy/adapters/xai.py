...
# xAI's public API is OpenAI-compatible for the endpoints Damon commonly
# uses. The Responses endpoint is included because Damon' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Damon-managed OAuth credentials."""

    auth_hint = "damon auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`damon auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`damon auth reset xai-oauth` or re-authenticate with "
                    "`damon auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `damon auth add xai-oauth --type oauth`."
            )
