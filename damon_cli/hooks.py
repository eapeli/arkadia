"""damon hooks — inspect and manage shell-script hooks.

Usage::

    damon hooks list
    damon hooks test <event> [--for-tool X] [--payload-file F]
    damon hooks revoke <command>
    damon hooks doctor

Consent records live under ``~/.damon/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.damon/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``damon hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: damon hooks {list|test|revoke|doctor}")
        print("Run 'damon hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.damon/config.yaml.")
        print("See `damon hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `damon hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `damon hooks test` and `damon hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Damon wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `damon hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `damon hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `damon hooks doctor`.")
