"""Slash command definitions and autocomplete for the Damon CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Damon config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Damon works on across turns until achieved", "Session",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.damon/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Damon Agent to the latest version", "Info"),
    CommandDef("version", "Show Damon Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`damon_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/damon`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Damon
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``damon skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``damon skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``damon skills list`` and
...
# unrelated command landed. These claim their slots right after /damon,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /damon <command>).
...
# native slot, the alias spelling stays reachable via /damon reset).
...
# ``/damon <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/damon" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /damon credits on Slack.
_SLACK_VIA_DAMON_ONLY = frozenset({"credits"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/damon <verb>`` subcommand.
    ...
    can still reach them via ``/damon <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/damon`` is always reserved as the first entry so the
    legacy ``/damon <subcommand>`` form keeps working for anything that
    ...
    # Reserve /damon as the catch-all top-level command.
    entries.append(("damon", "Talk to Damon or run a subcommand", "[subcommand] [args]"))
    seen.add("damon")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_DAMON_ONLY:
            # Intentionally Slack-via-/damon only (see _SLACK_VIA_DAMON_ONLY).
            return
    ...
    # everything except /damon, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://damon-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /damon handler.

    Maps both canonical names and aliases so /damon bg do stuff works
    the same as /damon background do stuff.

    Plugin-registered slash commands are included so ``/damon <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from damon_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
