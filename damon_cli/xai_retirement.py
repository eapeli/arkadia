...
Pure logic: walks a Damon config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `damon doctor` and a future `damon migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Damon config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Damon config and return retirement issues.
