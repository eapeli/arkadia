...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``damon_cli/`` — i.e. the git checkout for source
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/damon-agent`` image bakes a ``docker``
...
def _running_in_container() -> bool:
    """Thin wrapper around ``damon_constants.is_container`` (import-safe)."""
    try:
...
Hermes Agent runs as a published image (nousresearch/damon-agent), not a
...
def get_container_exec_info() -> Optional[dict]:
    ...
    damon_bin = info.get("damon_bin", "/data/current-package/bin/hermes")
