...
_UPSTREAM_REPO_URL = "https://github.com/NousResearch/damon-agent.git"
...
def check_for_updates() -> Optional[int]:
    ...
    if embedded_rev:
        ...
        # $HERMES_HOME/damon-agent/ may be a stale copy from --clone-all;
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$HERMES_HOME/damon-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/damon-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical NousResearch/damon-agent repo (forks don't get a link).
    """
