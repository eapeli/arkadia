...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.damon/auth.json`` even when HERMES_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_damon_root()`` for a profile-shaped HERMES_HOME can
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    print()
    print("Saved HERMES_SPOTIFY_CLIENT_ID to ~/.damon/.env")
    print()
    return raw
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.damon/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.damon/auth.json)."""
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.damon/auth.json even when HERMES_HOME points at a profile path
...
# ``get_default_damon_root()`` returns — ``~/.hermes`` on Linux/macOS,
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.damon/auth.json) called POST /api/oauth/token with Hermes's "
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=damon-agent` lets xAI attribute Hermes-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "https://github.com/NousResearch/damon-agent/issues/26990.",
            provider="xai-oauth",
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hermes auth store (~/.damon/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
