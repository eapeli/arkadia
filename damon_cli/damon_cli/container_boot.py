...
def reconcile_profile_gateways(
    *,
    ...
    Args:
        damon_home: The container's HERMES_HOME (typically /opt/data).
    ...
    default_prior_state = legacy_default_state or _read_desired_state(damon_home)
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off PID 1 argv, leaving the damon args.

    The container PID 1 argv looks like
    ``/init /opt/damon/docker/main-wrapper.sh <subcommand> [args...]`` and
