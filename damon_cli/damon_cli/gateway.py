...
# when damon_constants is cached from a pre-update version during `hermes update`.
...
def _sync_damon_home_from_systemd_unit(system: bool) -> None:
    ...
    :func:`get_damon_home` falls back to ``/root/.hermes`` — the wrong
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from damon_cli.service_manager import detect_service_manager, get_service_manager
...
def _profile_arg(damon_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        damon_home: Optional explicit HERMES_HOME path. Defaults to the current
...
def get_service_name() -> str:
    ...
    Default ``~/.hermes`` returns ``damon-gateway`` (backward compatible).
...
def get_launchd_plist_path() -> Path:
    ...
    Profile ``~/.damon/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _damon_home_for_target_user(target_home_dir: str) -> str:
    ...
    """
    current_hermes = get_damon_home().resolve()
...
Environment="HERMES_HOME={damon_home}"
Restart=always
RestartSec=5
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    with ``HERMES_HOME=/tmp/damon-e2e-<pr>`` exported; the restart path's
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # HERMES_HOME into the unit (manual E2E homes like /tmp/damon-e2e-NNN
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        ...
        "/opt/damon/docker/entrypoint.sh before the Hermes command."
    )
    print(
...
def _setup_weixin():
    ...
    print_info(
        "  3. Hermes will store the returned account_id/token in ~/.damon/.env."
    )
    print_info(
    ...
    if any_configured:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                print_info(
                    "  For persistence:   tmux new -s damon 'hermes gateway run'"
                )
                print_info(
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s damon 'hermes gateway run'"
                )
                print()
            ...
            print(
                "  tmux new -s damon 'hermes gateway run'         # persistent via tmux"
            )
            print(
