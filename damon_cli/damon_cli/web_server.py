...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_damon_home() at call time
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["damon", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    ...
    return _spawn_damon_action(subcommand, "gateway-restart"), False
...
async def check_damon_update(force: bool = False):
    ...
    ``POST /api/damon/update`` actually runs ``hermes update``.
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://damon-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "weixin": {
        ...
        "docs_url": "https://damon-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
    ...
    "webhook": {
        ...
        "docs_url": "https://damon-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.damon-agent.nousresearch.com"
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.damon/.anthropic_oauth.json`` — Hermes-managed PKCE flow (what
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://damon-agent.nousresearch.com/docs/guides/xai-grok-oauth",
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_damon_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_damon_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_damon_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_damon_home()`` at call time
...
def _discover_dashboard_plugins() -> list:
    ...
    3. Project plugins: ./.damon/plugins/  (only if HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
