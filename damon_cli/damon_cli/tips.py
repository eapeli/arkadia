...
TIPS = [
    ...
    # --- CLI Flags ---
    "hermes -c resumes your most recent CLI session. damon -c \"project name\" resumes by title.",
    ...
    "hermes chat --source telegram tags the session for filtering in damon sessions list.",
    ...
    "hermes auth add nous or damon auth add openai-codex sets up OAuth-based providers.",
    ...
    "hermes chat --source tool tags programmatic chats so they don't clutter damon sessions list.",
    'hermes dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'hermes sessions rename <ID> "new title" renames any past session; damon sessions delete <ID> removes one.',
    ...
    'hermes status --deep runs the full health sweep across every component; plain damon status is the quick view.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.damon/plugins/ — trust-gated by design.',
