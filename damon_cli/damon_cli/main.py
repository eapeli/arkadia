...
Usage:
    ...
    damon honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hermes + Honcho
    damon version             Show version
    damon update              Update to latest version
    damon uninstall           Uninstall Hermes Agent
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (hermes tools, damon setup, damon model) use
...
def _apply_profile_override() -> None:
    ...
    # /opt/data/profiles/coder).  If HERMES_HOME points to the damon root
    # instead (e.g. systemd hardcodes HERMES_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    damon_home_env = os.environ.get("HERMES_HOME", "")
    ...
    if profile_name is None and not os.environ.get("HERMES_S6_SUPERVISED_CHILD"):
        try:
            active_path = Path.home() / ".damon" / "active_profile"
    ...
    if profile_name is not None:
        try:
            ...
            return
        os.environ["HERMES_HOME"] = damon_home
...
def _ensure_tui_node() -> None:
    ...
    damon_home = os.environ.get("HERMES_HOME") or str(Path.home() / ".hermes")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HERMES_HOME": damon_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``hermes`` launcher present but ``damon_cli`` not importable.
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "damon_root", None):
        env["HERMES_DESKTOP_HERMES_ROOT"] = str(Path(args.damon_root).expanduser().resolve())
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://damon-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/damon-agent/archive/refs/heads/{branch}.zip"
    )
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/damon-agent.git",
    "git@github.com:NousResearch/damon-agent.git",
    "https://github.com/NousResearch/damon-agent",
    "git@github.com:NousResearch/damon-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/damon-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NousResearch/damon-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/damon-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/damon-agent.git' to add later."
            )
...
def _detect_concurrent_damon_instances(
    ...
    shims (``hermes.exe`` / ``damon-gateway.exe``) and returns ``(pid,
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``hermes.exe`` / ``damon-gateway.exe`` shims
    ...
    *,
    ...
    try:
        ...
        print(
            ...
            "Run `hermes update --force` after closing other damon processes."
        )
        return
...
def _ensure_fhs_path_guard() -> None:
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/damon-agent).
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://damon-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        if data.get("damon_requires"):
            print(f"Requires:     Hermes {data['damon_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.damon_requires:
        print(f"  Requires: Hermes {mf.damon_requires}")
...
def cmd_dashboard(args):
    ...
    if (
        ...
        # update-button guard also misfires).  get_default_damon_root()
        ...
        try:
            from damon_pkg.constants import get_default_damon_root
            env["HERMES_HOME"] = str(get_default_damon_root())
        except Exception:
...
def cmd_gateway_enroll(args):
    ...
    from damon_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def main():
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://damon-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://damon-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
