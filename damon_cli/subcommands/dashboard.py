"""``damon dashboard`` subcommand parser.

Extracted verbatim from ``damon_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Damon Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `damon dashboard` cmdlines and SIGTERM them directly — the same
    # path `damon update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running damon dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running damon dashboard processes and exit",
    )
    # Backward-compat shim: older Damon desktop app shells (<= 0.15.x) spawn the
    # backend as `damon dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Damon couldn't start" with no actionable cause. Accept and silently
    ...
    # `damon dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.damon/.env.
    # Nested subparser so bare `damon dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "DAMON_DASHBOARD_OAUTH_CLIENT_ID into ~/.damon/.env, and prints "
            "how to engage the login gate. Requires being logged in (damon setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://damon.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via DAMON_DASHBOARD_PORTAL_URL. Mainly for "
