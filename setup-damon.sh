...
# 5. Symlinks the 'damon' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/damon-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink damon into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up damon command..."
...
echo -e "${GREEN}✓${NC} Symlinked damon → $COMMAND_LINK_DISPLAY_DIR/hermes"
...
# ============================================================================
# Seed bundled skills into ~/.damon/skills/
# ============================================================================
...
echo ""
echo "Syncing bundled skills to ~/.damon/skills/ ..."
...
echo "Other commands:"
echo "  damon status        # Check configuration"
if is_termux; then
    echo "  damon gateway       # Run gateway in foreground"
else
    echo "  damon gateway install # Install gateway service (messaging + cron)"
fi
echo "  damon cron list     # View scheduled jobs"
echo "  damon doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m damon_cli.main setup
fi
