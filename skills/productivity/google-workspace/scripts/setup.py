...
# Ensure sibling modules (_damon_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _damon_home import display_damon_home, get_damon_home
...
def install_deps():
    ...
    print("  pip install 'damon-agent[google]'")
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_damon_home()}/google_token.json")
