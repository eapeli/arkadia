...
- Subscriptions persist to `~/.damon/webhook_subscriptions.json`

## How It Works

1. `hermes webhook subscribe` writes to `~/.damon/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status damon-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.damon/logs/gateway.log | tail -20`
