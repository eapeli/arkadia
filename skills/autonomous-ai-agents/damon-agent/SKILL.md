...
metadata:
  hermes:
    ...
    homepage: https://github.com/NousResearch/damon-agent
...
**Docs:** https://damon-agent.nousresearch.com/docs/
...
(`hermes mcp install <name>`): `skill_view(name="damon-agent", file_path="references/native-mcp.md")`.
...
Platform docs: https://damon-agent.nousresearch.com/docs/user-guide/messaging/
...
patterns: `skill_view(name="damon-agent", file_path="references/webhooks.md")`.
...
authoritative list or see the [live slash commands reference](https://damon-agent.nousresearch.com/docs/reference/slash-commands).
The registry of record is `damon_cli/commands.py` — every consumer
...
/reload-skills       Re-scan ~/.damon/skills/ for added/removed skills
...
```
~/.damon/config.yaml       Main configuration
~/.damon/.env              API keys and secrets (under $HERMES_HOME if set)
$HERMES_HOME/skills/        Installed skills
~/.damon/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.damon/state.db          Canonical session store (SQLite + FTS5)
~/.damon/logs/             Gateway and error logs
~/.damon/auth.json         OAuth tokens and credential pools
~/.damon/damon-agent/     Source code (if git-installed)
```

Profiles use `~/.damon/profiles/<name>/` with the same layout.
...
Full config reference: https://damon-agent.nousresearch.com/docs/user-guide/configuration
...
Full provider docs: https://damon-agent.nousresearch.com/docs/integrations/providers
...
Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.damon/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.
...
```
# Start
terminal(command="tmux new-session -d -s agent1 -x 120 -y 40 'damon'", timeout=10)
...
User docs: https://damon-agent.nousresearch.com/docs/user-guide/features/cron
...
- **Telemetry:** sidecar at `~/.damon/skills/.usage.json` holds
  per-skill `use_count`, `view_count`, `patch_count`,
  `last_activity_at`, `state`, `pinned`.

Config: `curator.*` (`enabled`, `interval_hours`, `min_idle_hours`,
`stale_after_days`, `archive_after_days`, `backup.*`).
User docs: https://damon-agent.nousresearch.com/docs/user-guide/features/curator
...
User docs: https://damon-agent.nousresearch.com/docs/user-guide/features/kanban
...
Check logs first:
```bash
grep -i "failed to send\|error" ~/.damon/logs/gateway.log | tail -20
```

Common gateway problems:
- **Gateway dies on SSH logout**: Enable linger: `sudo loginctl enable-linger $USER`
- **Gateway dies on WSL2 close**: WSL2 requires `systemd=true` in `/etc/wsl.conf` for systemd services to work. Without it, gateway falls back to `nohup` (dies when session closes).
- **Gateway crash loop**: Reset the failed state: `systemctl --user reset-failed damon-gateway`
...
|----------------|----------|
| Config options | `hermes config edit` or [Configuration docs](https://damon-agent.nousresearch.com/docs/user-guide/configuration) |
| Available tools | `hermes tools list` or [Tools reference](https://damon-agent.nousresearch.com/docs/reference/tools-reference) |
| Slash commands | `/help` in session or [Slash commands reference](https://damon-agent.nousresearch.com/docs/reference/slash-commands) |
| Skills catalog | `hermes skills browse` or [Skills catalog](https://damon-agent.nousresearch.com/docs/reference/skills-catalog) |
| Provider setup | `hermes model` or [Providers guide](https://damon-agent.nousresearch.com/docs/integrations/providers) |
| Platform setup | `hermes gateway setup` or [Messaging docs](https://damon-agent.nousresearch.com/docs/user-guide/messaging/) |
| MCP servers | `hermes mcp list` or [MCP guide](https://damon-agent.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `hermes profile list` or [Profiles docs](https://damon-agent.nousresearch.com/docs/user-guide/profiles) |
| Cron jobs | `hermes cron list` or [Cron docs](https://damon-agent.nousresearch.com/docs/user-guide/features/cron) |
| Memory | `hermes memory status` or [Memory docs](https://damon-agent.nousresearch.com/docs/user-guide/features/memory) |
| Env variables | `hermes config env-path` or [Env vars reference](https://damon-agent.nousresearch.com/docs/reference/environment-variables) |
| CLI commands | `hermes --help` or [CLI reference](https://damon-agent.nousresearch.com/docs/reference/cli-commands) |
| Gateway logs | `~/.damon/logs/gateway.log` |
| Session files | `hermes sessions browse` (reads state.db) |
| Source code | `~/.damon/damon-agent/` |

---

## Contributor Quick Reference

For occasional contributors and PR authors. Full developer docs: https://damon-agent.nousresearch.com/docs/developer-guide/
...
├── damon_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── damon_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.damon/config.yaml` (settings), `~/.damon/.env` (API keys) — both under `$HERMES_HOME` when it is set.
...
All handlers must return JSON strings. Use `get_damon_home()` for paths, never hardcode `~/.hermes`.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `damon_cli/commands.py`
...
- Tests auto-redirect `HERMES_HOME` to temp dirs — never touch real `~/.damon/`
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/damon_cli/test_auth_toctou_file_modes.py`)
- `signal.SIGALRM` → Unix-only (see `tests/conftest.py::_enforce_test_timeout`)
...
- Use `get_damon_home()` from `damon_constants` for all paths (profile-safe)
