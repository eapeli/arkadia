...
``~/.damon/logs/`` (profile-aware via ``get_damon_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``damon_cli.web_server``,
    ``damon_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from damon_pkg.constants import get_config_path, get_damon_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_damon_session_injector", False):
    ...
    _session_record_factory._damon_session_injector = True  # type: ignore[attr-defined]
...
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "damon_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("damon_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "damon_cli.web_server",
        "damon_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_damon_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._damon_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the damon group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from damon_cli.config import is_managed
