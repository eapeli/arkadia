...
## Token Lifecycle
- Store at `~/.damon/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `damon_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `damon_cli/models.py` — Add Gemini model catalog
4. `damon_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `damon_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `damon_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- damon-agent Copilot OAuth: `damon_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
