...
def damon_available() -> bool:
    return shutil.which("damon") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed damon CLI."""
    try:
        out = subprocess.run(
            ["damon", "kanban", "list", "--tenant", tenant, "--json"],
    ...
    out = subprocess.run(
        ["damon", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["damon", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not damon_available():
        print("ERROR: 'damon' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
