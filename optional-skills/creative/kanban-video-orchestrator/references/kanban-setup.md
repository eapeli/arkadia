...
3. **Configure profiles** — patch each profile's
   `~/.damon/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
...
Each profile has a YAML config at `~/.damon/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.damon/profiles/{profile}/config.yaml")
with open(p) as f:
...
Each profile gets a `SOUL.md` at `~/.damon/profiles/<name>/SOUL.md` that
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _damon_env="${HERMES_HOME:-$HOME/.hermes}/.env"
    if grep -q "^${var}=" "$_damon_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_damon_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_damon_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY damon ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY damon OPENROUTER_API_KEY || exit 1
# ...
```
