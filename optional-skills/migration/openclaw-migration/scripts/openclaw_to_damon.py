...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.damon/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.damon/tts/.",
    },
    "discord-settings": {
...
def _case_preserving_replacement(replacement: str):
    ...
    ``~/.damon/config.yaml`` (the real Hermes home) instead of the broken
    ``~/.Hermes/config.yaml``.
    """
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-damon/apply.ts "blocked by earlier
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-damon/plan.ts warnings[].
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: damon config set",
                        )
                    continue
        ...
        for oc_key, damon_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and damon_key not in secret_additions:
                secret_additions[damon_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, damon_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and damon_key not in secret_additions:
                        secret_additions[damon_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        damon_config = load_yaml_file(destination)
        current_model = damon_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = damon_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                damon_config["model"] = {"default": model_str}
            dump_yaml_file(destination, damon_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        damon_config = load_yaml_file(destination)
        existing_tts = damon_config.get("tts", {})
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        damon_cfg_path = self.target_root / "config.yaml"
        damon_cfg = load_yaml_file(damon_cfg_path)
        existing_mcp = damon_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            damon_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                damon_srv["command"] = srv["command"]
                if srv.get("args"):
                    damon_srv["args"] = srv["args"]
                if srv.get("env"):
                    damon_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    damon_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                damon_srv["url"] = srv["url"]
                if srv.get("headers"):
                    damon_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    damon_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                damon_srv["enabled"] = False
            if srv.get("timeout"):
                damon_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                damon_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                damon_srv["tools"] = {}
                if tools_cfg.get("include"):
                    damon_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    damon_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(damon_cfg_path)
            damon_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(damon_cfg_path, damon_cfg)
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = damon_cfg.get("agent") or {}
        ...
        if compaction:
            compression = damon_cfg.get("compression") or {}
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        damon_cfg_path = self.target_root / "config.yaml"
        damon_cfg = load_yaml_file(damon_cfg_path)
        sr = damon_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        damon_cfg_path = self.target_root / "config.yaml"
        damon_cfg = load_yaml_file(damon_cfg_path)
        custom_providers = damon_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(damon_cfg_path)
            damon_cfg["custom_providers"] = custom_providers
            dump_yaml_file(damon_cfg_path, damon_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            damon_cfg_path = self.target_root / "config.yaml"
            damon_cfg = load_yaml_file(damon_cfg_path)
            discord_hermes = damon_cfg.get("discord") or {}
            changed = False
            ...
            if changed and self.execute:
                damon_cfg["discord"] = discord_hermes
                dump_yaml_file(damon_cfg_path, damon_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        damon_cfg_path = self.target_root / "config.yaml"
        damon_cfg = load_yaml_file(damon_cfg_path)
        browser_hermes = damon_cfg.get("browser") or {}
        changed = False
        ...
        if changed:
            damon_cfg["browser"] = browser_hermes
            if self.execute:
                self.maybe_backup(damon_cfg_path)
                dump_yaml_file(damon_cfg_path, damon_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            damon_mode = mode_map.get(mode, "manual")
            damon_cfg.setdefault("approvals", {})["mode"] = damon_mode
            if self.execute:
                self.maybe_backup(damon_cfg_path)
                dump_yaml_file(damon_cfg_path, damon_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{damon_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "- Review `~/.damon/config.yaml` for any adjustments",
            "",
        ])
...
def main() -> int:
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.damon/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.damon/config.yaml")
        print("    2. Run: damon mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: damon cron")
